...
for i, (sub, ses_ind) in enumerate(sub_ses):
    try: 
        ...
        for ind, run_fpath in enumerate(run_list):
            ...
            new_df['sub'] = sub; new_df['ses'] = ses; new_df['run'] = run_type;
            new_df['meta'] = f"{sub}_{ses}_{run_type}"
            new_df['experiment_dur'] = run_df['param_experiment_duration']
            df_concat = total_df.append(new_df])
