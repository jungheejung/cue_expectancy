...
def load_nps_data(roi):
    """Loads NPS data and metadata for a given ROI."""
    npy_path = join(data_dir, f'{roi}_event-stimulus.npy')
    metadata_path = join(data_dir, f'{roi}_event-stimulus.json')
    
    npy_nps = np.load(npy_path)
    with open(metadata_path, 'r') as file:
        metadata = json.load(file)
    
    return npy_nps, metadata

def perform_analysis(nps_roi, roi, cuetype):
    """Performs main analysis for a given ROI and cue type."""
    print(f'______________ decoding {cuetype} ______________')
    df_pain_cuetype = nps_roi[(nps_roi['runtype'] == 'pain') & (nps_roi['cue'] == cuetype)].copy().reset_index(drop=True)
    
    # Further processing and analysis...
    # (Assuming other functions for filtering, modeling, and plotting are defined similarly and called here)
    
    # Save results
    cuename = cuetype.split('_')[0]
    f1df.to_csv(join(save_dir, f'roi-{roi}_cue-{cuename}_f1score.tsv'), sep='\t', index=False)
    accuracy_df.to_csv(join(save_dir, f'roi-{roi}_cue-{cuename}_accuracy.tsv'), sep='\t', index=False)


def preprocess_data(npy_nps, metadata, roi):
    """Preprocesses NPS data and metadata into a DataFrame."""
    singletrial_fname = metadata['filenames']
    nps_roi = pd.DataFrame({'singletrial_fname': singletrial_fname})
    
    # Assuming extract_metadata is defined elsewhere
    ...
    # Append single trial voxel weights
    prefix = f"{roi}_"
...
for roi in roi_list:
    ...
    npy_nps, metadata = load_nps_data(roi)
    nps_roi = preprocess_data(npy_nps, metadata, roi)
