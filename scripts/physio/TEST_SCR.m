...
task = 'cognitive';
dv = 'actual';
fname = '/Users/h/Dropbox/projects_dropbox/social_influence_analysis/analysis/physio/physio01_SCL/sub-all_condition-mean_runtype-pain_epochstart--1_epochend-15_samplingrate-25_ttlindex-2_physio-scltimecourse_dropNA.csv';
% fname = fullfile('/Volumes/spacetop_projects_social/analysis/physio/physio01_SCL/', strcat('sub-all_condition-mean_runtype-', task, '_epochstart--1_epochend-15_samplingrate-25_ttlindex-2_physio-scltimecourse.csv'));
...
for cue = 1:2
    for stim = 1:3
        ...
        if subset.src_subject_id == unique(T.src_subject_id)
            if dv == 'expect';
                dat_int_TEST{stim,cue} = subset.event02_expect_angle; 
            elseif dv == 'actual';
                dat_int_TEST{stim,cue} = subset.event04_actual_angle; 
            end
        end
    end
end
...
cue_expect.dat = cue_expect.dat(1:399,:);                     % Stimulus-locked 20 seconds epoch 
% cue_expect.dat = cue_expect.dat(1:200,:); 
...
set(gca, 'tickDir', 'out', 'tickLength', [.02 .02], 'linewidth', 2, 'ylim', [0 1.5], 'xlim', [1.5, 10.5], 'fontsize', 20);
if ~savefig
...
set(gca, 'ylim', [-.25, .25], 'xlim', [0 425], 'linewidth', 1.5, 'TickDir', 'out', 'TickLength', [.02 .02], 'Xtick', (0:5:24)*25, 'ytick', -.25:.1:.25);
