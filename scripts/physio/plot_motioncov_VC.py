...
parser.add_argument("--runtype", type=str,
                    help="specify runtype: pain, vicarious, cognitive")
...
physio_flist = glob.glob(os.path.join(physio_dir, sub, ses, f"{sub}_{ses}_task-cue*{runtype}*recording-ppg-eda-trigger_physio.tsv"))
for physio_fname in physio_flist:
    ...
    resampled_stim = signal.resample(physio["event_stimuli"], number_of_samples)
    ...
    threshold, upper, lower = (max(resampled_stim)-min(resampled_stim))/2, 1, 0
    confounds['trigger_stim'] = np.where(resampled_stim>threshold, upper, lower) * -1 # resampled_stim/15 
    # physio.plot(y = 'event_stimuli')
    ...
    filter_col.append('trigger_stim')
    ...
    plt.plot(confounds['physio'], alpha=1, linewidth=10, color="black")
    plt.plot(confounds['trigger_stim'], alpha=1, linewidth=2, color="blue")
