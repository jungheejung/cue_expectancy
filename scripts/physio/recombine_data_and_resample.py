...
import numpy as np
import pandas as pd
sub-0015_ses-01_run-05_runtype-pain_epochstart--1_epochend-8_physio-scltimecourse.csv

# get a list that matches this pattern
{sub}_{ses}_*_runtype-pain_epochstart--1_epochend-8_physio-scltimecourse.csv

# concatenate into dataframe while handeling info
# - sub
# - ses
# - run number
# - trial number
...
# %% stack all data ________________________
li = []
...
# %%df_new = df.append(pd.DataFrame(new_rows)).reset_index()
...
# subset_df.groupby(['src_subject_id', 'param_cue_type', 'param_stimulus_type'])['col_0':'col_224']
# Mean=subset_df.groupby(['src_subject_id', 'param_stimulus_type'])['col_0'].transform('mean')
# %% z score
zscore = pd.DataFrame(index=list(range(subset_df.shape[0])),
                      columns=['col_' + str(sub) for sub in range(225)])
for ind in np.arange(225):
    col_name = 'col_' + str(ind)
    zscore.loc[:, col_name] = subset_df.groupby(by=['src_subject_id'])[
        col_name].apply(lambda x: (x - x.mean())/x.std())

zdf = pd.concat([metadata_df[['src_subject_id', 'param_cue_type',
                'param_stimulus_type']], zscore], axis=1)

# %% average signal per condition
# meanval = pd.DataFrame(index = list(range(len(subset_df.src_subject_id.unique()))),
# columns = ['col_' + str(sub) for sub in range(225)])

# X = zdf.groupby(by = ['src_subject_id' , 'param_cue_type', 'param_stimulus_type']).apply(lambda x: x.mean())
# X.drop(columns = ['src_subject_id'], inplace=True)
# Xr = X.reset_index()
...
def plot_condition_timeseries(df, cond, level_list, col_start, col_end, color_list, line_style):
    for ind, stim in enumerate(level_list):
        ...
        plt.fill_between(timeseries, stim_mean - stim_sd/np.sqrt(N), stim_mean +
                         stim_sd/np.sqrt(N), color=f'{color_list[ind]}', alpha=0.2)
    plt.plot()
    # plt.show()
