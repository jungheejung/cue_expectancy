...
function FIR_spm_cue(sub, onset_dir, main_dir, fmriprep_dir, badruns_json, save_dir)
disp(strcat('--------------------',sub,'----------------'));
TR = 0.46;
T = 20;
mode = 0;
...
% ________________________________ Create a structure similar to the Python dictionary ________________________________
...
% ________________________________ find nifti files ________________________________
...
% ________________________________ find onset files ________________________________
...
% ________________________________ load badruns from json _________________________________________________________________
...
% ________________________________ intersection of nifti and onset files ________________________________
...
% ________________________________ construct onset table based on events table ________________________________
events = readtable(onset_fname, "FileType","delimitedtext");

cueH = events(strcmp(events.pmod_cuetype , 'high_cue'), :);
cueL = events(strcmp(events.pmod_cuetype , 'low_cue'), :);

TR = 0.46;

onset_stim          = round(events.onset03_stim/TR);
onset_epochCue_cueH = round(cueH.onset01_cue/TR);
onset_epochCue_cueL = round(cueL.onset01_cue/TR);
onset_rating        = [round(events.onset02_ratingexpect/TR); round(events.onset04_ratingoutcome/TR)];

onset_stim_Time = zeros(872,1);                onset_stim_Time(onset_stim)=1;
onset_epochCue_cueH_Time = zeros(872,1);       onset_epochCue_cueH_Time(onset_epochCue_cueH)=1;
onset_epochCue_cueL_Time = zeros(872,1);       onset_epochCue_cueL_Time(onset_epochCue_cueL)=1;
onset_rating_Time = zeros(872,1);              onset_rating_Time(onset_rating)=1;

Runc = { onset_epochCue_cueH_Time onset_epochCue_cueL_Time onset_stim_Time  onset_rating_Time};
...
for i = 1:length(fields)
    ...
    array = rois.(key);
    function_name(key, array);
    % ________________________________ extract time series via hrf_fit_onevoxel function ________________________________
    ...
    %  ________________________________ Create values for the additional columns to be appended ________________________________
    ...
    ROI_col = repmat(key, num_conditions, 1);    condition_col = {"cueH";  "cueL"; "stimulus" ;"rating" };
    disp(sub_col);    
    disp(size(sub_col)); disp(size(data));
    % ________________________________ Combine the data and additional columns ________________________________
    combinedDataCell = [];
    ...
    % ________________________________ Create a table from the combined data ________________________________
    ...
    % plot conditions and save ________________________________________________________________
    plot(h(:,1), 'color', 'red', 'LineStyle', '-');
    hold on;
    plot(h(:,2), 'color', 'blue', 'LineStyle', '--');
