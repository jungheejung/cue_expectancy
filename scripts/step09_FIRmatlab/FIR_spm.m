...
for i = 1:length(fields)
    ...
    disp(dataTable);
    save_fname = fullfile(save_dir, sub, strcat(sub,'_',ses,'_',run,'_runtype-',runtype{1},'-roi-',key,'_tr-',size(data,2),'.tsv' ));
    writetable(dataTable, 'sample_data.csv');
    %     H_All_Reg(:,sub,i)=h(:,1);
    %     H_All_Neg(:,sub,i)=h(:,2);
    %     H_All_Neu(:,sub,i)=h(:,3);
...
%     save TODO:
    save_plotname = fullfile(save_dir, sub, strcat(sub,'_',ses,'_',run,'_runtype-',runtype{1},'-roi-',key,'_tr-',size(data,2),'.png' ));
