...
for sub_ind = 1:length(sub_list)
    ...
    disp('=== Checking npspos structure ===')
    disp(['npspos class: ', class(npspos)])
    disp(['npspos size: ', num2str(size(npspos))])
    if isobject(npspos)
        disp(['npspos is object array of length: ', num2str(length(npspos))])
        if length(npspos) > 0
            disp(['First element .dat size: ', num2str(size(npspos(1).dat))])
        end
    else
        disp(['npspos.dat size: ', num2str(size(npspos.dat))])
    end
    %% Prepare region names (assumes pos and neg regions are constant across files)
    if sub_ind == 1 && isempty(resultTable)
...
% dataRow = [ image_names{:}, nps_values{:}, [npspos(:).dat], [npsneg(:).dat],npspos_exp_by_region{:}, npsneg_exp_by_region{:} ];
...
% Don't unpack the cells - keep everything as cells
dataRow = {image_names, nps_values, npspos.dat, npsneg.dat, npspos_exp_by_region, npsneg_exp_by_region};

% Now flatten properly for each trial
num_trials = length(image_names);
allRows = cell(num_trials, numel(allRegionNames));

for trial_idx = 1:num_trials
    allRows(trial_idx, :) = [image_names(trial_idx), ...
                              num2cell(nps_values(trial_idx)), ...
                              num2cell(npspos.dat(trial_idx)), ...
                              num2cell(npsneg.dat(trial_idx)), ...
                              num2cell(npspos_exp_by_region{trial_idx}), ...
                              num2cell(npsneg_exp_by_region{trial_idx})];
end

% Create the table
subtable = cell2table(allRows, 'VariableNames', allRegionNames);
    subtable = newRow;


    resultTable = [subtable; newRow];

writetable(subtable, fullfile(main_dir, strcat(sub,'_CANlab_applyNPS_singletrial_rampupplateau_corr.csv')));
end

%% Save the result table to a file (optional)
writetable(resultTable, fullfile(main_dir, 'CANlab_applyNPS_singletrial_rampupplateau_corr.csv'));
% end
