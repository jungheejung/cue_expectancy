...
save_dir = '/dartfs-hpc/rc/lab/C/CANlab/labdata/projects/spacetop_projects_cue/analysis/fmri/nilearn/deriv01_signature/rampup_plateau/NPS';
...
% Metadata details
metadata = struct();
metadata.code_name = 'step02_apply_NPS_corr';
...
output_path = fullfile(save_dir, strcat('sub-all_signature-NPScorr_runtype-pvc_event-', key, '.json'));%fullfile(metadata.output_directory, 'metadata.json');
...
skipped_file_path = fullfile(save_dir, 'skipped_subjects.txt');

% Write the skipped subjects to a text file
if ~isempty(skipped_subjects)
    % For MATLAB R2022a and newer
    writelines(skipped_subjects', skipped_file_path);  % Note the transpose (') for column format
    
    % For older MATLAB versions, use fprintf
    % Open the file
    fid = fopen(skipped_file_path, 'w');
    if fid == -1
        error('Cannot create skipped subjects file');
    end
    % Write each subject on a new line
