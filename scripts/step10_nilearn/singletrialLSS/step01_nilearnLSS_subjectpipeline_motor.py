...
def restructure_task_cue_beh_NOttl(beh_fname):
    import pandas as pd
    import numpy as np
    beh = pd.read_csv(beh_fname, sep = '\t')
    beh_tsv = os.path.basename(beh_fname)
    ...
    # Create a new Pandas Series with the mapped values
    rampup = pd.Series(beh['pmod_stimtype']).map(rampup_mapping)
    rampdown = pd.Series(beh['pmod_stimtype']).map(rampdown_mapping)
    onset05_rampup = pd.DataFrame({
        'onset' : list(beh['onset03_stim'] + rampup),
        'duration' : list(rampup),
        'trial_type' : list(np.repeat('stimulus',len(beh['onset03_stim']))),
        'sub': list(np.repeat(sub,len(beh['onset03_stim']))),
        'ses': list(np.repeat(ses,len(beh['onset03_stim']))),
        'run': list(np.repeat(run,len(beh['onset03_stim']))),
    ...
    onset06_rampdown = pd.DataFrame({
        'onset' : list(beh['onset03_stim'] + rampdown + 5 ),
        'duration' : list(rampdown),
        'trial_type' : list(np.repeat('stimulus',len(beh['onset03_stim']))),
        'sub': list(np.repeat(sub,len(beh['onset03_stim']))),
        'ses': list(np.repeat(ses,len(beh['onset03_stim']))),
        'run': list(np.repeat(run,len(beh['onset03_stim']))),
...
def restructure_task_cue_beh_painttl(beh_fname):
    import pandas as pd
    import numpy as np
    beh = pd.read_csv(beh_fname, sep = '\t')
    beh_tsv = os.path.basename(beh_fname)
    ...
    onset05_rampup = pd.DataFrame({
        'onset' : list(beh['TTL1'] ),
        'duration' : list(beh['TTL2'] - beh['TTL1']),
        'trial_type' : list(np.repeat('stimulus',len(beh['onset03_stim']))),
        'sub': list(np.repeat(sub,len(beh['onset03_stim']))),
        'ses': list(np.repeat(ses,len(beh['onset03_stim']))),
        'run': list(np.repeat(run,len(beh['onset03_stim']))),
    ...
    onset06_rampdown = pd.DataFrame({
        'onset' : list(beh['TTL3'] ),
        'duration' : list(beh['TTL4'] - beh['TTL3'] ),
        'trial_type' : list(np.repeat('stimulus',len(beh['onset03_stim']))),
        'sub': list(np.repeat(sub,len(beh['onset03_stim']))),
        'ses': list(np.repeat(ses,len(beh['onset03_stim']))),
        'run': list(np.repeat(run,len(beh['onset03_stim']))),
...
save_singletrial_dir = '/dartfs-hpc/scratch/f0042x1/singletrial_rampupdown_motor'
save_fig_dir = '/dartfs-hpc/rc/lab/C/CANlab/labdata/projects/spacetop_projects_cue/figure/fmri/nilearn/singletrial_rampupdown'
...
def ttl_exists(beh_fname):
    ...
    return os.path.exists(ttl_fname)
...
for beh_fname in beh_list:
    ...
    singletrial_list = events_df.loc[(events_df['trial_type'] == 'expectrating') | (events_df['trial_type'] == 'outcomerating')].index.tolist()
