...
def restructure_task_cue_beh(beh_fname):
    ...
    sub_num = int(
        [match for match in beh_tsv.split("_") if "sub" in match][0].split("-")[1]
    )
    ses_num = int(
        [match for match in beh_tsv.split("_") if "ses" in match][0].split("-")[1]
    )
    run_num = int(
        [match for match in beh_tsv.split("_") if "run" in match][0].split("-")[1]
    )
    runtype = [match for match in beh_tsv.split("_") if "runtype" in match][0].split(
        "-"
    )[1]
    ...
    onset01_cue = pd.DataFrame(
        {
            ...
            "singletrial_fname": [
                f"{sub}_{ses}_{run}_runtype-{runtype}_event-cue_trial-{i:03d}_cuetype-{cuetype_list[i]}.nii.gz"
                for i in range(len(beh["onset01_cue"]))
            ],  # for trial_num in range(len(beh['onset01_cue']))]
        }
    )
    onset02_expectrating = pd.DataFrame(
        {
            ...
            "trial_type": list(
                np.repeat("expectrating", len(beh["onset02_ratingexpect"]))
            ),
            ...
            "singletrial_fname": [
                f"{sub}_{ses}_{run}_runtype-{runtype}_event-expectrating_trial-{i:03d}_cuetype-{cuetype_list[i]}.nii.gz"
                for i in range(len(beh["onset02_ratingexpect"]))
            ],
        }
    )
    ...
    onset03_stim = pd.DataFrame(
        {
            ...
            "singletrial_fname": [
                f"{sub}_{ses}_{run}_runtype-{runtype}_event-stimulus_trial-{i:03d}_cuetype-{cuetype_list[i]}_stimintensity-{stimtype_list[i]}.nii.gz"
                for i in range(len(beh["onset03_stim"]))
            ],
        }
    )
    ...
    onset04_outcomerating = pd.DataFrame(
        {
            ...
            "trial_type": list(
                np.repeat("outcomerating", len(beh["onset04_ratingoutcome"]))
            ),
            ...
            "outcomerating": beh.pmod_outcomeangle,
            "singletrial_fname": [
                f"{sub}_{ses}_{run}_runtype-{runtype}_event-outcomerating_trial-{i:03d}_cuetype-{cuetype_list[i]}_stimintensity-{stimtype_list[i]}.nii.gz"
                for i in range(len(beh["onset04_ratingoutcome"]))
            ],
        }
    )
    events_df = pd.concat(
        [onset01_cue, onset02_expectrating, onset03_stim, onset04_outcomerating]
    )
...
def utils_globrunlist(beh_list, key="run", stringlist_to_keep=["ttl"]):
    ...
    bids_info = [
        [match for match in os.path.basename(beh_fname).split("_") if key in match][0]
...
parser.add_argument("--session-num", type=int, help="specify slurm array id")
...
save_singletrial_dir = join(main_dir, "analysis/fmri/nilearn/singletrial")
save_fig_dir = join(main_dir, "figure/fmri/nilearn/singletrial")
...
beh_list = glob.glob(
    os.path.join(onset_dir, sub, ses, f"{sub}_{ses}_task-cue_*runtype-*.tsv")
)
...
for beh_fname in beh_clean_list:
    print(beh_fname)
    run_info = [
        match for match in os.path.basename(beh_fname).split("_") if "run" in match
    ][0]
    run_num = int(re.findall(r"\d+", run_info)[0].lstrip("0"))
    run_type = [
        match for match in os.path.basename(beh_fname).split("_") if "runtype" in match
    ][0].split("-")[1]
    run = f"run-{run_num:02d}"
    ...
    fmri_file = os.path.join(
        ...
        ses,
        "func",
        f"{sub}_{ses}_task-social_acq-mb8_run-{run_num}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz",
    )
    confounds_file = os.path.join(
        ...
        ses,
        "func",
        f"{sub}_{ses}_task-social_acq-mb8_run-{run_num}_desc-confounds_timeseries.tsv",
    )
    ...
    singletrial_list = events_df.loc[
        (events_df["trial_type"] == "cue") | (events_df["trial_type"] == "stimulus")
    ].index.tolist()
    for i_trial in singletrial_list:
        ...
        nib.save(
            beta_map,
            os.path.join(save_singletrial_subdir, events_df.singletrial_fname[i_trial]),
        )
