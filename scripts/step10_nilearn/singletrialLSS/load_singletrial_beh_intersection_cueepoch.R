...
construct_trial_name <- function(trial_index, cue, sub, ses, run, runtype) {
  sprintf("%s_%s_%s_runtype-%s_event-cue_trial-%03d_cuetype-%s.nii.gz",
          sub, ses, run, runtype, trial_index, 
          gsub("_cue", "", cue))
}
...
for (sub_dir in sub_dirs) {
  ...
  for (file_path in event_files) {
    ...
    stim <- df[df$trial_type == "cue", c("trial_index", "cue")]
    ...
    stim$singletrial_fname <- stim %>%
      ...
      pmap_chr(function(trial_index, cue,  sub, ses, run, runtype) {
        construct_trial_name(trial_index, cue,  sub, ses, run, runtype)
      })
    stim <- stim[c( "sub","ses","run","runtype","trial_index", "cue","singletrial_fname")]
...
write.table(final_df, 
            file = file.path(events_dir, "sub-all_task-all_epoch-cue_events.tsv"), 
            sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)

myjson <- list(dataframe_description = "This is a metadata file of behavioral data, 
  intended to serve the single trials repo in 
  `/dartfs-hpc/rc/lab/C/CANlab/labdata/projects/spacetop_projects_cue/analysis/fmri/nilearn/singletrial_rampupplateau`",
  cue = "scripts/step10_nilearn/singletrialLSS/load_singletrial_beh_intersection_cueepoch.R",
  input = "single trials data from `/dartfs-hpc/rc/lab/C/CANlab/labdata/projects/spacetop_projects_cue/analysis/fmri/nilearn/singletrial_rampupplateau`
  and behavioral data from BIDS"
)
write_json(myjson, file.path(events_dir, "sub-all_task-all_epoch-cue_events.json"))