...
beh_list = glob.glob(
    os.path.join(onset_dir, sub, ses, f"{sub}_{ses}_task-cue_*runtype-*events.tsv")
)
...
# for index in sorted(remove_idx, reverse=True):
#     del beh_list_copy[index]
# beh_clean_list = sorted(utils_globrunlist(beh_list, key = 'run', stringlist_to_keep=['ttl']))
