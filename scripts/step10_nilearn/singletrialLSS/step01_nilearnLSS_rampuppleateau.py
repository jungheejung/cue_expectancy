...
def restructure_task_cue_beh_NOttl(beh_fname):
    ...
    onset03_stim = pd.DataFrame({
        'onset' : list(beh['onset03_stim']),
        'duration' : list(rampup + 5),#list(np.repeat(5,len(beh['onset03_stim']))),
        'trial_type' : list(np.repeat('stimulus',len(beh['onset03_stim']))),
        # 'full_trial_type' : list('event-stimulus_cue-' + beh.pmod_cuetype.str.split('_').str.get(0) + '_stim-' + beh.pmod_stimtype.str.split('_').str.get(0)),
        'sub': list(np.repeat(sub,len(beh['onset03_stim']))),
        'ses': list(np.repeat(ses,len(beh['onset03_stim']))),
...
def restructure_task_cue_beh_painttl(beh_fname):
    ...
    onset03_stim = pd.DataFrame({
        'onset' : list(beh['TTL1']),
        'duration' : list(beh['TTL3'] - beh['TTL1']), #list(np.repeat(5,len(beh['onset03_stim']))),
        'trial_type' : list(np.repeat('stimulus',len(beh['onset03_stim']))),
        # 'full_trial_type' : list('event-stimulus_cue-' + beh.pmod_cuetype.str.split('_').str.get(0) + '_stim-' + beh.pmod_stimtype.str.split('_').str.get(0)),
        'sub': list(np.repeat(sub,len(beh['onset03_stim']))),
        'ses': list(np.repeat(ses,len(beh['onset03_stim']))),
...
fmriprep_dir = '/dartfs-hpc/rc/lab/C/CANlab/labdata/projects/spacetop_projects_cue/analysis/fmri/smooth6mm'#'/dartfs-hpc/rc/lab/C/CANlab/labdata/data/spacetop_data/derivatives/fmriprep/results/fmriprep'
# save_singletrial_dir = '/dartfs-hpc/rc/lab/C/CANlab/labdata/projects/spacetop_projects_cue/singletrial_rampupdown'
...
for beh_fname in beh_list:
    ...
    for i_trial in singletrial_list:
        ...
        vif_fname = 'vif_' + description + '.tsv'
        Path(join(save_singletrial_dir, 'vif')).mkdir(parents = True, exist_ok = True)
