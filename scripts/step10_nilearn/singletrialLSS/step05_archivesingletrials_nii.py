...
singletrial_dir = join( '/Volumes/seagate/cue_singletrials/uncompressed_singletrial_rampupplateau')
designated_dir = join( '/Volumes/seagate/cue_singletrials/uncompressed_singletrial_rampupplateau/archive')
...
for basename in singletrial_basename:
    ...
    singletrial_fname = join(singletrial_dir, sub_bids, basename + '.nii')
...
for subject, session_runs in badjson.items():
    for session_run in session_runs:
        ...
        filename_pattern = f"{subject}_{session_run_formatted_updated}_*.nii"
