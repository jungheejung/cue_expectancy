...
singletrial_dir = "/dartfs-hpc/rc/lab/C/CANlab/labdata/projects/spacetop_projects_cue/analysis/fmri/nilearn/singletrial_rampupdown"
save_signaturedir = "/dartfs-hpc/rc/lab/C/CANlab/labdata/projects/spacetop_projects_cue/analysis/fmri/nilearn/deriv01_signature/rampupdown"
key = "NPS"
...
mask_priv_fname = glob.glob(
    os.path.join(mask_priv_dir, "**", signature_dict[key]), recursive=True
)
mul_sig_fname = glob.glob(
    os.path.join(mul_sig_dir, "**", signature_dict[key]), recursive=True
)
...
display = plotting.plot_stat_map(
    image.mean_img(stacked_singletrial),
    display_mode="mosaic",
    cut_coords=(5, 4, 10),
    title="display_mode='z', cut_coords=5",
)
...
# %% -------------------------------------------------------------------------
#  apply signature and save extracted values
# ----------------------------------------------------------------------------
...
nps_df.to_csv(
    os.path.join(
        save_signaturedir,
        f"signature-{key}_sub-{save_sub}_runtype-{runtype}_event-{event}.tsv",
    )
)
