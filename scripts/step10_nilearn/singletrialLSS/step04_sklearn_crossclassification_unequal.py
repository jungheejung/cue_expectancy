...
X_pain, paindf = get_Xdata(singletrial_dir, sub_list = sub_list, runtype = 'pain', event='stimulus', ntrials=36, mask_fname = mask_fname, badruns_fname = badruns_fname)
...
# TODO: assert paindf['sub'] == cogdf['sub']
...
for train_index, test_index in cv.split(X_pain, y):
    ...
    clf.fit(X_pain_train, y[train_index]) # TODO: remove the transpose. X_pain, y. shape should match for axis 0
    y_cog = clf.predict(X_cog_test) # TODO: standard scalar. .fit_transform(X_train) -> mean/sd of the transformed data
