...
def _event_sort(df, ind_first, ev, ev_name, dur, mod, regressor, cue_type, stim_type):
    ...
    df.loc[ind_first : ind_first + len(df[ev]) - 1, "onset"] = (
        df[ev] - df["param_trigger_onset"]
    )
...
for sub in sub_list:
    ...
    if beh_list:
        for ind, fpath in enumerate(sorted(beh_list)):
            ...
            sub_num = int(
                re.findall("\d+", [match for match in fname.split("_") if "sub" in match][0])[0])
            ses_num = int(
                re.findall(
                    "\d+", [match for match in fname.split("_") if "ses" in match][0]
                )[0]
            )
            run_num = int(
                re.findall(
                    "\d+", [match for match in fname.split("_") if "run" in match][0]
                )[0]
            )
            ...
            trial_num = len(df.event03_stimulus_displayonset)
            new.loc[cue_num : cue_num + trial_num - 1, "onset"] = list(
                df["event03_stim_ttl-plateau"]
            )
            new.loc[cue_num : cue_num + trial_num - 1, "ev"] = "stim"
            new.loc[cue_num : cue_num + trial_num - 1, "dur"] = list(
                df["event03_stim_ttl-plateau-dur"]
            )
            ...
            new.loc[cue_num : cue_num + trial_num - 1, "cue_type"] = list(
                df.param_cue_type
            )
            new.loc[cue_num : cue_num + trial_num - 1, "stim_type"] = list(
                df.param_stimulus_type
            )

            # expect actual rating ________________________________________________________________
            new.loc[0 : cue_num + trial_num - 1, "expect_rating"] = list(
                pd.concat([df.event02_expect_angle_demean] * 2, ignore_index=True)
            )
            new.loc[0 : cue_num + trial_num - 1, "actual_rating"] = list(
                pd.concat([df.event04_actual_angle_demean] * 2, ignore_index=True)
            )
            ...
            rating = pd.concat(
                [df.event02_expect_displayonset, df.event04_actual_displayonset]
            ).reset_index(drop=True)
            rt = pd.concat([df.event02_expect_RT, df.event04_actual_RT]).reset_index(
                drop=True
            )
            ...
            rate_df.sort_values(
                by=["rating"], ascending=True, inplace=True, ignore_index=True
            )
            ...
            matlabname = (
                f"{sub}_ses-{ses_num:02d}_run-{run_num:02d}_rating_{keyword}.csv"
            )
            matlab_rating.to_csv(
                os.path.join(single_dir, sub, matlabname),
                index=False,
                header=["rating", "rt"],
            )  # sub-####_ses-##_run-##_event-rating.csv

            new.loc[0 : cue_num + trial_num - 1, "cue_con"] = pd.concat(
                [df["param_cue_type"].map(dict_cue)] * 2, ignore_index=True
            )
            new.loc[0 : cue_num + trial_num - 1, "stim_lin"] = pd.concat(
                [df["param_stimulus_type"].map(dict_stim)] * 2, ignore_index=True
            )
            new.loc[0 : cue_num + trial_num - 1, "stim_quad"] = pd.concat(
                [df["param_stimulus_type"].map(dict_stim_q)] * 2, ignore_index=True
            )
            #
            new["sub"] = sub_num
            new["ses"] = ses_num
            ...
            new["nifti_name"] = (
                "sub-"
                + new["sub"].astype(str).str.zfill(4)
                + "_ses-"
                + new["ses"].astype(str).str.zfill(2)
                + "_run-"
                + new["run"].astype(str).str.zfill(2)
                + "-"
                + new["task"]
                + "_ev-"
                + new["ev"]
                + "-"
                + new["num"].astype(str).str.zfill(4)
            )
            # print(f"sub-{new.sub:04d}_ses-{new['ses']:02d}_run-{new['run']:02d}*.nii.gz")

            # dummy regressors
            range_start = cue_num + trial_num + 1
            range_end = cue_num + trial_num + nuissance_num + 1
            new.loc[range_start : range_end,"nifti_name",] = list(nuissance)
            new["sub"] = sub_num
            new.loc[range_start : range_end, "ses"] = ses_num
            new.loc[range_start : range_end, "run"] = run_num
            new.loc[range_start : range_end, "task",] = task_name
            new.loc[range_start : range_end, "regressor",] = False
            subject_dataframe = subject_dataframe.append(new)

        subject_dataframe.reset_index(inplace=True)
        subject_dataframe.to_csv(
            os.path.join(single_dir, sub, f"{sub}_singletrial_{keyword}.csv"),
            index=False,
        )
    else:
