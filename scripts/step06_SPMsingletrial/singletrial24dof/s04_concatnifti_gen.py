...
nifti_dir = os.path.join(main_dir, 'analysis', 'fmri', 'spm', 'multivariate_24dofcsd', 's02_isolatenifti')
outputnifti_dir = os.path.join(main_dir, 'analysis', 'fmri', 'spm', 'multivariate_24dofcsd', 's03_concatnifti')
...
event_list = ['cue', 'stim']
total_list = list(itertools.product(sub_list, event_list))
for i, (sub, event) in enumerate(total_list):

    print("nilearn file concatenation")
    print(f"subject: {sub}")
    niis = glob.glob(join(nifti_dir, sub, f"{sub}_*_run-*-*_task-social_ev-{event}*.nii"))
    if niis:
        ...
        concat_fname = join(concat_dir, f"{sub}_task-social_run-general_ev-{event}.nii")
        concat_nii.to_filename(concat_fname)
        concat_txt_fname = join(concat_dir, f"niftifname_{sub}_task-social_run-general_ev-{event}.txt")
