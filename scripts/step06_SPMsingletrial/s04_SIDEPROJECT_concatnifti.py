...
for i, (sub, run_type, event) in enumerate(total_list):
    ...
    if niis:
        if run_type=='vicarious' or run_type=='cognitive':
            ...
            concat_fname = join(outputnifti_dir, sub, f"{sub}_task-social_run-{run_type}_ev-{event}.nii")
            concat_nii.to_filename(concat_fname)
            concat_txt_fname = join(outputnifti_dir, sub, f"niftifname_{sub}_task-social_run-{run_type}_ev-{event}.txt")
        ...
        if run_type=='pain':
            nifti_list = sorted(niis)
            concat_nii = nl.image.concat_imgs(nifti_list, memory_level=0, auto_resample=False, verbose=0)
            concat_fname = join(outputnifti_dir, sub, f"{sub}_task-social_run-pain_ev-{event}.nii")
            concat_nii.to_filename(concat_fname)
            concat_txt_fname = join(outputnifti_dir, sub, f"niftifname_{sub}_task-social_run-{run_type}_ev-{event}.txt")
            _rewrite_concatfname(concat_txt_fname, nifti_list)
            print(concat_nii)
