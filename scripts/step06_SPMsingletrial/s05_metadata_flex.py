...
# load niftifname txt 
    subject_csv = os.path.join(main_dir, 'data', 'dartmouth', ttl_dict[task], sub, f"{sub}_singletrial_{task}.csv" )
    nifti_fname = os.path.join(nifti_dir, sub, f"niftifname_{sub}_task-pain-{task}_ev-{ev}.txt")
    if os.path.exists(subject_csv) & os.path.exists(nifti_fname):
        ...
        nifti['sub'] = nifti['fname'].str.split(f'sub-(\d+)_ses-(\d+)_run-(\d+)-([A-Za-z-]+)_task-social_ev-([A-Za-z-]+)-(\d+)', expand=True)[1].astype(int)
        nifti['ses'] = nifti['fname'].str.split(f'sub-(\d+)_ses-(\d+)_run-(\d+)-([A-Za-z-]+)_task-social_ev-([A-Za-z-]+)-(\d+)', expand=True)[2].astype(int)
        nifti['run'] = nifti['fname'].str.split(f'sub-(\d+)_ses-(\d+)_run-(\d+)-([A-Za-z-]+)_task-social_ev-([A-Za-z-]+)-(\d+)', expand=True)[3].astype(int)
        nifti['task'] = nifti['fname'].str.split(f'sub-(\d+)_ses-(\d+)_run-(\d+)-([A-Za-z-]+)_task-social_ev-([A-Za-z-]+)-(\d+)', expand=True)[4]
        nifti['ev'] = nifti['fname'].str.split(f'sub-(\d+)_ses-(\d+)_run-(\d+)-([A-Za-z-]+)_task-social_ev-([A-Za-z-]+)-(\d+)', expand=True)[5]
        nifti['num'] = nifti['fname'].str.split(f'sub-(\d+)_ses-(\d+)_run-(\d+)-([A-Za-z-]+)_task-social_ev-([A-Za-z-]+)-(\d+)', expand=True)[6].astype(int)   
        ...
        mask = meta[['sub', 'ses', 'run', 'ev', 'num']].apply(lambda x: all(np.in1d(x, nifti[['sub', 'ses', 'run','ev' ,'num']])), axis=1)
        filtered = meta.loc[mask, ]
        save_fname = os.path.join(nifti_dir, sub, f"metadata_{sub}_task-pain-{task}_ev-{ev}.csv")
