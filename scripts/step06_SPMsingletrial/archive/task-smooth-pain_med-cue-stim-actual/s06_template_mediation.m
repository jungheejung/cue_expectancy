...
sublist = [3,4,5,6,7,8,9,10,14,15,16,19,20,21,23,24,25,26,28,29,30,31,32,33,35,37,43,47,51,53,55,58,60];

% 2,7,11,13,17,18,19,20,21,23,25,43,60
sublist=[3,4,5,6,8,9,10,14,15,16,24,26,28,29,30,31,32,33,35,37,47,51,53,55,58];
...
for s = 1:length(sublist)
    ...
    X{1, s} = T.cue_con; %(:,event);% T.cue; %
    M{1, s} = char(fname_nii);
    Y{1, s} = T.actual_rating %(:,strcat(y_rating, '_rating')); %T.actual_rating;
