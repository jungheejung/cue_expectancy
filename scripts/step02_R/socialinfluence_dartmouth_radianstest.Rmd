...
Saved in : /Users/h/Documents/projects_local/social_influence_analysis/scripts/R/socialinfluence_dartmouth_radianstest.Rmd
python jupyter notebook in : /Users/h/Documents/projects_local/social_influence_analysis/scripts/calculateDegrees/c03_plotratings_D.ipynb
open side by side

[ ] revamp plot based on mini thermode experiment
[ ] pain analysis
[ ] cognitive accuracy analysis
[ ] cognitive RT analysis
...
```{r}
# plot function
plot_rainclouds = function(within_average, group_level_condition_average,  GGTITLE, TITLE, XLAB, YLAB,
                           task_name, w, h, COLOR) {

g <- ggplot(data = within_average, aes(y = mean_dev_per_sub, x = Priming, fill = Priming)) +
  geom_flat_violin(aes(fill = Priming), position = position_nudge(x = .1, y = 0), adjust = 1.5,
                   trim = FALSE, alpha = .5, colour = NA) +
  geom_line(data = within_average, aes(group = sub_id, y = mean_dev_per_sub, x = as.numeric(Priming)-.15, fill = Priming), linetype = "solid", color = "grey", alpha = .3) +
  geom_point(aes(x = as.numeric(Priming)-.15, y = mean_dev_per_sub, color = Priming),
             position = position_jitter(width = .05), size = 1, alpha = 0.8, shape = 20) +
  geom_boxplot(aes(x = Priming, y = mean_dev_per_sub, fill = Priming),width = .1,
               outlier.shape = NA, alpha = 0.8, width = .1, colour = "black") +
  geom_errorbar(data = group_level_condition_average, aes(x = as.numeric(Priming)+.1, y = mean_dev_per_sub_norm_mean,
                             colour = Priming, 
                              ymin = mean_dev_per_sub_norm_mean-se, 
                              ymax = mean_dev_per_sub_norm_mean+se), width = .05) +
...
common_path = Sys.glob(file.path(main_dir,'dartmouth', 'beh_raw', 'sub-*', 'ses-*','*_task-social_*-cognitive_beh.csv'))
filter_path = common_path[!str_detect(common_path,pattern="sub-0001")]

cog_dataset <- do.call("rbind",lapply(filter_path,FUN=function(files){ read.csv(files)}))
is.nan.data.frame <- function(x)
  do.call(cbind, lapply(x, is.nan))
cog_dataset[is.nan(cog_dataset)] <- NA

```



## convert to theta -- prob non applicable for dartmouth data because I have the calculated theta in the dataset itself instead of running sqxol
```{r}
# # expectation rating
...
# levels
cog_dataset$levels[cog_dataset$event03_stimulus_type == "low_stim"] <- -0.5 # social influence task
cog_dataset$levels[cog_dataset$event03_stimulus_type == "med_stim"] <- 0 # no influence task
cog_dataset$levels[cog_dataset$event03_stimulus_type == "high_stim"] <- 0.5 # no influence task

cog_dataset$levels_factor = factor(cog_dataset$param_stimulus_type)
#contrast code 1 linear
cog_dataset$level_con_linear[cog_dataset$event03_stimulus_type == "low_stim"] <- -0.5
cog_dataset$level_con_linear[cog_dataset$event03_stimulus_type == "med_stim"] <- 0
cog_dataset$level_con_linear[cog_dataset$event03_stimulus_type == "high_stim"] <- 0.5

# contrast code 2 quadratic
cog_dataset$level_con_quad[cog_dataset$event03_stimulus_type == "low_stim"] <- -0.33
cog_dataset$level_con_quad[cog_dataset$event03_stimulus_type == "med_stim"] <- 0.66
cog_dataset$level_con_quad[cog_dataset$event03_stimulus_type == "high_stim"] <- -0.33

# social cude contrast
cog_dataset$social_cue[cog_dataset$param_cue_type == 'low_cue'] <- -0.5 # social influence task
cog_dataset$social_cue[cog_dataset$param_cue_type == 'high_cue'] <- 0.5 # no influence task
...
```{r}
cog_dataset$subject = factor(cog_dataset$src_subject_id)
cog_expect_countNA = subset(cog_dataset, cog_dataset$event02_expect_RT == "NaN")
                            #| cog_dataset$expect_r < 150)
cog_actual_countNA = subset(cog_dataset, cog_dataset$event04_actual_RT == "NaN" )
                           # | cog_dataset$actual_r < 150)
```
#### plot per participant - Cognitive Expect
```{r}
#participant 
# 2, 11, 15, 16, 25
lost_ce_NA = aggregate(cog_dataset$event02_expect_RT, list(cog_dataset$subject),function(x) sum(is.na(x)))
...
#participant 
# 2, 11, 15, 16, 25
lost_ca_NA = aggregate(cog_dataset$event04_actual_responseonset, list(cog_dataset$subject),function(x) sum(is.na(x)))
...
```{r}
model.cog_expect = lmer(event02_expect_angle ~ param_cue_type + (param_cue_type | src_subject_id), data=cog_dataset)
summary(model.cog_expect)
```

# effect size estimate
```{r}
r2mlm(model.cog_expect)
```
...
# once the R column is added back in, add it into the filter NA criterion
```{r}
cog_dataset$subject = factor(cog_dataset$src_subject_id)
# cog_expect_removeNA = subset(cog_dataset, cog_dataset$event02_expect_RT != "NA")
# # #& cog_dataset$expect_r >= 150 & cog_dataset$subject != c(2,11,15,16,25))
# # 
# cog_actual_removeNA = subset(cog_dataset, cog_dataset$event04_actual_RT != "NA")
# # #& cog_dataset$actual_r >= 150& cog_dataset$subject != c(2,11,15,16,25))

cog_expect_removeNA = cog_dataset[!is.na(cog_dataset["event02_expect_angle"]),]
cog_actual_removeNA = cog_dataset[!is.na(cog_dataset["event04_actual_angle"]),]
# cog_expect_removeNA = cog_dataset[complete.cases(cog_dataset[ , "event02_expect_RT"]),]
#cog_expect_removeNA = subset(cog_dataset, cog_dataset$event02_expect_RT != "NaN" )#& cog_dataset$subject != c(2))
                               #cog_dataset$expect_r >= 150 & 
                               #cog_dataset$subject != c(2))
#cog_expect_removeNA$expect_degree = cog_expect_removeNA$expect_theta*180/pi
# cog_actual_removeNA = cog_dataset[complete.cases(cog_dataset[ , "event04_actual_RT"]),]

#cog_actual_removeNA = subset(cog_dataset, cog_dataset$event04_actual_RT != "NaN")# & cog_dataset$subject != c(2)) #& cog_dataset$actual_r >= 150
...
relabels
```{r}
cog_expect_removeNA$social_name[cog_expect_removeNA$param_cue_type == 'high_cue'] <- "high cue" 
cog_expect_removeNA$social_name[cog_expect_removeNA$param_cue_type == 'low_cue'] <- "low cue" # no influence task

cog_expect_removeNA$levels_name[cog_expect_removeNA$param_stimulus_type == 'high_stim'] <- "high" # no influence task
cog_expect_removeNA$levels_name[cog_expect_removeNA$param_stimulus_type == 'med_stim'] <- "med" # no influence task
cog_expect_removeNA$levels_name[cog_expect_removeNA$param_stimulus_type == 'low_stim'] <- "low" # no influence task
```

```{r}
cog_expect_removeNA$levels_ordered <- factor(cog_expect_removeNA$param_stimulus_type, levels=c("low_stim","med_stim","high_stim"))
...
c_expect_d = ddply(cog_expect_removeNA, .(subject, social_ordered),
      summarize, 
      mean_degree_per_sub = mean(event02_expect_angle), 
      sd = sd(event02_expect_angle))
```
```{r}
...
#                           group = social_name, colour = social_name), linetype = 3) +
  ...
  geom_errorbar(data = c_expect_a, aes(x = as.numeric(social_ordered)+.1, y = mean_degree_per_sub_norm_mean,
...
#ggsave('/Users/h/Documents/projects_local/social_influence_analysis/fig/dartmouth_sub2-10_cognitive_expect.png', width = w, height = h)
```
...
model.cog_actual = lmer(event04_actual_angle ~ param_cue_type*param_stimulus_type +  (param_cue_type*param_stimulus_type| src_subject_id), data= cog_actual_removeNA)
...
```{r}
# PSEUDOCODE: radians ~ cue + administerlevel + cue*administerlevel + (cue*administerlevel | subject)
model.cog_actual = lmer(event04_actual_angle ~ param_cue_type*level_con_linear + param_cue_type *level_con_quad+ (param_cue_type*level_con_linear + param_cue_type *level_con_quad| subject), data= cog_actual_removeNA)

summary(model.cog_actual)
```
```{r}
# PSEUDOCODE: radians ~ cue + administerlevel + cue*administerlevel + (cue*administerlevel | subject)
model.cog_actual_randomeffects = lmer(event04_actual_angle ~ param_cue_type*level_con_linear + param_cue_type *level_con_quad+ (param_cue_type*level_con_linear + param_cue_type *level_con_quad| subject) + (param_cue_type*level_con_linear + param_cue_type *level_con_quad | session_id), data= cog_actual_removeNA)

summary(model.cog_actual_randomeffects)
```

# effect size
```{r}
r2mlm(model.cog_actual)

```

relabels
```{r}
cog_actual_removeNA$social_name[cog_actual_removeNA$param_cue_type == 'high_cue'] <- "high cue" 
cog_actual_removeNA$social_name[cog_actual_removeNA$param_cue_type == 'low_cue'] <- "low cue" # no influence task

cog_actual_removeNA$levels_name[cog_actual_removeNA$param_stimulus_type == 'high_stim'] <- "high" # no influence task
cog_actual_removeNA$levels_name[cog_actual_removeNA$param_stimulus_type == 'med_stim'] <- "med" # no influence task
cog_actual_removeNA$levels_name[cog_actual_removeNA$param_stimulus_type == 'low_stim'] <- "low" # no influence task

```

```{r}
cog_actual_removeNA$levels_ordered = factor(cog_actual_removeNA$levels_name, levels=c("low","med","high"))
...
c_actual_d = ddply(cog_actual_removeNA, .(subject, levels_ordered,social_ordered),
      summarize, 
      mean_degree_per_sub = mean(event04_actual_angle), 
      sd = sd(event04_actual_angle))
...
h = 3
g
ggsave('/Users/h/Documents/projects_local/social_influence_analysis/fig/dartmouth_sub-2-10_cognitive_actual.png', width = w, height = h)
...
ggplot(cog_expect_removeNA, 
        aes(x = event02_expect_RT, y = param_cue_type)) +
...
ggplot(cog_actual_removeNA, 
        aes(x = event04_actual_RT, y = param_stimulus_type)) +
  geom_density_ridges(rel_min_height = 0.01, jittered_points = TRUE,
                      position = position_points_jitter(width = 0.5, height = 0),
                      point_shape = "|", point_size = 2,
                      alpha = 0.7) +
...
ggplot(cog_actual_removeNA, 
        aes(x = event04_actual_RT, y = param_cue_type)) +
  geom_density_ridges(rel_min_height = 0.01, jittered_points = TRUE,
                      position = position_points_jitter(width = 0.5, height = 0),
                      point_shape = "|", point_size = 2,
                      alpha = 0.7) +
...
```{r}
# merge and concatenate files
v_common_path = Sys.glob(file.path(main_dir,'dartmouth', 'beh_raw', 'sub-*', 'ses-*','*_task-social_*-vicarious_beh.csv'))
v_filter_path = v_common_path[!str_detect(v_common_path,pattern="sub-0001")]

vic_dataset <- do.call("rbind",lapply(v_filter_path,FUN=function(files){ read.csv(files)}))
is.nan.data.frame <- function(x)
  do.call(cbind, lapply(x, is.nan))
vic_dataset[is.nan(vic_dataset)] <- NA
```

```{r}

# subject
vic_dataset = as.data.frame(vic_dataset)
vic_dataset$subject = factor(vic_dataset$src_subject_id)

vic_dataset$levels[vic_dataset$param_stimulus_type == 'low_stim'] <- -0.5 # social influence task
vic_dataset$levels[vic_dataset$param_stimulus_type == 'med_stim'] <- 0 # no influence task
vic_dataset$levels[vic_dataset$param_stimulus_type == 'high_stim'] <- 0.5 # no influence task

#contrast code 1 linear
vic_dataset$level_con_linear[vic_dataset$param_stimulus_type == 'low_stim'] <- -0.5
vic_dataset$level_con_linear[vic_dataset$param_stimulus_type == 'med_stim'] <- 0
vic_dataset$level_con_linear[vic_dataset$param_stimulus_type == 'high_stim'] <- 0.5

# contrast code 2 quadratic
vic_dataset$level_con_quad[vic_dataset$param_stimulus_type == 'low_stim'] <- -0.33
vic_dataset$level_con_quad[vic_dataset$param_stimulus_type == 'med_stim'] <- 0.66
vic_dataset$level_con_quad[vic_dataset$param_stimulus_type == 'high_stim'] <- -0.33

# social cude contrast
vic_dataset$social_cue[vic_dataset$param_cue_type == 'low_cue'] <- -0.5 # social influence task
vic_dataset$social_cue[vic_dataset$param_cue_type == 'high_cue'] <- 0.5 # no influence task
...
```{r}
vic_dataset$subject = factor(vic_dataset$src_subject_id)
vic_expect_removeNA = vic_dataset[!is.na(vic_dataset["event02_expect_angle"]),]
vic_actual_removeNA = vic_dataset[!is.na(vic_dataset["event04_actual_angle"]),]
```
...
model.vic_expect = lmer(event02_expect_angle ~ param_cue_type + (param_cue_type | subject), data=  vic_expect_removeNA)
...
relabels
```{r}
vic_expect_removeNA$social_name[vic_expect_removeNA$param_cue_type == "high_cue"] <- "high cue" 
vic_expect_removeNA$social_name[vic_expect_removeNA$param_cue_type == "low_cue"] <- "low cue" # no influence task

vic_expect_removeNA$levels_name[vic_expect_removeNA$param_stimulus_type == "high_stim"] <- "high" # no influence task
vic_expect_removeNA$levels_name[vic_expect_removeNA$param_stimulus_type == "med_stim"] <- "med" # no influence task
vic_expect_removeNA$levels_name[vic_expect_removeNA$param_stimulus_type == "low_stim"] <- "low" # no influence task
```
```{r}
...
g <- ggplot(data = v_expect_d, aes(y = mean_degree_per_sub, x = social_ordered, fill = social_ordered)) +
 ...
 #            position = position_jitter(width = .05), size = 1, alpha = 0.8, shape = 20) +
  ...
  # use summary stats __________________________________________________________________________________
  geom_errorbar(data = v_expect_a, aes(x = as.numeric(social_ordered)+.1, y = mean_degree_per_sub_norm_mean,
                             colour = social_ordered, 
                              ymin = mean_degree_per_sub_norm_mean-se, 
                              ymax = mean_degree_per_sub_norm_mean+se), width = .05) +
...
model.vic_actual = lmer(event04_actual_angle ~ param_cue_type*level_con_linear + param_cue_type *level_con_quad + (param_cue_type*level_con_linear + param_cue_type *level_con_quad | subject), data=  vic_actual_removeNA)
...
model.vic_actual = lmer(event04_actual_angle ~ param_cue_type*level_con_linear + param_cue_type *level_con_quad + (param_cue_type*level_con_linear + param_cue_type *level_con_quad | subject) + (param_cue_type*level_con_linear + param_cue_type *level_con_quad | session_id) , data=  vic_actual_removeNA)
summary(model.vic_actual)
...
relabel levels
```{r}
vic_actual_removeNA$social_name[vic_actual_removeNA$param_cue_type == "high_cue"] <- "high cue" 
vic_actual_removeNA$social_name[vic_actual_removeNA$param_cue_type == "low_cue"] <- "low cue" # no influence task

#total_actual_removeNA$levels_name = factor(total_actual_removeNA$levels, levels=c("low", "med", "high"))

vic_actual_removeNA$levels_name[vic_actual_removeNA$param_stimulus_type == "high_stim"] <- "high" # no influence task
vic_actual_removeNA$levels_name[vic_actual_removeNA$param_stimulus_type == "med_stim"] <- "med" # no influence task
vic_actual_removeNA$levels_name[vic_actual_removeNA$param_stimulus_type == "low_stim"] <- "low" # no influence task

```
```{r}
vic_actual_removeNA$levels_ordered <- factor(vic_actual_removeNA$param_stimulus_type, levels=c("low", "med", "high"))
```
```{r}
...
summary statistics
```{r}
v_d = ddply(vic_actual_removeNA, .(subject, levels_ordered,social_ordered),
      summarize, 
      mean_degree_per_sub = mean(event04_actual_angle), 
      sd = sd(event04_actual_angle))
```
...
g <- ggplot(data = v_d, aes(y = mean_degree_per_sub, x = levels_ordered, fill = social_ordered)) +
  geom_flat_violin(aes(fill = social_ordered), position = position_nudge(x = .1, y = 0), adjust = 1.5,
                   trim = FALSE, alpha = .7, colour = NA) +
  geom_line(data = v_d, aes(group = subject, y = mean_degree_per_sub, x = as.numeric(levels_ordered)-.15, fill = social_ordered), linetype = 3, color = "grey") +
  #geom_point(aes(group = subject, x = as.numeric(levels_ordered)-.15, y = mean_degree_per_sub, color = social_ordered),
 #            position = position_jitter(width = .05), size = 1, alpha = 0.8, shape = 20) +
  geom_point(aes(x = as.numeric(levels_ordered)-.15, y = mean_degree_per_sub, color = social_ordered),
             position = position_jitter(width = .05), size = 1, alpha = 0.8, shape = 20) +
  ...
  geom_errorbar(data = v_a, aes(x = as.numeric(levels_ordered)+.1, y = mean_degree_per_sub_norm_mean,
                              group = social_ordered, colour = social_ordered, 
                              ymin = mean_degree_per_sub_norm_mean-se, 
                              ymax = mean_degree_per_sub_norm_mean+se), width = .05) +
...
```{r}
RT_vic_actual_d = ddply(vic_actual_removeNA, .(subject, levels_ordered,social_ordered),
      summarize, 
      mean_RT_per_sub = mean(event04_actual_RT), 
      sd = sd(event04_actual_RT))
```
...
ggplot(vic_expect_removeNA, 
        aes(x = event02_expect_RT, y = param_stim_type)) +
  geom_density_ridges(rel_min_height = 0.01, jittered_points = TRUE,
                      position = position_points_jitter(width = 0.5, height = 0),
                      point_shape = "|", point_size = 2,
                      alpha = 0.7) +
...
ggplot(vic_expect_removeNA, 
        aes(x = event02_expect_RT, y = social_ordered)) +
  geom_density_ridges(rel_min_height = 0.01, jittered_points = TRUE,
                      position = position_points_jitter(width = 0.5, height = 0),
                      point_shape = "|", point_size = 2,
                      alpha = 0.7) +
...
ggplot(vic_actual_removeNA, 
        aes(x = event04_actual_RT, y = levels_ordered)) +
  geom_density_ridges(rel_min_height = 0.01, jittered_points = TRUE,
                      position = position_points_jitter(width = 0.5, height = 0),
                      point_shape = "|", point_size = 2,
                      alpha = 0.7) +
...
ggplot(vic_actual_removeNA, 
        aes(x = event04_actual_RT, y = social_ordered)) +
  geom_density_ridges(rel_min_height = 0.01, jittered_points = TRUE,
                      position = position_points_jitter(width = 0.5, height = 0),
                      point_shape = "|", point_size = 2,
                      alpha = 0.7) +
...
```{r}
# merge and concatenate files
p_common_path = Sys.glob(file.path(main_dir,'dartmouth', 'beh_raw', 'sub-*', 'ses-*','*_task-social_*-pain_beh.csv'))
p_filter_path = p_common_path[!str_detect(p_common_path,pattern="sub-0001")]

p_dataset <- do.call("rbind",lapply(p_filter_path,FUN=function(files){ read.csv(files)}))
is.nan.data.frame <- function(x)
  do.call(cbind, lapply(x, is.nan))
p_dataset[is.nan(p_dataset)] <- NA
```

```{r}

# subject
p_dataset$subject = factor(p_dataset$src_subject_id)

p_dataset$levels[p_dataset$param_stimulus_type == 'low_stim'] <- -0.5 # social influence task
p_dataset$levels[p_dataset$param_stimulus_type == 'med_stim'] <- 0 # no influence task
p_dataset$levels[p_dataset$param_stimulus_type == 'high_stim'] <- 0.5 # no influence task

#contrast code 1 linear
p_dataset$level_con_linear[p_dataset$param_stimulus_type == 'low_stim'] <- -0.5
p_dataset$level_con_linear[p_dataset$param_stimulus_type == 'med_stim'] <- 0
p_dataset$level_con_linear[p_dataset$param_stimulus_type == 'high_stim'] <- 0.5

# contrast code 2 quadratic
p_dataset$level_con_quad[p_dataset$param_stimulus_type == 'low_stim'] <- -0.33
p_dataset$level_con_quad[p_dataset$param_stimulus_type == 'med_stim'] <- 0.66
p_dataset$level_con_quad[p_dataset$param_stimulus_type == 'high_stim'] <- -0.33

# social cude contrast
p_dataset$social_cue[p_dataset$param_cue_type == 'low_cue'] <- -0.5 # social influence task
p_dataset$social_cue[p_dataset$param_cue_type == 'high_cue'] <- 0.5 # no influence task
...
#participant 
# 2, 11, 15, 16, 25
lost_pe_na = aggregate(p_dataset$event02_expect_RT, list(p_dataset$subject),function(x) sum(is.na(x)))
#lost_ve_r = aggregate(vic_dataset$expect_r, list(vic_dataset$subject),function(x) sum(x < 150))
```
```{r}
#plot(lost_ca_NA$Group.1, lost_ca_NA$x)
ggplot(lost_pe_na, aes(x = Group.1, y = x))+ 
  geom_bar(stat = "identity", position = "identity") +
  xlab("subject ID") + 
  ylab("freq. of NA") + 
  ggtitle("Pain task - Expectation ratings") +
  theme_bw() 
...
#participant 
# 2, 11, 15, 16, 25
lost_pa_NA = aggregate(p_dataset$event04_actual_RT, list(p_dataset$subject),function(x) sum(is.na(x)))
#lost_pa_r = aggregate(p_dataset$actual_r, list(p_dataset$subject),function(x) sum(x < 150))
```

```{r}
#plot(lost_ca_NA$Group.1, lost_ca_NA$x)
ggplot(lost_pa_NA, aes(x = Group.1, y = x))+ 
  geom_bar(stat = "identity", position = "identity") +
  xlab("subject ID") + 
  ylab("freq. of NA") + 
  ggtitle("Pain task - Actual ratings") +
  theme_bw() 

# ggplot(lost_pa_r, aes(x = Group.1, y = x))+ 
#   geom_bar(stat = "identity", position = "identity") +
#   xlab("subject ID") + 
#   ylab("Ratings with r < 150") + 
#   ggtitle("Pain task - Actual ratings") +
...
```{r}
p_dataset$subject = factor(p_dataset$src_subject_id)
p_expect_removeNA = p_dataset[!is.na(p_dataset["event02_expect_angle"]),]
p_actual_removeNA = p_dataset[!is.na(p_dataset["event04_actual_angle"]),]
```


# 2-1. expect rating
```{r}
p_expect_removeNA$subject = factor(p_expect_removeNA$src_subject_id)
...
model.vic_expect = lmer(event02_expect_angle ~ param_cue_type + (param_cue_type | subject), data=  p_expect_removeNA)
summary(model.vic_expect)
...
relabels
```{r}
p_expect_removeNA$social_name[p_expect_removeNA$param_cue_type == "high_cue"] <- "high cue" 
p_expect_removeNA$social_name[p_expect_removeNA$param_cue_type == "low_cue"] <- "low cue" # no influence task

p_expect_removeNA$levels_name[p_expect_removeNA$param_stimulus_type == "high_stim"] <- "high" # no influence task
p_expect_removeNA$levels_name[p_expect_removeNA$param_stimulus_type == "med_stim"] <- "med" # no influence task
p_expect_removeNA$levels_name[p_expect_removeNA$param_stimulus_type == "low_stim"] <- "low" # no influence task
```
```{r}
p_expect_removeNA$levels_ordered <- factor(p_expect_removeNA$levels_name, levels=c("low", "med", "high"))
p_expect_removeNA$social_ordered <- factor(p_expect_removeNA$social_name, levels=c("low cue", "high cue"))
```
summarize cognitive expect data
```{r}
p_expect_d = ddply(p_expect_removeNA, .(subject, social_ordered),
      summarize, 
      mean_degree_per_sub = mean(event02_expect_angle), 
      sd = sd(event02_expect_angle))
```
```{r}
p_expect_a = summarySEwithin(data=p_expect_d, 
                    measurevar = "mean_degree_per_sub", 
                    withinvars = c("social_ordered"), 
                    idvar = "subject")
...
g <- ggplot(data = p_expect_d, aes(y = mean_degree_per_sub, x = social_ordered, fill = social_ordered)) +
  geom_flat_violin(aes(fill = social_ordered), position = position_nudge(x = .1, y = 0), adjust = 1.5,
                   trim = FALSE, alpha = .5, colour = NA) +
  geom_line(data = p_expect_d, aes(group = subject, y = mean_degree_per_sub, x = as.numeric(social_ordered)-.15, fill = social_ordered), linetype = 3, color = "grey") +
  #geom_point(aes(group = subject, x = as.numeric(levels_ordered)-.15, y = mean_degree_per_sub, color = social_name),
 #            position = position_jitter(width = .05), size = 1, alpha = 0.8, shape = 20) +
  geom_point(aes(x = as.numeric(social_ordered)-.15, y = mean_degree_per_sub, color = social_ordered),
             position = position_jitter(width = .05), size = 1, alpha = 0.8, shape = 20) +
  ...
  # use summary stats __________________________________________________________________________________
  geom_errorbar(data = p_expect_a, aes(x = as.numeric(social_ordered)+.1, y = mean_degree_per_sub_norm_mean,
                             colour = social_ordered, 
                              ymin = mean_degree_per_sub_norm_mean-se, 
                              ymax = mean_degree_per_sub_norm_mean+se), width = .05) +
  ...
  #scale_fill_manual(values = c("#D73027", "#4575B4"))+
  #scale_color_manual(values = c("#D73027", "#4575B4"))+
  ggtitle("Pain task - expected ratings") +
  # coord_flip() + #vertical vs horizontal
  xlab("Stimulus intensity levels") +
  ylab("Degrees ") +
  theme_bw() 
...
ggsave('/Users/h/Documents/projects_local/social_influence_analysis/fig/dartmouth_pain_sub-2-10_expect.png', width = w, height = h)
...
model.p_actual_m0 = lmer(event04_actual_angle ~ param_cue_type + (param_cue_type | subject) + (param_cue_type | session_id), data=  p_actual_removeNA)
summary(model.p_actual_m0)
...
model.p_actual = lmer(event04_actual_angle ~ param_cue_type*level_con_linear + param_cue_type *level_con_quad + (param_cue_type*level_con_linear + param_cue_type *level_con_quad | subject), data=  p_actual_removeNA)
summary(model.p_actual)

```
```{r}
anova(model.p_actual_m0,model.p_actual, refit=FALSE)
```


relabel levels
```{r}
p_actual_removeNA$social_name[p_actual_removeNA$param_cue_type == "high_cue"] <- "high cue" 
p_actual_removeNA$social_name[p_actual_removeNA$param_cue_type == "low_cue"] <- "low cue" # no influence task

#total_actual_removeNA$levels_name = factor(total_actual_removeNA$levels, levels=c("low", "med", "high"))

p_actual_removeNA$levels_name[p_actual_removeNA$param_stimulus_type == "high_stim"] <- "high" # no influence task
p_actual_removeNA$levels_name[p_actual_removeNA$param_stimulus_type == "med_stim"] <- "med" # no influence task
p_actual_removeNA$levels_name[p_actual_removeNA$param_stimulus_type == "low_stim"] <- "low" # no influence task

```
```{r}
p_actual_removeNA$levels_ordered <- factor(p_actual_removeNA$param_stimulus_type, levels=c("low", "med", "high"))
```
```{r}
p_actual_removeNA$levels_ordered <- factor(p_actual_removeNA$levels_name, levels=c("low", "med", "high"))
p_actual_removeNA$social_ordered <- factor(p_actual_removeNA$social_name, levels=c("low cue", "high cue"))
```

summary statistics
```{r}
p_d = ddply(p_actual_removeNA, .(subject, levels_ordered,social_ordered),
      summarize, 
      mean_degree_per_sub = mean(event04_actual_angle), 
      sd = sd(event04_actual_angle))
```
...
g <- ggplot(data = p_d, aes(y = mean_degree_per_sub, x = levels_ordered, fill = social_ordered)) +
  geom_flat_violin(aes(fill = social_ordered), position = position_nudge(x = .1, y = 0), adjust = 1.5,
                   trim = FALSE, alpha = .7, colour = NA) +
  geom_line(data = p_d, aes(group = subject, y = mean_degree_per_sub, x = as.numeric(levels_ordered)-.15, fill = social_ordered), linetype = 3, color = "grey") +
  #geom_point(aes(group = subject, x = as.numeric(levels_ordered)-.15, y = mean_degree_per_sub, color = social_ordered),
 #            position = position_jitter(width = .05), size = 1, alpha = 0.8, shape = 20) +
  geom_point(aes(x = as.numeric(levels_ordered)-.15, y = mean_degree_per_sub, color = social_ordered),
             position = position_jitter(width = .05), size = 1, alpha = 0.8, shape = 20) +
  ...
  geom_errorbar(data = p_a, aes(x = as.numeric(levels_ordered)+.1, y = mean_degree_per_sub_norm_mean,
                              group = social_ordered, colour = social_ordered, 
                              ymin = mean_degree_per_sub_norm_mean-se, 
                              ymax = mean_degree_per_sub_norm_mean+se), width = .05) +
...
ggsave('/Users/h/Documents/projects_local/social_influence_analysis/fig/dartmouth_pain_actual.png', width = w, height = h)
