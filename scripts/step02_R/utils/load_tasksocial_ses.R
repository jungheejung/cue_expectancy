load_tasksocial_ses <- function(datadir, taskname, subject_varkey, ses, iv, dv, exclude) {
  # INPUT:
  ...
  common_path <- Sys.glob(file.path(datadir, "sub-*", paste("ses-",sprintf("%02d", ses)), filename
  ))
