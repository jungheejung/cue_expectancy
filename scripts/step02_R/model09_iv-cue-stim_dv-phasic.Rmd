...
phasic = read.csv("/Volumes/spacetop_projects_social/data/physio/physio02_preproc/group_epochstart-0_epochend-5_physio-scr.csv")
```
...
phasic$log_scr_peak <- log10(phasic$SCR_Peak_Amplitude +1)
dv <- "log_scr_peak" # DON"T USE EDA_Peak_Amplitude (misleading)
iv_keyword <- "cue-stim"
dv_keyword <- "scr_peak"
xlab <- ""
taskname <- "full"
...
fullmodel = lmer(log_scr_peak ~ 1+ cue_factor + stim_con_linear + stim_con_quad + cue_factor:stim_con_linear + cue_factor:stim_con_quad + (1  |subject), data = phasic)
...
subject = factor(phasic$src_subject_id)
...
cognitive = lmer(log_scr_peak ~ 1+ cue_factor + stim_con_linear + stim_con_quad + cue_factor:stim_con_linear + cue_factor:stim_con_quad + (1  |subject), data = phasic[phasic$param_task_name == 'cognitive',])
...
# VICARIOUS
```{r lmer fullmodel, class.source = 'fold-show'}
vicarious = lmer(log_scr_peak ~ 1+ cue_factor + stim_con_linear + stim_con_quad + cue_factor:stim_con_linear + cue_factor:stim_con_quad + (1  |subject), data = phasic[phasic$param_task_name == 'vicarious',])
...
# PAIN
```{r lmer fullmodel cognitive, class.source = 'fold-show'}
pain = lmer(log_scr_peak ~ 1+ cue_factor + stim_con_linear + stim_con_quad + cue_factor:stim_con_linear + cue_factor:stim_con_quad + (1  |subject), data = phasic[phasic$param_task_name == 'pain',])
...
for (taskname in c( "vicarious", "cognitive")) { #"pain"
    ggtitle <- paste(taskname, " - SCR phasic signal")
