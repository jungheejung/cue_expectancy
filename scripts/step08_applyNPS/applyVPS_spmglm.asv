...
%         
        nps_values = apply_mask(dat, vpsw, 'pattern_expression', 'ignore_missing');
        nps_corr_values = apply_mask(dat, vpsw, 'pattern_expression', 'correlation', 'ignore_missing');
        nps_cosine_values = apply_mask(dat, vpsw, 'pattern_expression', 'cosine_similarity', 'ignore_missing');
...
%         dat.metadata_table = [dat.metadata_table meta_nifti(:,end)];
    ...
    table_fname = fullfile(vps_dir, 'model-02_CcEScA', strcat('model-02_CcEScA_', con, '.csv'));
