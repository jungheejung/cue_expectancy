...
# Create the ggplot
g <- ggplot(df_filter.NA, aes(x = sub, y = SIIPS, fill = sub)) +
...
g <- plot_lineplot_twofactor_subset(subsetSIIPS, taskname = "pain",
                        iv1 = "stim_ordered", iv2 = "cue_ordered",
                        mean = "mean_per_sub_norm_mean", error = "se",
                        color = c("low cue" = "#5D5C5C", "#D73027"), ggtitle = "Within pain task: SIIPS dotproducts as a function of stimulus intensity level and cue",
...
library(parallel)
ncores <- detectCores()
diff_optims <- allFit(model.SIIPSses, maxfun = 1e5, parallel = 'multicore', ncpus = ncores)
# diff_optims <- allFit(model.full, maxfun = 1e5)
diff_optims_OK <- diff_optims[sapply(diff_optims, is, "merMod")]
lapply(diff_optims_OK, function(x) x@optinfo$conv$lme4$messages)
```
