...
model.factorize = lmer(event04_actual_angle ~ cue_con*stim_con_linear*event02_expect_angle + cue_con*stim_con_quad*event02_expect_angle + lag.04outcomeangle + (cue_con|src_subject_id), data = pvc)
...
total <-
  plot_twovariable(
    df_dropna, iv1, iv2, group, subject, min, max, xlab, ylab,
...
low <-
  plot_twovariable(
    df_low, iv1, iv2, group, subject, min, max, xlab, ylab,
...
med <-
  plot_twovariable(
    df_med, iv1, iv2, group, subject, min, max, xlab, ylab,
...
high <-
  plot_twovariable(
    df_high, iv1, iv2, group, subject, min, max, xlab, ylab,
...
```{r echo=FALSE, message=TRUE, warning=TRUE}
model.factorize = lmer(event04_actual_angle~ cue_con*stim_con_linear*event02_expect_angle + cue_con*stim_con_quad*event02_expect_angle +lag.04outcomeangle+ (1|src_subject_id), data = pvc)
# summary(model.factorize)
tab_model(model.factorize, p.val = "kr", show.df = TRUE)
```
