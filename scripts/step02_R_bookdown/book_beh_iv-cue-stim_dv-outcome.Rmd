...
for (taskname in c("pain", "vicarious", "cognitive")) {
  # 1. filter based on SPM intersecting subject ids _____________________________________
  subid_df <-
    read.csv(file.path(
      main_dir,
      paste0(
        "/scripts/step04_SPM/6cond_highlowcue_rampupplateau/SPM_6condrampup_",
        taskname,
  ...
  # 2. identify how many would be removed if we grab the intersection # _____________________________________
  ...
  # 3. identify how many trials are present in each condition # _____________________________________
...
# 4. calculate difference scores and summarize _____________________________
...
# 6. plot results
# 6-1. parameters __________________________________________________________
...
# 6-2. plot rain cloud plots _______________________________________________
...
# 6-3.  plot geom range ____________________________________________________
p2.se <- "sd" # se, sd, ci
...
combined_se_calc_cooksd <- data.frame()
combined_subjectwise <- data.frame()
...
combined_subjectwise <- rbind(combined_subjectwise, outcome_subjectwise)
...
g <-
  plot_cueexpectancy_twofactor(
    ...
    legend_factor_colors = c("#C5263A", "#0b5fa6"),
...
plot_lineplot_twofactor_subset_SCATTER <- function(subjectwise, groupwise, taskname, iv1, iv2, 
                                              sub_mean, group_mean, error, subject_col, 
                                              color, ggtitle, xlab = NULL, ylab = NULL, 
                                              shift_width = 0.1) {
  
  # Ensure the inputs are data frames
  subjectwise <- as.data.frame(subjectwise)
  groupwise <- as.data.frame(groupwise)
  
  # Check if 'taskname' column exists in 'groupwise' and 'subjectwise'
  if ("task" %in% names(groupwise) && !is.null(taskname)) {
    group_subset <- groupwise %>% filter(task == taskname)
    subject_subset <- subjectwise %>% filter(task == taskname)
  } else {
    group_subset <- groupwise
    subject_subset <- subjectwise
  }
  ...
  # Define shifting factor: shift both 'red' and 'blue' conditions
  subject_subset <- subject_subset %>%
    mutate(x_shift = as.numeric(as.factor(.data[[iv1]])) + 
                     ifelse(.data[[iv2]] == "red", -shift_width, 
                     ifelse(.data[[iv2]] == "blue", shift_width, 0)))
  ...
  # Base plot with group means
  g <- ggplot(data = group_subset, aes(
    x = x_shift, 
    y = .data[[group_mean]],
    group = as.factor(.data[[iv2]]),
    color = as.factor(.data[[iv2]])
    ...
    # Subject-wise means as tiny scatter points (shifted for both 'red' and 'blue')
    geom_point(data = subject_subset, aes(
      x = x_shift,
      y = .data[[sub_mean]],
      group = as.factor(.data[[iv2]]),
      color = as.factor(.data[[iv2]])
    ), size = 1, alpha = 0.3, position = position_jitter(width = 0.02, height = 0)) +
    
    # Error bars for group means
    geom_pointrange(aes(
      ymin = (.data[[group_mean]] - .data[[error]]),
      ymax = (.data[[group_mean]] + .data[[error]])
    ), size = 0.4, width = 0.1) +
    
    # Line plot for group means
    geom_line(linewidth = 1, aes(linetype = as.factor(.data[[iv2]]))) + 
    
    ggtitle(ggtitle) +
    xlab(xlab) +
    ylab(ylab) +
    scale_x_continuous(breaks = unique(as.numeric(as.factor(group_subset[[iv1]]))), 
                       labels = unique(group_subset[[iv1]])) +
    scale_color_manual(values = color) +
    scale_linetype_manual(values = c("solid", "solid")) +
...
plot_lineplot_twofactor_subset_LINE <- function(subjectwise, groupwise, taskname, iv1, iv2, 
                                              sub_mean, group_mean, error, subject_col, 
                                              color, ggtitle, xlab = NULL, ylab = NULL, 
                                              shift_width = 0.1) {
  ...
  required_subject_cols <- c("stim_ordered", "cue_ordered", "mean_per_sub", "sub")
  required_group_cols <- c("stim_ordered", "cue_ordered", "mean_per_sub_norm_mean", "se")
  
  if (!all(required_subject_cols %in% names(subject_subset))) {
    stop("subjectwise data is missing required columns: ", paste(setdiff(required_subject_cols, names(subject_subset)), collapse=", "))
  }
  if (!all(required_group_cols %in% names(group_subset))) {
    stop("groupwise data is missing required columns: ", paste(setdiff(required_group_cols, names(group_subset)), collapse=", "))
  }
  ...
  # Ensure iv1 is treated as a factor with correct ordering (Low -> Med -> High)
  level_order <- c("Low", "Med", "High")
  subject_subset <- subject_subset %>%
    mutate(iv1_factor = factor(iv1, levels = level_order, ordered = TRUE))
  
  group_subset <- group_subset %>%
    mutate(iv1_factor = factor(iv1, levels = level_order, ordered = TRUE))
  
  # Define shifting factor: shift both 'red' and 'blue' conditions consistently
  subject_subset <- subject_subset %>%
    mutate(x_shift = as.numeric(iv1_factor) + 
                     ifelse(iv2 == "high cue", -shift_width, 
                     ifelse(iv2 == "low cue", shift_width, 0)))
  ...
  # Ensure individual subject lines are properly grouped and connected
  subject_subset <- subject_subset %>%
    group_by(subject_col, iv2) %>%
    arrange(iv1_factor)
  ...
  g <- ggplot(data = group_subset, aes(
    ...
    # Subject-wise lines connecting all iv1 levels properly
    geom_line(data = subject_subset, aes(
      x = x_shift,
      y = sub_mean,
      group = interaction(subject_col, iv2),
      color = as.factor(iv2)),
      alpha = 0.4, size = 0.5) +
    
    # Error bars for group means
    geom_pointrange(aes(
      ymin = (group_mean - error),
      ymax = (group_mean + error)
    ), size = 0.4, width = 0.1) +
    
    # Line plot for group means
    geom_line(data = group_subset, aes(
      x = x_shift, 
      y = group_mean,
      group = as.factor(iv2),
      linetype = as.factor(iv2)), 
      linewidth = 1) +
    
    ggtitle(ggtitle) +
    xlab(xlab) +
    ylab(ylab) +
    scale_x_continuous(breaks = unique(as.numeric(group_subset$iv1_factor)), 
                       labels = level_order) +
    scale_color_manual(values = color) +
    scale_linetype_manual(values = c("solid", "solid")) +
...
# Example Usage:
# subjectwise_data <- read.csv("subjectwise_data.csv")
# groupwise_data <- read.csv("groupwise_data.csv")
# plot_lineplot_twofactor_subset_LINE(subjectwise_data, groupwise_data, taskname = "pain", iv1 = "stim_ordered", iv2 = "cue_ordered", 
#                                     sub_mean = "mean_per_sub", group_mean = "mean_per_sub_norm_mean", error = "se", 
#                                     subject_col = "sub", color = c("red", "blue"), ggtitle = "Pain", xlab = "Stimulus intensity", ylab = "Outcome Rating")


```

```{r}
plot_lineplot_twofactor_subset_LINE_DEP <- function(subjectwise, groupwise, taskname, iv1, iv2, 
                                              sub_mean, group_mean, error, subject_col, 
                                              color, ggtitle, xlab = NULL, ylab = NULL, 
                                              shift_width = 0.1) {
  ...
  # Ensure iv1 is treated as a factor with correct ordering
  subject_subset <- subject_subset %>%
    mutate(iv1_factor = factor(.data[[iv1]], levels = unique(.data[[iv1]])))
  ...
  # Ensure individual subject lines are properly grouped and connected
  subject_subset <- subject_subset %>%
    group_by(.data[[subject_col]], .data[[iv2]]) %>%
    arrange(iv1_factor)
  ...
  g <- ggplot(data = group_subset, aes(
    ...
    geom_line(data = subject_subset, aes(
      x = x_shift,
      y = .data[[sub_mean]],
      group = interaction(.data[[subject_col]], .data[[iv2]]),
      color = as.factor(.data[[iv2]])),
      alpha = 0.4, size = 0.5) +
    ...
    ylab(ylab) +
    scale_x_continuous(breaks = unique(as.numeric(group_subset$iv1_factor)), 
                       labels = levels(group_subset$iv1_factor)) +
    scale_color_manual(values = color) +
    scale_linetype_manual(values = c("solid", "solid")) +
...
SUBJECT = as.data.frame(combined_subjectwise)
SUB_MEAN= "mean_per_sub"
GROUP_MEAN= "mean_per_sub_norm_mean"
ERROR="se"
SUBJECT_COL="sub"
...
LINEIV2 = "cue_ordered"
p1 = plot_lineplot_twofactor_subset_LINE(SUBJECT, DATA, 
                                         taskname = 'pain',
                                         iv1 = LINEIV1, iv2 = LINEIV2, 
                                         sub_mean = SUB_MEAN, group_mean = GROUP_MEAN, 
                                         error = ERROR, subject_col = SUBJECT_COL,
                                         color = color, ggtitle = tools::toTitleCase('pain'), 
                                         xlab = "Stimulus intensity", ylab = "Outcome Rating",
                                         shift_width = 0.1)
# p1 = plot_lineplot_twofactor_subset( DATA, 'pain',
#              LINEIV1, LINEIV2, GROUP_MEAN, ERROR, color, ggtitle = tools::toTitleCase('pain'), xlab = "Stimulus intensity", ylab = "Outcome Rating")
p1
```
...
ERROR = "ci"
dv_keyword = "outcome"
p1 = plot_lineplot_twofactor_subset_TEST(DATA, 'pain',
             LINEIV1, LINEIV2, MEAN, ERROR, color, ggtitle = tools::toTitleCase('pain'), xlab = "Stimulus intensity", ylab = "Outcome Rating")
# , ylim=c(10,90))
p2 = cueR::plot_lineplot_twofactor_subset(DATA,'vicarious',
             LINEIV1, LINEIV2, MEAN, ERROR, color,ggtitle = tools::toTitleCase("vicarious")) #, ylim=c(10,90))
p3 = cueR::plot_lineplot_twofactor_subset(DATA, 'cognitive',
...
DATA = as.data.frame(combined_se_calc_cooksd)
  ...
  dv_keyword <- "outcomerating"
  line_thickness <- 0.2
  p1 <- cueR::plot_lineplot_twofactor_subset(DATA, "pain",
...
for (taskname in c("pain", "vicarious", "cognitive")) {
  ...
  dv_keyword <- "outcomerating"
  line_thickness <- 0.2
  ylim_current <- ylim_values[[taskname]]
  p1 <- cueR::plot_lineplot_twofactor_subset(DATA[DATA$ses == "ses-01" , ], taskname,
                               LINEIV1, LINEIV2, MEAN, ERROR, color,
                               ggtitle = 'Visit 1', ylab = "Outcome rating")#,
                               #ylim=ylim_current)#c(40, 90))
  p1 <- p1 + theme(
  aspect.ratio = 1, # Adjust this as needed
  axis.line = element_line(size = line_thickness)
)
  p2 = cueR::plot_lineplot_twofactor_subset(DATA[DATA$ses == "ses-03", ], taskname,
                               LINEIV1, LINEIV2, MEAN, ERROR, color,
  ...
  p3 = cueR::plot_lineplot_twofactor_subset(DATA[DATA$ses == "ses-04", ], taskname,
                               LINEIV1, LINEIV2, MEAN,ERROR, color,
