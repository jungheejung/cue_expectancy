...
subjects_with_inadequate_data <- df.na %>%
...
print(sprintf("after filtering out subjects that have less than 3 trials in cell, we have N=%d -> N=%d",length(unique(df$sub)), length(unique(df_filter$sub)) ))
...
for (taskname in c("pain", "vicarious", "cognitive")) {
# 4. summary stats __________________________________________________________
...
```{r}
model.full <- lmer(outcomerating ~ cue * stimulusintensity + (cue * stimulusintensity|sub), data = beh)
summary(model.full)
...
for (taskname in c("pain", "vicarious", "cognitive")) {
  subdf <- beh[beh$runtype == taskname, ]
...
for (taskname in c("pain", "vicarious", "cognitive")){
beh.na <- beh %>%
  filter(!is.na(outcomerating) & runtype == taskname)
...
print(paste("----------", taskname))
print(summary(fullmodel))
sjPlot::tab_model(fullmodel,
                title = paste0("Multilevel-modeling: \nlmer(OUTCOME ~ CUE * STIM + (CUE * STIM | sub))", taskname),
                CSS = list(css.table = '+font-size: 12;'))
}
# archive ______________________________________________________________________
# 
# library(Matrix)
# library(glmmTMB)
# library(TMB)
