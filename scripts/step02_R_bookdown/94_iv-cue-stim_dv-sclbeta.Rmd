...
beta <- read.table(file = file.path(
  '/Volumes/spacetop_projects_cue/analysis/physio/nobaseline/glm/glm-factorial_task-pain_scr.tsv'
  # main_dir, paste0("analysis/physio/nobaseline/glm/factorial/glm-factorial_task-", taskname, "_scr.tsv")
  ), sep = '\t', header = TRUE)
```
...
beta_subset <- beta[,c("X","filename","sub","ses","run","runtype","intercept","high_stim.high_cue" ,"high_stim.low_cue", "med_stim.high_cue","med_stim.low_cue","low_stim.high_cue","low_stim.low_cue")]
beta_long <- beta %>%
...
model.factorial <- lmer(beta ~ STIM_linear*cue_factor + STIM_quadratic*cue_factor + (STIM_linear + cue_factor|sub), data = beta_con)

# model.factorial <- glmmTMB::glmmTMB(beta ~ STIM_linear*cue_factor + STIM_quadratic*cue_factor + (STIM_linear +cue_factor|sub), data = beta_con, family="gaussian")

summary(model.factorial)
sjPlot::tab_model(model.factorial,
              title = paste0(tools::toTitleCase(taskname), ": \nlmer(SCR beta ~ Stim * Cue + (Stim + Cue | sub), data = beta)"),
...
taskname <- "pain"
# model_iv2 <- "cue_ordered"
analysis_dir <- file.path(main_dir, "analysis", "mixedeffect", "sclbeta", as.character(Sys.Date()))
...
k <- plot_lineplot_twofactor_subsetthick(SCLstim_groupwise,
                        ...
                        color = c( "high" = "#fd0000",
                                  "low" = "#365af1"),
                        ggtitle = title,
                        xlab = "Stimulus Intensity", ylab = "SCR beta coefficients (A.U.)")
...
model.betases <- lmer(beta ~
                         CUE_high_gt_low*STIM_linear*SES_linear +
                          ...
                          (CUE_high_gt_low + STIM_linear + SES_linear|sub), data = beta_long,
                      # optimizer="nloptwrap.NLOPT_LN_NELDERMEAD"
...
library(glmmTMB)
beta_long$beta_scaled <- scale(beta_long$beta)

# model.betases <- glmmTMB::glmmTMB(formula = beta ~
#                           CUE_high_gt_low*STIM_linear*SES_linear +
#                           # CUE_high_gt_low*STIM_quadratic*SES_linear +
#                           CUE_high_gt_low*STIM_linear*SES_quadratic +
#                           # CUE_high_gt_low*STIM_quadratic*SES_quadratic +
#                           (1  |sub), family = "gaussian",
#                          data = beta_long,
#                          ziformula=~1,
...
sjPlot::tab_model(model.betases,
                  title = "Multilevel-modeling: \nlmer(beta ~ CUE * STIM * SES + (Cue + SES + STIM| sub), data = p)",
                  CSS = list(css.table = '+font-size: 12;'))
```

```{r}
diff_optims <- allFit(model.betases, maxfun = 1e5)
