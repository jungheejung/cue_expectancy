...
# TODO: FIX scripts/step02_R/utils/df_filter_ses_trial.R
# df_summary <- df_filter_ses_trial(data, session_colname = "session_id", subject_colname = "src_subject_id", session_threshold = 3, trial_threshold = 50)
data <- df %>%
semi_join(subset_N, by = "src_subject_id")

# ------------------------------------------------------------------------------
#                 data formatting: center ratings, count trials
# ------------------------------------------------------------------------------
maindata <- data %>%
group_by(src_subject_id) %>%
...
data_a3lag_omit <-
  data_a3lag[complete.cases(data_a3lag$lag.04outcomeangle), ]
df <- data_a3lag_omit
# data_a3lag
...
sp <- plot_twovariable(
  ...
  xlab = "Expectation rating\n(subjectwise-centered)", ylab = "Outcome rating\n(subjectwise-centered)",
  ggtitle="", color_scheme = c("high_cue" ="#941100","low_cue" =  "#5D5C5C"),
...
## behavioral only expectation deman
...
# ------------------------------------------------------------------------------
#                organize factors and list plot paramters
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
#                summary statistics for cue and stim factor
# ------------------------------------------------------------------------------
...
## 1) behavioral :: line plots cue _ stim _ expect (demean outcome)

Here, I plot the demeaned-outcome rating as a function of cue, stimulus intensity, and expectation interaction.

```{r, fig.height=10}
# Function to create individual plots
create_individual_plot <- function(data, iv1, iv2, intensity,
                                   xmin = -50, xmax = 50, ymin = -50, ymax = 50,
                                   xlab, ylab, ggtitle, color_scheme, size = 2, subtitle_size, text_size) {
  plot <- plot_twovariable(
    data, iv1 = iv1, iv2 = iv2,
    group = "cuetype", subject = "sub",
    xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax,
    xlab = xlab, ylab = ylab,
    ggtitle = ggtitle,
    color_scheme = color_scheme,
    alpha = 0.6, fit_lm = TRUE, lm_method = "lm", identity_line = FALSE, size = size,
    subtitle_size=subtitle_size, text_size=text_size
  )
...
create_legend <- function() {
  ...
  legend_plot <- ggplot(legend_data, aes(x = sixcond, y = 1, color = sixcond)) +
    geom_point() +
    scale_color_manual(values = legend_data$color) +
    theme_void() +
    theme(legend.position = "bottom") +
    guides(color = guide_legend(title = "Condition"))
  return(ggplotGrob(legend_plot)$grobs[[which(sapply(ggplotGrob(legend_plot)$grobs, function(x) x$name) == "guide-box")]])
}
```
...
iv1 <- "demean_EXPECT"
iv2 <- "demean_OUTCOME"
XAXIS_TEXT <- "Expectation rating\n(subjectwise-centered)"
YAXIS_TEXT <- "Outcome rating\n(subjectwise-centered)"
TEXT_FONTSIZE <- 12
LABEL_FONTSIZE <- 15
TITLE_FONTSIZE <- 20
...
pvc.df <- maindata.df %>%
  mutate(
    cuetype = case_when(cue_ordered == "low" ~ "low",
...
df_dropna <-
  pvc.df[!is.na(pvc.df$demean_EXPECT) &
           !is.na(pvc.df$demean_OUTCOME),]

# Create individual plots
df_intensity <- list(low = df_dropna[df_dropna$stimintensity == "low",],
                     med = df_dropna[df_dropna$stimintensity == "med",],
                     high = df_dropna[df_dropna$stimintensity == "high",])

intensities <- c("low", "med", "high")
color_mapping <- list(
  "high" = c("high" = "#941100", "low" = "#5D5C5C"),
  "med" = c("high" = "#DB6000", "low" = "#5D5C5C"),
  "low" = c("high" = "#FF8800", "low" = "#5D5C5C")
)

# ----------------------------------------------------------------------------
#                  plot
# ----------------------------------------------------------------------------
plots <- lapply(intensities, function(intensity) {
  create_individual_plot(
    df_intensity[[intensity]],
    iv1 = "demean_EXPECT",
    iv2 = "demean_OUTCOME",
    xlab = ifelse(intensity == "low", " ", "Expectation rating"),
    ylab = ifelse(intensity == "high", " ", "Outcome rating"),
    ggtitle = paste(tools::toTitleCase(intensity), "intensity"),
    color_scheme = color_mapping[[intensity]],
    subtitle_size = TITLE_FONTSIZE,
    text_size = TEXT_FONTSIZE
  )
})
combined_plots <- do.call(grid.arrange, c(plots, ncol=3))

# ----------------------------------------------------------------------------
#                  create common axis label
# ----------------------------------------------------------------------------
y_axis_label <-
  textGrob(YAXIS_TEXT,
           rot = 90,
           gp = gpar(fontsize = LABEL_FONTSIZE),
           vp = viewport(y = unit(.56, "npc") + unit(0.5, "lines")))
x_axis_label <-
  textGrob(XAXIS_TEXT,
           rot = 0,
           gp = gpar(fontsize = LABEL_FONTSIZE))
num_rows <- length(combined_plots) + 1  # +1 for the legend

legend_grob <- create_legend()
heights <- c(rep(1, length(run_types)), 2)
# ----------------------------------------------------------------------------
#                  combined plots across 3 intensities
# ----------------------------------------------------------------------------
#x_combined_plot <- grid.arrange(combined_plots, x_axis_label, nrow = 2, heights = c(1, .1))
#roi_combined_plot <- grid.arrange(y_axis_label, combined_plots, ncol = 2, widths = c(1, 10))
final_plot <- grid.arrange(y_axis_label,
    ...
    heights = c(4, 1, 1),
    top = textGrob(
    sprintf("Outcome rating as a function of cue x stim x expectation"),
    gp = gpar(fontsize = TITLE_FONTSIZE, fontface = "bold"), vjust = 0.2
...
ggsave(file.path(analysis_dir, "beh_iv-cue-stim-dv-demeanoutcome-demeanexpect.png"), plot = final_plot, bg = "transparent", device = "png")
```

## 2) behavioral :: line plots cue _ stim _ intensity ( outcome)
...
# ----------------------------------------------------------------------------
maindata.df <- simple_contrasts_beh(maindata)
maindata.df$OUTCOME <- maindata.df$event04_actual_angle
pvc.df <- maindata.df %>%
  mutate(
    cuetype = case_when(cue_ordered == "low" ~ "low",
                        cue_ordered == "high" ~ "high"),
...
#x_combined_plot <- grid.arrange(combined_plots, x_axis_label, nrow = 2, heights = c(1, .1))
#roi_combined_plot <- grid.arrange(y_axis_label, combined_plots, ncol = 2, widths = c(1, 10))
final_plot2 <- grid.arrange(y_axis_label,
    arrangeGrob(combined_plots,
    x_axis_label,
    legend_grob,
    ncol = 1,
...
ggsave(file.path(analysis_dir, 'figB.png'), plot = fig.bsave)
fig.bsave
```
...
### NPS: stim _ cue _ expect (demean)
...
### Fig C: behavioral cue _ stim _ expect
...
### Fig D: 3 panel cue _ stim _ expect
...
```{r}
write.csv(NPS.df,file.path(analysis_dir, 'dataframe.csv'))
```
...
### 3 way interaction behavioral :: line plots cue _ stim _ intensity (demean outcome)
