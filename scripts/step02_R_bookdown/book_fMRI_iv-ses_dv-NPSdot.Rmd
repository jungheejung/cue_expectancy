# [fMRI] NPSdot \~ session {#NPSdot_ses}

## What is the purpose of this notebook? {.unlisted .unnumbered}

-   Here, I model NPS dot product values as a function of cue, stimulus intensity and expectation ratings.
...
-   11/12/2024: Updated based on NPS dot product values
-   inputs from ./scripts/step10_nilearn/signature/step02_apply_nps.m
...
TODO: make the NPS values available on github, to bypass VPN remote access to the csv files. 
```{r}
analysis_folder <- "fmri_NPSdot"          
beh <- readr::read_tsv(file.path(main_dir, "data/beh/sub-all_task-all_events.tsv"))

################################################################################
# RUN ONCE: concatenate NPS values

# load behavioral and NPS data and grab the intersection

file_list <- list.files(path =file.path('/Volumes/spacetop_projects_cue', "analysis/fmri/nilearn/deriv01_signature/rampup_plateau/NPS"), 
                        pattern = "^sub-\\d+_signature-NPScorr_runtype-pvc_event-stimulus\\.csv$", full.names = TRUE)
#"sub*_signature-NPScorr_runtype-pvc_event-stimulus.csv", full.names = TRUE)
...
remote_dir = '/Volumes/spacetop_projects_cue'

native_tsv = file.path(main_dir, 'analysis', 'fmri', 'nilearn', 'deriv01_signature', 'rampup_plateau', 'sub-all_signature-NPS_runtype-pain_event-stimulus.tsv')
native_json = file.path(main_dir, 'analysis', 'fmri', 'nilearn', 'deriv01_signature', 'rampup_plateau', 'sub-all_signature-NPS_runtype-pain_event-stimulus.json')
curated_tsv_file = file.path(main_dir, 'data', 'curated', 'sub-all_signature-NPS_runtype-pain_event-stimulus.tsv')
curated_json = file.path(main_dir, 'data', 'curated', 'sub-all_signature-NPS_runtype-pain_event-stimulus.json')

remote_tsv = file.path(remote_dir, 'analysis', 'fmri', 'nilearn', 'deriv01_signature', 'rampup_plateau', 'sub-all_signature-NPS_runtype-pain_event-stimulus.tsv')
remote_json = file.path(remote_dir, 'analysis', 'fmri', 'nilearn', 'deriv01_signature', 'rampup_plateau', 'sub-all_signature-NPS_runtype-pain_event-stimulus.json')
remote_curated_tsv_file = file.path(remote_dir, 'data', 'curated', 'sub-all_signature-NPS_runtype-pain_event-stimulus.tsv')
remote_curated_json = file.path(remote_dir, 'data', 'curated', 'sub-all_signature-NPS_runtype-pain_event-stimulus.json')

write.table(df_merge, file = native_tsv, sep = "\t", row.names = FALSE, quote = FALSE)
write.table(df_merge, file = curated_tsv_file, sep = "\t", row.names = FALSE, quote = FALSE)
write.table(df_merge, file = remote_tsv, sep = "\t", row.names = FALSE, quote = FALSE)
write.table(df_merge, file = remote_curated_tsv_file, sep = "\t", row.names = FALSE, quote = FALSE)
...
metadata <- list(
  ...
  Description = "This file includes NPS dotproduct, correlation, and cosine similarity values with pain single trial values.",
  ColumnInfo = lapply(names(df), function(col_name) {
    list(
      ColumnName = col_name,
      DataType = class(df[[col_name]])
    )
  }),
  GeneratedFrom = list(
    Code = list(
...
write_json(metadata, native_json, pretty = TRUE)
write_json(metadata, curated_json, pretty = TRUE)
write_json(metadata, remote_json, pretty = TRUE)
write_json(metadata, remote_curated_json, pretty = TRUE)
cat("Metadata JSON file saved as:", native_json)
###############################################################################
df_merge <- readr::read_tsv(file.path(main_dir, "data/curated/sub-all_signature-NPS_runtype-pain_event-stimulus.tsv"))
...
`data` is a subset of `data_merge`, only filtering out "pain" trials
```{r}
...
ggtitle <- paste(taskname, " - NPS dot (degree)"); title <- paste(taskname, " - actual")
w <- 10; h <- 6
...
sub_mean <- "mean_per_sub"; group_mean <- "mean_per_sub_norm_mean"; dv_keyword <- "NPSdot"
se <- "se"; subject <- "sub"; 
ggtitle <- paste(taskname, " - NPS dot products"); title <- paste(taskname, " - NPS")
xlab <- ""; ylab <- "NPS (dot)";
ylim <- c(-5, 15)
...
g <- plot_lineplot_twofactor_subset(subsetNPS, taskname = "pain",
                        ...
                        xlab = "Stimulus intensity", ylab = "NPS (dot)")
...
> Modeling random slopes of intensity effects, we find a significant effect of stimintensity on NPS dot product values, but not cue effects. The greater the stimulus intensity, the greater the NPS values (b = 0.004, SE = 0.002, t(98.89) =  2.027, p < .05). There is no significant cue effect
...
model.npscuestim <- lmer(nps ~
                          ...
                          (1 + STIM_linear|sub), data = df_filter.NA, 
                         control = lmerControl(optimizer = "nmkbw")
                    )

summary(model.npscuestim)
sjPlot::tab_model(model.npscuestim,
                  title = "Multilevel-modeling: \nlmer(NPSdot ~ CUE * STIM + (CUE + STIM | sub), data = pvc)",
                  CSS = list(css.table = '+font-size: 12;'))


model.singular <- lmer(nps ~
                          CUE_high_gt_low*STIM_linear + CUE_high_gt_low*STIM_quadratic +
...
model.npscuestim <- lmer(nps ~
                          ...
                          ( STIM_quadratic|sub), data = df_filter,
                         control = lmerControl(optimizer = "nloptwrap", optCtrl = list(algorithm = "NLOPT_LN_BOBYQA")))
                                 #    control=lmerControl(optimizer="nmkbw"
                                 # ))
...
df_filter.no1 <- df_filter.NA[df_filter.NA$trial_index != 1, ]
...
NPSstimcue_subjectwise <- meanSummary(df_filter.no1,
                                      c(subject, model_iv1, model_iv2), "nps")
NPSstimcue_groupwise <- summarySEwithin(
  data = NPSstimcue_subjectwise,
  measurevar = "mean_per_sub",
  withinvars = c(model_iv1, model_iv2),
...
model.npscuestim.no1 <- lmer(nps ~
                          CUE_high_gt_low*STIM_linear + CUE_high_gt_low*STIM_quadratic +
                          (STIM_linear + CUE_high_gt_low|sub), data = df_filter.no1, 
                          control = lmerControl(optimizer = "nloptwrap", optCtrl = list(algorithm = "NLOPT_LN_BOBYQA"))
                         #control = lmerControl(optimizer = "nmkbw")
                    )
...
Let's look at the progression of NPS dot products over time. Is it indicative of convergence, and perhaps a learned effect? This will help us link the results back to prediction error.
...
for (p in 1:total_pages) {
  ...
  g <- plot_per_sub(df_clean, "trial_index_sub", dv = "nps", sub = "sub", xlab_str = "No. of trials",
             ylab_str = "NPS (dot)", taskname, ncol_num, nrow_num, p)
  print(g)
}
...
for (p in 1:total_pages) {
  # print(plot_per_sub(df_clean.no1, taskname, ncol_num, nrow_num, p))
  g <- plot_per_sub(df_clean.no1, "trial_index_sub", dv = "nps", sub = "sub", xlab_str = "No. of trials",
             ylab_str = "NPS (dot)", taskname, ncol_num, nrow_num, p)
  print(g)
}
...
sjPlot::tab_model(model.npsses,
                  title = "Multilevel-modeling: \nlmer(NPSdot ~ CUE * STIM * SES + (STIM + SES | sub), data = pvc)",
                  CSS = list(css.table = '+font-size: 12;'))
```

> Using nloptwrap.NLOPT_LN_BOBYQA optimizer, random slopes of stimulus intensity and session, are modeled using orthogonal contrast codes. Cue effects on NPS dot product values are significant for average stim and average session (b = -0.228, SE = 0.112, t(3743) = -2.031, p = 0.042). Stimulus intensity effects are significant; higher temperature leads to greater NPS response (b = 1.174, SE = 0.137, t(3310) = 8.547, p < 0.001), also, quadratic effects of stimulu inensity (b = 0.343, SE = 0.120, t(3743) = 2.862, p = 0.004). Quadratic session effects are also marginally signfiicant, in that the middle session was perceived more painful (b = -0.250, SE = 0.132, t(3307) = -1.899, p = 0.058) The NPS values decrease over time. 
...
# CUE_high_gt_low+STIM+EXPECT_demean
sjPlot::tab_model(model.npsses,
                  title = "Multilevel-modeling: \nlmer(NPSdot ~ CUE * STIM * SES + (CUE + STIM + SES| sub), data = pain)",
                  CSS = list(css.table = '+font-size: 12;'))
...
for (sesname in c("ses-01", "ses-03", "ses-04")) {
taskname = "pain"
    ggtitle <- paste(taskname, " - NPS dot product")
    title <- paste(taskname, " - NPSdot")
    subject <- "sub"
    w <- 10
    h <- 6
