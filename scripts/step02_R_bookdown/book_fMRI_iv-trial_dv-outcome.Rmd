...
plot_per_sub <- function(df, iv, dv, sub, xlab_str, ylab_str, taskname, ncol_num, nrow_num, page) {
  ...
  dv <- sym(dv)  
  sub <- sym(sub) 
  df_task <- df[df$runtype == taskname, ]
...
data_p2_long = df_clean %>%
  pivot_longer(cols = c( 'outcomerating'),
...
## 4. plot Outcome rating as a function of subjects (removing first trial)
```{r}
...
df_filter.no1.pain <- df_filter.no1[df_filter.no1$runtype == taskname,]
## Prepare data
df_clean.no1.pain <- df_filter.no1.pain %>%  group_by(sub) %>%  mutate(trial_index_sub = row_number()) %>%  ungroup()

ncol_num <- 3; nrow_num <- 4 # Set parameters for pagination
total_pages <- ceiling(length(unique(df_clean.no1$sub)) / (ncol_num * nrow_num))

## Loop through pages and render the plot
for (p in 1:total_pages) {
  g <- plot_per_sub(df_clean.no1, "trial_index_sub", dv = "outcomerating", sub = "sub", xlab_str = "No. of trials",
             ylab_str = "Outcome rating", taskname, ncol_num, nrow_num, p)
  print(g)
}
```
...
df_filter.no1 <- data_extracted[data_extracted$trial_index != 1, ]
## Prepare data
df_clean.no1 <- df_filter.no1 %>%  group_by(sub) %>%  mutate(trial_index_sub = row_number()) %>%  ungroup()
taskname <- "pain"
ncol_num <- 3; nrow_num <- 4 # Set parameters for pagination
total_pages <- ceiling(length(unique(df_clean.no1$sub)) / (ncol_num * nrow_num))
...
ncol_num <- 3
nrow_num <- 4
total_pages <- ceiling(length(unique(df_clean.no1.pain$sub)) / (ncol_num * nrow_num))
cue_colors <- c("low_cue" = "blue","high_cue" = "red")

# Loop through pages and render the plot
for (p in 1:total_pages) {
  # print(plot_per_sub(df_clean.no1, taskname, ncol_num, nrow_num, p))
  g <- plot_per_cuesub(df_clean.no1.pain, "trial_index_sub", 
                       dv = "outcomerating", 
                       sub = "sub", 
                       cue = "cue",
                       colors = cue_colors,
