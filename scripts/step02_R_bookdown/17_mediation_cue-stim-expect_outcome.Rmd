...
```
```{r}
with(pvc, mediation.effect.plot(x = cue_con, 
...
library(MBESS)
library(bayestestR)
with(pvc, MBESS::mediation(x = cue_con, 
                    mediator = event02_expect_angle, 
...
library(MBESS)
library(bayestestR)
model.0 <- lm(event04_actual_angle ~ cue_con, pvc)
model.M <- lm(event02_expect_angle ~ cue_con, pvc)
model.Y <- lm(event04_actual_angle ~ cue_con + event02_expect_angle, pvc)
...
model.factorize_demean = lmer(demean_outcome~ cue_con*stim_con_linear*demean_expect+cue_con*stim_con_quad*demean_expect+lag.demean_outcome+ (cue_con*demean_expect|src_subject_id), data = pvc)
