...
```{r echo=FALSE}
# Sort the data by median "outcome" in ascending order _________________________
dv<- "OUTCOME"
subject <- "sub"
sorted_data <- df.centered_NA %>%
  dplyr::group_by(!!rlang::sym(subject) ) %>%
  dplyr::summarize(median_outcome = median( !!rlang::sym(dv) , na.rm = TRUE)) %>%
  dplyr::arrange(median_outcome) #%>%
  # dplyr::select(!!rlang::sym(subject) )
...
# sorted_subjects <- sorted_data$subject

# Reorder the 'subject' factor in df.centered_NA based on the sorted order
df.centered_NA$subject <- factor(df.centered_NA$subject, levels = sorted_subjects)
...
g_plotly
# Reorder the "subject" factor based on the sorted order _______________________
# df.centered_NA$subject <- factor(df.centered_NA$subject, levels = sorted_data$subject)
# 
# 
# # Create the ggplot ____________________________________________________________
# g <- ggplot2::ggplot(df.centered_NA, aes(x = subject, y = OUTCOME, fill = subject)) +
#   geom_boxplot(outlier.shape = NA, width = 1.2, position = position_dodge(1)) +  
#   geom_jitter(width = .1, alpha = 0, size = 1) +
#   labs(x = "Subject", y = "Pain Outcome Rating") +
#   theme_classic() +
#   theme(legend.position = "none") +
...
# 
# g_plotly <- ggplotly(g, tooltip = c("x", "y"))
# g_plotly
