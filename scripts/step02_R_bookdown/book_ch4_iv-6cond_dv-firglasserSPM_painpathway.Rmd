# fMRI :: FIR ~ pathway {#fmritimeseries_painpathway}
...
TODO:
* [ ] Run model PER ROI
* [ ] Run model for cue epoch not just stim epoch
## references
...
analysis_folder <- paste0("book_fir_iv-6cond_dv-firglasserSPM_ttl2")
analysis_dir <-
  file.path(main_dir,
            "analysis",
            "mixedeffect",
...
```{r}
roi_list <- c("painpathway")#c("dACC", "PHG", "V1", "SM", "MT", "RSC", "LOC", "FFC", "PIT", "pSTS", "AIP", "premotor") #, 'rINS')
...
```{r warning=FALSE}
roi_list <- c("painpathway")# c("dACC", "PHG", "V1", "SM", "MT", "RSC", "LOC", "FFC", "PIT", "pSTS", "AIP", "premotor") # 'rINS', 'TPJ',
for (ROI in roi_list) {
  datadir <- file.path(main_dir, "analysis/fmri/spm/fir/ttl2_painpathway")
  # taskname = 'pain'
  exclude <- "sub-0001"
...
all_plots <- list()
TR_length <- 42
roi_list <- c("painpathway") #, "PHG", "V1", "SM", "MT", "RSC", "LOC", "FFC", "PIT", "pSTS", "AIP", "premotor")
#'rINS')
for (roi in roi_list) {
