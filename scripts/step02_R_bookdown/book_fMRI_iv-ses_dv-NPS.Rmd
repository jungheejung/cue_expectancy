...
ggtitle <- paste(taskname, " - NPS (degree)"); title <- paste(taskname, " - actual")
w <- 10; h <- 6
...
sub_mean <- "mean_per_sub"; group_mean <- "mean_per_sub_norm_mean"; dv_keyword <- "NPS"
se <- "se"; subject <- "sub";
ggtitle <- paste(taskname, " - NPS Cooksd removed"); title <- paste(taskname, " - NPS")
xlab <- ""; ylab <- "NPS (degree)";
ylim <- c(-10, 60)
...
model.npscuestim.no1 <- lmer(NPS ~
                          CUE_high_gt_low*STIM_linear + CUE_high_gt_low*STIM_quadratic +
                          (CUE_high_gt_low+STIM_linear|sub), data = df_filter.no1,
                         control = lmerControl(optimizer = "nmkbw")
                    )
...
Let's look at the progression of NPS dot values over time. Is it indicative of convergence, and perhaps a learned effect? This will help us link the results back to prediction error.
...
for (p in 1:total_pages) {
  ...
  g <- plot_per_sub(df_clean, "trial_index_sub", dv = "NPS", sub = "sub", xlab_str = "No. of trials",
             ylab_str = "NPS", taskname, ncol_num, nrow_num, p)
  print(g)
}
...
#                                 lmer
################################################################################
subjects_with_inadequate_data <- df_clean.no1 %>%
  group_by(sub, CUE_high_gt_low, STIM_linear, SES_linear) %>%
  dplyr::summarise(count = n(), .groups = 'drop') %>%
  filter(count < 2) %>%
  distinct(sub) %>%
...
# s1.sig <- df_filter_ses[df_filter_ses$runtype == "pain" & df_filter_ses$ses == sesname,]
    ...
    s1.sig <- df_filter_ses %>%
        filter(runtype == "pain", ses == sesname)
...
dv <- "NPS"
    ...
    str(actual_subjectwise)
    summary(actual_subjectwise)
    actual_groupwise <- summarySEwithin(
        data = actual_subjectwise,
        measurevar = "mean_per_sub",
        withinvars = c(model_iv1, model_iv2), idvar = subject
...
<!--                         mean = "mean_per_sub_norm_mean", error = "se", -->
<!--                         color = c("low cue" = "#0b5fa6","high cue" = "#C5263A"),  -->
<!--                         ggtitle = paste0("NPS N =", length(unique(NPSstimcue_subjectwise$sub)), "taskname: ", taskname), -->
<!--                         xlab = "Stimulus intensity", ylab = "Average NPS score") -->
<!-- g<- g + theme(aspect.ratio=1, -->
<!--           text = element_text(size = 18), # Default text size for the plot -->
<!--           axis.title = element_text(size = 22, ), # Axis titles -->
<!--           axis.text = element_text(size = 15), # Axis text (x and y) -->
...
<!--   geom_point(size = 2)  # Adjust point size -->
<!-- g -->
<!-- ggsave(file.path(analysis_dir, paste0("NPS_maineffect_task-",taskname, "_", as.character(Sys.Date()), ".svg")), plot=g, dpi=300, w=4, h=4)  -->
<!-- } -->
<!-- # g -->
<!-- ``` -->
