---
title: "49_iv-6cond_dv-firglasserSPM_ttl2_rINS"
...
for (ROI in roi_list) {
    ...
    datadir = "/Users/h/Documents/projects_local/cue_expectancy/analysis/fmri/spm/fir/ttl1par"
# taskname = 'pain'
exclude <- "sub-0001"
...
```{r}
  ...
  ggtitle = "sandbox"
  color = c("#5f0f40","#ae2012", "#fcbf49")
    #   if (is.factor(groupwise_sorted$tr_ordered)) {
    #     groupwise_sorted$tr_ordered <- as.numeric(as.character(groupwise_sorted$tr_ordered))
    # }
    g <- ggplot(
      data = groupwise_sorted,
      aes(
        x = .data[[iv1]],
        y = .data[[mean]],
        group = factor(.data[[iv2]]),
      ...
      theme(legend.key = element_rect(fill = "white", colour = "white")) +
      theme_bw() +
      scale_x_continuous(breaks = seq(0, 42, by = 10)) 
