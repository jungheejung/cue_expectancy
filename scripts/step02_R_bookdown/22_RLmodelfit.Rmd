...
analysis_dir <- file.path(main_dir, "analysis", "mixedeffect", "21_modelfit", as.character(Sys.Date()))
...
ERROR = "ci"
dv_keyword = "actual"
p1 = plot_lineplot_twofactor_subset(DATA, task = "pain",
               LINEIV1, LINEIV2, MEAN, ERROR, color, ggtitle = 'pain' , ylab = "Outcome rating\n(subjectwise-demeaned)")
p1 + theme(text = element_text(size = 15)) +theme(aspect.ratio=1) +
  theme(axis.line = element_line(colour = "black"),
      panel.background = element_blank(),
      plot.subtitle = ggtext::element_textbox_simple(size= 11))
...
low <-
  plot_twovariable(
    ...
    xlab = "expectation rating\n(subjectwise mean-centered)", ylab = "outcome ratingn\n(subjectwise mean-centered)",
...
for (signature_key in c("NPSpos")) {
  ...
  main_dir <- dirname(dirname(getwd()))
  analysis_folder  = paste0("CCNfigures-", signature_key)
...
fig.b <- plot_lineplot_twofactor_subset(NPSstimcue_groupwise, taskname = "pain", 
...
ggsave('/Users/h/Dropbox (Dartmouth College)/projects_dropbox/social_influence_analysis/analysis/mixedeffect/CCNfigures/figB.png', plot = fig.bsave)
fig.bsave
```
...
save.sp <- a2 +  
  ...
  theme(
    # axis.line = element_line(colour = "black"),
      ...
      plot.subtitle = ggtext::element_textbox_simple(size= 11))
save.sp
ggsave('/Users/h/Dropbox (Dartmouth College)/projects_dropbox/social_influence_analysis/analysis/mixedeffect/CCNfigures/figA2.png', save.sp)
...
a1.save <- a1 + theme(text = element_text(size = 20)) +theme(aspect.ratio=1) +
  theme(
    # axis.line = element_line(colour = "black"),
      ...
      plot.subtitle = ggtext::element_textbox_simple(size= 11))
a1.save
ggsave('/Users/h/Dropbox (Dartmouth College)/projects_dropbox/social_influence_analysis/analysis/mixedeffect/CCNfigures/figA1.png', p1)
...
plots
ggsave('/Users/h/Dropbox (Dartmouth College)/projects_dropbox/social_influence_analysis/analysis/mixedeffect/CCNfigures/figC.png', plots)
```
...
pvc.df$sub = pvc.df$src_subject_id
df_dropna <- pvc.df[!is.na(pvc.df$demean_expect_model) & !is.na(pvc.df$demean_outcome_model),]
df_low = df_dropna[df_dropna$stimintensity == "low", ]
low <-
  plot_twovariable(
    df_low, iv1 = "demean_expect_model", iv2 = "demean_outcome_model",
    group = "cuetype", subject = "sub",
    xmin=-50, xmax=50, ymin=-50, ymax=50,
    xlab = " ", ylab = "Outcome rating",
    ggtitle = "Low intensity", #DB6000
    color_scheme = c("high" ="#d8476c","low" =  "#5D5C5C"),
    alpha = .6, fit_lm = TRUE, lm_method = "lm", identity_line=FALSE, size=NULL
  )
...
med <-
  plot_twovariable(
    df_med, iv1 = "demean_expect_model", iv2 = "demean_outcome_model",
    group = "cuetype", subject = "sub",
    xmin=-50, xmax=50, ymin=-50, ymax=50,
    xlab = "Expectation rating", ylab = " ",
    ggtitle = "Medium intensity",
    color_scheme = c("high" ="#d83969","low" =  "#5D5C5C"),
    alpha = .6, fit_lm = TRUE, lm_method = "lm", identity_line=FALSE, size=NULL
  )
...
high <-
  plot_twovariable(
    df_high, iv1 = "demean_expect_model", iv2 = "demean_outcome_model",
    group = "cuetype", subject = "sub",
    xmin=-50, xmax=50, ymin=-50, ymax=50,
    xlab = " ", ylab = " ",
    ggtitle = "High intensity",
    color_scheme = c("high" ="#d81e5b","low" =  "#5D5C5C"),
    alpha = .6, fit_lm = TRUE, lm_method = "lm", identity_line=FALSE, size=NULL
  )
...
plots
p1 <- plots 
ggsave(file.path(analysis_dir, "modelfit_iv-cue-stim-dv-painjepma.png"), plot = p1, bg = "transparent", device = "png")
```
...
low <-
  plot_twovariable(
    ...
    xlab = "expectation rating\n(subjectwise mean-centered)", ylab = "Outcome rating",
    ggtitle = "Low intensity", #DB6000
    color_scheme = c("low" ="#C506FF","high" =  "#5D5C5C"),
    alpha = .6, fit_lm = TRUE, lm_method = "lm", identity_line=FALSE, size=NULL
  )
...
med <-
  plot_twovariable(
    ...
    xlab = "expectation rating\n(subjectwise mean-centered)", ylab = "Outcome rating",
    ggtitle = "Medium intensity",
    color_scheme = c("high" ="#5D5C5C","low" =  "#5D5C5C"),
    alpha = .6, fit_lm = TRUE, lm_method = "lm", identity_line=FALSE, size=NULL
  )
...
expect_fname = '/Users/h/Dropbox (Dartmouth College)/projects_dropbox/social_influence_analysis/data/simulated/0411/table_exp.csv'
outcome_fname = '/Users/h/Dropbox (Dartmouth College)/projects_dropbox/social_influence_analysis/data/simulated/0411/table_pain.csv'
...
plots_title <- annotate_figure(plots, top = text_grob("individual differences\n - cue effects from outcome ratings", color = "black", face = "bold", size = 15))
plots
ggsave('/Users/h/Dropbox (Dartmouth College)/projects_dropbox/social_influence_analysis/analysis/mixedeffect/CCNfigures/figD.png', plots)
```
...
maindata <- data %>%
group_by(src_subject_id) %>%
mutate(event02_expect = as.numeric(event02_expect_angle)) %>%
mutate(event04_outcome = as.numeric(event04_actual_angle)) %>%
mutate(avg_outcome_beh = mean(event04_outcome, na.rm = TRUE)) %>%
mutate(demean_outcome_beh = event04_outcome - avg_outcome_beh) %>%
mutate(avg_expect_beh = mean(event02_expect, na.rm = TRUE)) %>%
mutate(demean_expect_beh = event02_expect - avg_expect_beh)
...
pvc.df$sub = pvc.df$src_subject_id
df_dropna <- pvc.df[!is.na(pvc.df$demean_expect_beh) & !is.na(pvc.df$demean_outcome_beh),]
df_low = df_dropna[df_dropna$stimintensity == "low", ]
low <-
  plot_twovariable(
    df_low, iv1 = "demean_expect_beh", iv2 = "demean_outcome_beh",
    group = "cuetype", subject = "sub",
    xmin=-50, xmax=50, ymin=-50, ymax=50,
    xlab = "expectation rating\n(subjectwise mean-centered)", ylab = "Outcome rating",
    ggtitle = "Low intensity", #DB6000
...
med <-
  plot_twovariable(
    df_med, iv1 = "demean_expect_beh", iv2 = "demean_outcome_beh",
    group = "cuetype", subject = "sub",
    xmin=-50, xmax=50, ymin=-50, ymax=50,
    xlab = "expectation rating\n(subjectwise mean-centered)", ylab = "Outcome rating",
    ggtitle = "Medium intensity",
...
high <-
  plot_twovariable(
    df_high, iv1 = "demean_expect_beh", iv2 = "demean_outcome_beh",
    group = "cuetype", subject = "sub",
    xmin=-50, xmax=50, ymin=-50, ymax=50,
    xlab = "expectation rating\n(subjectwise mean-centered)", ylab = "Outcome rating",
    ggtitle = "High intensity",
...
## save dataframe
```{r}
write.csv(NPS.df,'/Users/h/Dropbox (Dartmouth College)/projects_dropbox/social_influence_analysis/analysis/mixedeffect/CCNfigures/dataframe.csv')
```
