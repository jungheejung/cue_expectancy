...
for (taskname in c("pain", "vicarious", "cognitive")) {
  # 1. filter based on SPM intersecting subject ids
      # "scripts/step04_SPM/6cond_highlowcue_rampupplateau/SPM_6condrampup_vicarious_contrast_BIDSid.csv
  subid_df <-

    read.csv(file.path(
      main_dir,
      paste0(
        "scripts/step04_SPM/6cond_highlowcue_rampupplateau/SPM_6condrampup_",        taskname,
...
data.ses$sub <- as.factor(data.ses$sub)
  ...
  model_iv2 <- "cue_ordered"
subject <- "sub"
data.ses$stim_ordered <- as.factor(data.ses$stim_ordered)
data.ses$cue_ordered <- as.factor(data.ses$cue_ordered)
data.ses$sub <- as.factor(data.ses$sub)
 ...
 # calculate mean and se
  ...
  g <- plot_halfrainclouds_twofactorthick(
