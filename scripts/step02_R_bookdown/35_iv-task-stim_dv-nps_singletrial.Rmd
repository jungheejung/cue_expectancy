...
```
```{r}
model.nps <- lmer(NPSpos ~ cue_con*stim_con_linear + cue_con*stim_con_quad + factor(ses) +  (cue_con|sub), data = data_screen)
...
```{r}
print("eta squared")
eta_squared(model.nps, partial = TRUE)
parameters::model_parameters(model.nps)
lme.dscore(model.nps, data_screen, type = "lme4")
#r2mlm(model.nps)
emm1 = emmeans(model.nps, specs = pairwise ~ cue_con|stim_con_linear)
```
