...
model.alltask <- lmer(NPSpos ~ task_con_linear*stim_factor + task_con_quad*stim_factor + (task|sub), data = pvc)
...
```{r}
# kableExtra::kable_styling(tibble::as_tibble(lme.dscore(model.alltask, pvc, type = "lme4")))
kableExtra::kable_styling(
...
### Cohen's d: NPS stimulus intensity d = 1.16, cue level d = 0.45 {.unlisted .unnumbered}
```{r echo=FALSE, message=FALSE, warning=TRUE, paged.print=FALSE}
...
#r2mlm(model.npscuestim)
#emm1 = emmeans(model.npscuestim, specs = pairwise ~ cue_con|stim_con_linear)
```
...
library(plotly)
# lmer(NPSpos ~ cue_con*stim_con_linear*event02_expect_angle + cue_con*stim_con_quad*event02_expect_angle + (cue_con|sub), data = data_screen)

fig <- plot_ly(data_screen, x = ~event02_expect_angle, y = ~NPSpos, z = ~event04_actual_angle, color = ~cue_con, colors = c('#BF382A', '#0C4B8E'))
fig <- fig %>% add_markers()
...
### Session 1: 2x3 stimulus intensity * cue {.unlisted .unnumbered}
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
combined_se_calc_cooksd <- data.frame()
taskname = "pain"
    ggtitle <- paste(taskname, " - actual judgment (degree)")
...
### Session 3: 2x3 stimulus intensity * cue {.unlisted .unnumbered}
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
combined_se_calc_cooksd <- data.frame()
taskname = "pain"
    ggtitle <- paste(taskname, " - actual judgment (degree)")
...
### Session 4: 2x3 stimulus intensity * cue {.unlisted .unnumbered}
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
combined_se_calc_cooksd <- data.frame()
taskname = "pain"
    ggtitle <- paste(taskname, " - actual judgment (degree)")
