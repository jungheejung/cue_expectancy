...
file.sources = list.files(c("/Users/h/Dropbox/projects_dropbox/social_influence_analysis/scripts/step02_R/utils"), pattern="*.R", full.names=TRUE, ignore.case=TRUE)
...
## NPS ~ session * cue * stimulus_intensity
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
iv2 = "OUTCOME_demean"; iv1 = "NPS_demean" # "NPSpos"
group = "cuetype"; subject = "sub"
# low stimulus intensity
...
low <-
  plot_twovariable(
    demean_low, iv1 , iv2,
    group = group, subject = subject,
     ymin=-50, ymax=50, xmin=-50, xmax=50,
    ylab = "Outcome rating \n(subjectwise mean-centered)", xlab =  paste(signature_key, "\n(subjectwise mean-centered)"),
    ggtitle = "Low intensity",
    color_scheme = c("cuetype-high" ="#FF8800","cuetype-low" =  "#5D5C5C"),
    alpha = .5, fit_lm = TRUE, lm_method = "lm"
  )
...
model.npsoutcome <- lmer(OUTCOME_demean ~ CUE_high_gt_low*STIM_linear*NPS_demean + CUE_high_gt_low*STIM_quadratic*NPS_demean + (CUE_high_gt_low*STIM*NPS_demean|sub), data = demean_dropna)
sjPlot::tab_model(model.npsoutcome,
                  title = "Multilevel-modeling: \nlmer(OUTCOME_demean ~ CUE * STIM * NPS_demean + (CUE * STIM *NPS_demean | sub), data = pvc)",
                  CSS = list(css.table = '+font-size: 12;'))
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
iv1 = "EXPECT_demean"; iv2 = "NPS_demean" # "NPSpos"
group = "cuetype"; subject = "sub"
# low stimulus intensity
demean_low = demean_dropna[demean_dropna$stimintensity == "low", ]
low <-
  plot_twovariable(
    ...
    group = group, subject = subject,
    ymin=-10, ymax=10, xmin=-40, xmax=40,
    xlab = "Expectation rating", ylab = paste(signature_key, " \n(mean-centered)"),
    ggtitle = "Low intensity",
    color_scheme = c("cuetype-high" ="#FF8800","cuetype-low" =  "#5D5C5C"),
    alpha = .5, fit_lm = TRUE, lm_method = "lm"
  )
...
```{r echo=TRUE, message=FALSE, warning=TRUE, paged.print=FALSE}
model.npsexpect <- lmer(NPS_demean ~ 
...
sjPlot::tab_model(model.npsexpect,
                  title = "Multilevel-modeling: \nlmer(NPS_demean ~ CUE * STIM * EXPECT + (CUE + STIM + EXPECT| sub), data = pvc)",
                  CSS = list(css.table = '+font-size: 12;'))
```
