...
- Main model: `lmer(ROI ~ cue * stim)`
- Main question: do ROI beta values, derived from single trial models, differ as a function of cue type and Stimulus intensity?
- If there is a main effect of cue on pain ROI, does this cue effect differ depending on task type?
- Is there an interaction between the two factors?
- IV:
- cue (high / low)
- stim (high / med / low)
- DV: ROI beta coeffieints
- NOTE: "flatten" means that we combine Left and Right hemisphere ROIs from the same region
...
model_savefname <- file.path(
  analysis_dir,
  paste(
    "lmer_painpathway-",dv,"_",as.character(Sys.Date()),".txt",
    sep = ""
  )
)
...
# Function to parse lmer output text files
parse_lmer_output <- function(filepath) {
  # Read the file
  lines <- readLines(filepath)

  # Find the fixed effects table
  fixed_start <- which(str_detect(lines, "Fixed effects:"))
  if (length(fixed_start) == 0) return(NULL)

  # Find where the table ends (usually at "---" or correlation section)
  fixed_end <- which(str_detect(lines, "^---$|^Signif\\. codes:|^Correlation of Fixed Effects:"))[1]

  # Extract table lines
  table_lines <- lines[(fixed_start + 1):(fixed_end - 1)]
  table_lines <- table_lines[table_lines != ""]

  # Parse into data frame
  results <- list()
  for (i in 2:length(table_lines)) {  # Skip header
    line <- table_lines[i]
    # Split by whitespace
    parts <- str_trim(unlist(str_split(line, "\\s+")))

    if (length(parts) >= 5) {
      results[[i-1]] <- data.frame(
        term = parts[1],
        estimate = as.numeric(parts[2]),
        std.error = as.numeric(parts[3]),
        df = as.numeric(parts[4]),
        statistic = as.numeric(parts[5]),
        p.value = as.numeric(parts[6]),
...
# Get all lmer output files
lmer_files <- list.files(analysis_dir, pattern = "^lmer_painpathway-.*\\.txt$", full.names = TRUE)

# Parse all files and combine
all_results <- data.frame()

for (file in lmer_files) {
  # Extract DV name from filename
  dv_name <- str_extract(basename(file), "(?<=lmer_painpathway-).*(?=_20)")

  # Parse the file
  results <- parse_lmer_output(file)

  if (!is.null(results)) {
    results$dv <- dv_name
    results$dv_label <- dv_name  # You can map to newlist if needed
...
write.csv(all_results,
          file.path(analysis_dir, paste0("all_painpathway_lmer_results_", Sys.Date(), ".csv")),
          row.names = FALSE)

# Create summary of significant effects (p < 0.05)
significant_results <- all_results %>%
  filter(p.value < 0.05, term != "(Intercept)") %>%
  select(dv, dv_label, term, estimate, std.error, df, statistic, p.value, conf.low, conf.high) %>%
  arrange(term, p.value)

# Save significant results
write.csv(significant_results,
          file.path(analysis_dir, paste0("significant_painpathway_effects_", Sys.Date(), ".csv")),
          row.names = FALSE)
...
# Count significant effects by term
sig_summary <- significant_results %>%
  group_by(term) %>%
  summarise(
    n_significant = n(),
    regions = paste(dv, collapse = ", ")
  )

cat("\n=== SUMMARY BY PREDICTOR ===\n")
print(sig_summary)

# Highlight very significant effects (p < 0.001)
highly_sig <- all_results %>%
  filter(p.value < 0.001, term != "(Intercept)") %>%
  select(dv, term, estimate, std.error, statistic, p.value, conf.low, conf.high) %>%
  arrange(p.value)

write.csv(highly_sig,
          file.path(analysis_dir, paste0("highly_significant_effects_p001_", Sys.Date(), ".csv")),
          row.names = FALSE)
