...
# count_trial <- df_clean %>% count("src_subject_id") 
# included_ids <- count_trial[(count_trial$freq > 60) & (count_trial$freq < 73),]$src_subject_id
# excluded_ids <- count_trial[(count_trial$freq < 60) & (count_trial$freq > 72),]$src_subject_id
...
subject <- "subject"
exclude <-
  "sub-0001|sub-0003|sub-0004|sub-0005|sub-0025|sub-0999" #/ "sub-0999|sub-0001|sub-0002|sub-0003|sub-0004|sub-0005|sub-0006|sub-0007|sub-0008|sub-0009|sub-0010|sub-0011"
data <- data.frame()
# load data _____________________________________
data <-
  load_task_social_df(
...
count_trial <- df_clean %>% count("src_subject_id")
included_ids <-
  count_trial[(count_trial$freq > 60) &
                (count_trial$freq < 73), ]$src_subject_id

# Subset the data frame to include only the listed IDs

data_p2_long = df_clean %>%
  pivot_longer(
    cols = c('event04_actual_angle'),
    names_to = "rating_type",
    values_to = "rating_value"
  )
data_p2_long <-
  data_p2_long[data_p2_long$src_subject_id %in% included_ids,]

data_p2_long$cue_name[data_p2_long$param_cue_type == "high_cue"] <-
  "high cue"
data_p2_long$cue_name[data_p2_long$param_cue_type == "low_cue"] <-
  "low cue"

data_p2_long$stim_name[data_p2_long$param_stimulus_type == "high_stim"] <-
  "high"
data_p2_long$stim_name[data_p2_long$param_stimulus_type == "med_stim"] <-
  "med"
data_p2_long$stim_name[data_p2_long$param_stimulus_type == "low_stim"] <-
  "low"
...
trialorder_subjectwise_p2 <- meanSummary(data_p2_long,
                                         c(model_iv2, "trial_index"), dv)
...
trialorder_subjectwise_p2[nrow(trialorder_subjectwise_p2) + 1,] <- c("low cue", 73, NA, NA)
trialorder_subjectwise_p2[nrow(trialorder_subjectwise_p2) + 1,] <- c("high cue", 73, NA, NA)
g <- ggplot(data = trialorder_subjectwise_p2,
            aes(x = trial_index,
                y = mean_per_sub,
                color = cue_ordered))+
                # group = rating_type)) +
                geom_segment(data = trialorder_subjectwise_p2,
                             aes(xend = c(tail(trial_index, n = -1), NA),
                                 yend = c(tail(mean_per_sub, n = -1), NA))) +
                  geom_point(
                    data = trialorder_subjectwise_p2,
                    aes(# shape = as.character(rating_type),
                      x = trial_index,
                      y = mean_per_sub,
                      # group = rating_type,
                      color = cue_ordered),
