...
# ==============================================================================
```{r main_loop, fig.width=10, fig.height=18, warning=FALSE, message=FALSE}
for (roi in roi_list) {
  ...
  for (run_type in run_types) {
    ...
    #   conditions: "cueH", "cueL"  (no underscore, no separate() needed)
    # --------------------------------------------------------------------------
    ...
    gw_cue$tr_sequence <- as.numeric(gw_cue$tr_sequence)
    ...
    #   conditions: "cueH_stimH", "cueL_stimH", etc. (excludes rating, cueH, cueL)
    # --------------------------------------------------------------------------
    ...
    final_plot <- grid.arrange(
      ...
      widths = c(0.5, 10),
      top    = textGrob(
        sprintf("%s  |  task: %s  |  %s", roi, run_type, Sys.Date()),
...
# =================================
```{r}
# DEBUG: test run_timeseries_lmer on one ROI/contrast directly
test_roi <- "Amy_L"
test_run <- "pain"

cue_long_test <- df[df$condition %in% c("cueH", "cueL") &
                    df$runtype == test_run &
                    df$ROI == test_roi, ] %>%
...
sw_test <- meanSummary(cue_long_test, c("sub", "tr_ordered", "cue_ordered"), "tr_value")
sw_test$mean_per_sub <- as.numeric(sw_test$mean_per_sub)

# test single TR
df_tr1 <- sw_test[sw_test$tr_ordered == "tr10", ]
cat("nrow:", nrow(df_tr1), "\n")
cat("conditions:", paste(unique(df_tr1$cue_ordered), collapse=", "), "\n")

fit <- lmer(mean_per_sub ~ cue_ordered + (1|sub), data = df_tr1)
# summary(fit)$coefficients
# print(summary(fit)$coefficients) 

cat("nrow df:", nrow(df), "\n")
cat("ROIs in df:", paste(unique(df$ROI), collapse=", "), "\n")
cat("conditions in df:", paste(unique(df$condition), collapse=", "), "\n")
cat("runtypes in df:", paste(unique(df$runtype), collapse=", "), "\n")
```
# =======
# # # LMER
# ========
...
# lmer significant ROIs after cluster correction
```{r}
# ==============================================================================
#   lmer on timeseries: test condition effect at each TR, per ROI
# ==============================================================================
...
opposite_rois <- est_wide %>%
  ...
  mutate(
    ...
    direction_cue_epoch  = ifelse(cue_epoch_cue  > 0, "cueH > cueL", "cueL > cueH"),
    direction_stim_epoch = ifelse(stim_epoch_cue > 0, "cueH > cueL", "cueL > cueH")
  ) %>%
...
#
# FIR2HTW opposite TR wise analysis and correct TR x epoch
#
...
<!-- # Ribbon -->
<!-- ```{r} -->
<!-- plot_timeseries_ribbon <- function(data, iv1, iv2, mean, error,  -->
<!--                                    xlab, ylab, ggtitle,  -->
<!--                                    color_mapping, show_legend = TRUE) { -->

<!--   # Calculate ribbon bounds (mean ± SE) -->
<!--   data$ymin <- data[[mean]] - data[[error]] -->
<!--   data$ymax <- data[[mean]] + data[[error]] -->

<!--   ggplot(data, aes_string(x = iv1, y = mean,  -->
<!--                           color = iv2, fill = iv2,  -->
<!--                           group = iv2)) + -->
<!--     geom_ribbon(aes(ymin = ymin, ymax = ymax),  -->
<!--                 alpha = 0.2,        # transparency for the shaded area -->
<!--                 color = NA) +       # no outline on ribbon -->
<!--     geom_line(size = 1.2) +         # thicker line is easier to see -->
...
<!-- ```{r} -->
<!-- # ============================================================================= -->
<!-- #   NEW RIBBON PLOTTING FUNCTION - Add this to your helper functions section -->
<!-- # ============================================================================= -->
<!-- plot_timeseries_ribbon <- function(data, iv1, iv2, mean, error,  -->
<!--                                    xlab, ylab, ggtitle,  -->
<!--                                    color_mapping, show_legend = TRUE) { -->
...
<!--   p <- ggplot(data, aes_string(x = iv1, y = mean,  -->
<!--                                color = iv2, fill = iv2,  -->
<!--                                group = iv2)) + -->
<!--     geom_ribbon(aes(ymin = ymin, ymax = ymax),  -->
<!--                 alpha = 0.2,        # semi-transparent shading -->
<!--                 color = NA) +       # no outline on ribbon -->
<!--     geom_line(size = 1.0) +         # solid line -->
<!--     scale_color_manual(values = color_mapping, name = "") + -->
<!--     scale_fill_manual(values = color_mapping, name = "") + -->
<!--     labs(x = xlab, y = ylab, title = ggtitle) -->
...
<!-- ``` -->
<!-- # ============================================================================= -->
<!-- #   MAIN LOOP - Modified to use ribbons -->
<!-- # ============================================================================= -->
<!-- ```{r main_loop, fig.width=10, fig.height=18, warning=FALSE, message=FALSE} -->
<!-- for (roi in roi_list) { -->
<!--   # ---------------------------------------------------------------------------- -->
...
<!--     p1 <- panel_theme(plot_timeseries_ribbon(  # CHANGED: ribbon instead of bar -->
<!--       gw_cue, -->
<!--       iv1            = "tr_sequence", -->
<!--       iv2            = "cue_ordered", -->
<!--       mean           = "mean_per_sub_norm_mean", -->
<!--       error          = "se", -->
<!--       xlab           = "Time (s); TR = 0.46s",  # CHANGED -->
<!--       ylab           = "BOLD (A.U.)",            # CHANGED -->
<!--       ggtitle        = paste0("A  Cue epoch — Main effect of cue", -->
<!--                               "  (N=", length(unique(sw_cue$sub)), ")"), -->
<!--       color_mapping  = CUE_COLORS, -->
<!--       show_legend    = TRUE -->
...
<!--     # -------------------------------------------------------------------------- -->
<!--     #   PLOT 2 — Main effect of STIM during stim epoch -->
<!--     # -------------------------------------------------------------------------- -->
<!--     sw_stim <- meanSummary(stim_long, -->
<!--                            c("sub", "tr_ordered", "stim_ordered"), -->
<!--                            "tr_value") -->
...
<!--     p2 <- panel_theme(plot_timeseries_ribbon(  # CHANGED -->
<!--       gw_stim, -->
<!--       iv1            = "tr_sequence", -->
<!--       iv2            = "stim_ordered", -->
<!--       mean           = "mean_per_sub_norm_mean", -->
...
<!--     # -------------------------------------------------------------------------- -->
<!--     #   PLOT 3 — Main effect of CUE during stim epoch -->
<!--     # -------------------------------------------------------------------------- -->
<!--     sw_cue_stim <- meanSummary(stim_long, -->
<!--                                c("sub", "tr_ordered", "cue_ordered"), -->
<!--                                "tr_value") -->
...
<!--     p3 <- panel_theme(plot_timeseries_ribbon(  # CHANGED -->
<!--       gw_cue_stim, -->
<!--       iv1            = "tr_sequence", -->
<!--       iv2            = "cue_ordered", -->
<!--       mean           = "mean_per_sub_norm_mean", -->
...
<!--     # -------------------------------------------------------------------------- -->
<!--     #   PLOT 4 — Cue x Stim interaction during stim epoch -->
<!--     # -------------------------------------------------------------------------- -->
<!--     sw_6cond <- meanSummary(stim_long, -->
<!--                             c("sub", "tr_ordered", "sixcond"), -->
<!--                             "tr_value") -->
...
<!--     p4 <- panel_theme(plot_timeseries_ribbon(  # CHANGED -->
<!--       gw_6cond, -->
<!--       iv1            = "tr_sequence", -->
<!--       iv2            = "sixcond", -->
<!--       mean           = "mean_per_sub_norm_mean", -->
...
<!--                          labels = round(make_breaks(gw_6cond), 2), -->
<!--                          limits = range(gw_6cond$tr_sequence)) + -->
<!--       theme(axis.title.x = element_text(size = COMMONAXIS_FONTSIZE)) -->
...
<!--   # Reshape to wide format: subjects x TRs for each condition -->
<!--   require(tidyr) -->
<!--   require(dplyr) -->
...
<!--   time_points_factor <- sort(unique(data[[time_var]])) -->
<!--     # If time_var is a factor like "tr1", "tr2", extract numbers -->
<!--   if (is.factor(time_points_factor)) { -->
<!--     time_indices <- as.numeric(time_points_factor)  # 1, 2, 3, ..., 42 -->
<!--     time_points <- (time_indices - 1) * 0.46        # Convert to seconds: 0, 0.46, 0.92, ... -->
<!--   } else { -->
<!--     time_points <- as.numeric(as.character(time_points_factor)) -->
<!--   } -->
<!--   n_trs <- length(time_points) -->

<!--   # Create wide matrices for each condition -->
<!--   cond1_data <- data %>% -->
<!--     filter(!!sym(condition_var) == conditions[1]) %>% -->
<!--     select(all_of(c(subject_var, time_var, value_var))) %>% -->
<!--     pivot_wider(names_from = all_of(time_var),  -->
...
<!--   # Helper function: find consecutive runs (clusters) -->
<!--   find_clusters <- function(significant_trs) { -->
<!--     if (sum(significant_trs) == 0) return(list()) -->
...
<!--     for (i in seq_along(rle_result$lengths)) { -->
<!--       if (rle_result$values[i]) { -->
<!--         cluster_indices <- current_pos:(current_pos + rle_result$lengths[i] - 1) -->
<!--         cluster_list <- c(cluster_list, list(cluster_indices)) -->
<!--       } -->
<!--       current_pos <- current_pos + rle_result$lengths[i] -->
<!--     } -->
<!--     return(cluster_list) -->
...
<!--   # Helper function: compute cluster mass -->
<!--   compute_cluster_mass <- function(t_stats, clusters) { -->
<!--     if (length(clusters) == 0) return(0) -->
<!--     masses <- sapply(clusters, function(c) sum(t_stats[c])) -->
<!--     return(max(abs(masses)))  # return largest absolute cluster mass -->
<!--   } -->

<!--   # STEP 1: Compute observed t-statistics at each TR -->
<!--   observed_t <- numeric(n_trs) -->
<!--   observed_p <- numeric(n_trs) -->
...
<!--     # Random flips for each subject (same across all TRs to preserve temporal structure) -->
<!--     flip <- sample(c(-1, 1), n_subjects, replace = TRUE) -->
...
<!--       # Flip signs randomly -->
<!--       diff_perm <- diff * flip -->
<!--       # t-statistic from one-sample t-test on permuted differences -->
<!--       perm_t[tr_idx] <- mean(diff_perm) / (sd(diff_perm) / sqrt(n_subjects)) -->
<!--     } -->
...
<!--   # STEP 4: Compute cluster-corrected p-value -->
<!--   cluster_p_value <- mean(null_distribution >= observed_cluster_mass) -->

<!--   # STEP 5: Identify significant clusters -->
<!--   significant_clusters <- list() -->
<!--   if (cluster_p_value < alpha && length(observed_clusters) > 0) { -->
<!--     # All clusters are significant if omnibus test passes -->
<!--     for (i in seq_along(observed_clusters)) { -->
<!--       cluster_mass <- sum(observed_t[observed_clusters[[i]]]) -->
<!--       cluster_p <- mean(null_distribution >= abs(cluster_mass)) -->
...
<!-- # ============================================================================= -->
<!-- #   Function to add cluster highlights to plot -->
<!-- # ============================================================================= -->

<!-- add_cluster_highlights <- function(plot_obj, cluster_results, alpha = 0.3) { -->

<!--   if (length(cluster_results$significant_clusters) == 0) { -->
<!--     message("No significant clusters to highlight") -->
<!--     return(plot_obj) -->
<!--   } -->

<!--   # Get y-axis range from the plot -->
<!--   y_range <- ggplot_build(plot_obj)$layout$panel_params[[1]]$y.range -->
<!--   y_min <- y_range[1] -->
<!--   y_max <- y_range[2] -->

<!--   # Add rectangles for each significant cluster -->
<!--   for (cluster in cluster_results$significant_clusters) { -->
<!--     time_range <- range(cluster$time_points) -->
...
<!--                alpha = alpha, -->
<!--                fill = "gray") + -->
<!--       # adding the horizontal bar at the bottom -->
<!--       annotate("segment", -->
<!--                x = time_range[1], -->
<!--                xend = time_range[2], -->
<!--                y = y_max * 0.05, -->
...
<!-- # ============================================================================= -->
<!-- #   Modified ribbon plotting function -->
<!-- # ============================================================================= -->
...
<!--   # Add cluster highlights if provided -->
<!--   if (!is.null(cluster_results)) { -->
<!--     p <- add_cluster_highlights(p, cluster_results, alpha = 0.15) -->
<!--   } -->
...
<!--                           "tr_value") -->
<!--     sw_cue$mean_per_sub <- as.numeric(sw_cue$mean_per_sub) -->
<!--     sw_cue <- sw_cue %>% add_tr_sequence()  # ADD THIS LINE -->

<!--     # RUN CLUSTER PERMUTATION TEST for cue effect -->
<!--     message("\n=== Testing cue epoch: cueH vs cueL ===") -->
<!--     cluster_cue <- cluster_permutation_test( -->
<!--       data = sw_cue, -->
<!--       condition_var = "cue_ordered", -->
<!--       time_var = "tr_sequence",  # CHANGED from tr_ordered -->
<!--       # time_var = "tr_ordered", -->
<!--       value_var = "mean_per_sub", -->
<!--       subject_var = "sub", -->
<!--       conditions = c("cueH", "cueL"), -->
...
<!--     message(sprintf("Cluster-corrected p-value: %.4f", cluster_cue$cluster_p_value)) -->
<!--     if (length(cluster_cue$significant_clusters) > 0) { -->
<!--       for (i in seq_along(cluster_cue$significant_clusters)) { -->
<!--         clust <- cluster_cue$significant_clusters[[i]] -->
<!--         message(sprintf("  Cluster %d: TRs %s (p = %.4f)", -->
<!--                         i, -->
<!--                         paste(range(clust$time_points), collapse = "-"), -->
<!--                         clust$p_value)) -->
<!--       } -->
<!--     } -->
...
<!--     p1 <- panel_theme(plot_timeseries_ribbon( -->
<!--       gw_cue, -->
<!--       iv1            = "tr_sequence", -->
<!--       iv2            = "cue_ordered", -->
<!--       mean           = "mean_per_sub_norm_mean", -->
<!--       error          = "se", -->
<!--       xlab           = "Time (s); TR = 0.46s", -->
<!--       ylab           = "BOLD (A.U.)", -->
<!--       ggtitle        = paste0("A  Cue epoch — Main effect of cue", -->
<!--                               "  (N=", length(unique(sw_cue$sub)), -->
<!--                               ", p=", round(cluster_cue$cluster_p_value, 3), ")"), -->
<!--       color_mapping  = CUE_COLORS, -->
<!--       show_legend    = TRUE, -->
<!--       cluster_results = cluster_cue -->
<!--     )) + -->
<!--       scale_x_continuous(breaks = make_breaks(gw_cue), -->
<!--                          labels = round(make_breaks(gw_cue), 2), -->
<!--                          limits = range(gw_cue$tr_sequence)) -->
...
<!--                            "tr_value") -->
<!--     sw_stim$mean_per_sub <- as.numeric(sw_stim$mean_per_sub) -->
<!--     sw_stim <- sw_stim %>% add_tr_sequence()  # ← ADD THIS LINE -->

<!--     # Note: For main effect of stim (3 levels), we'll test stimH vs stimL -->
<!--     message("\n=== Testing stim epoch: stimH vs stimL ===") -->
<!--     # sw_stim_2level <- sw_stim %>% filter(stim_ordered %in% c("stimH", "stimL")) -->
<!--     # PLOT 2 - Stim effect -->
<!--     sw_stim_2level <- sw_stim %>%  -->
<!--      filter(stim_ordered %in% c("stimH", "stimL")) %>% -->
<!--      add_tr_sequence()  # ADD THIS LINE -->

<!--     cluster_stim <- cluster_permutation_test( -->
<!--       data = sw_stim_2level, -->
<!--       condition_var = "stim_ordered", -->
<!--       # time_var = "tr_ordered", -->
<!--       time_var = "tr_sequence",  -->
<!--       value_var = "mean_per_sub", -->
<!--       subject_var = "sub", -->
<!--       conditions = c("stimH", "stimL"), -->
<!--       n_permutations = 5000, -->
<!--       threshold_p = 0.05, -->
<!--       alpha = 0.05 -->
<!--     ) -->

<!--     message(sprintf("Cluster-corrected p-value: %.4f", cluster_stim$cluster_p_value)) -->
...
<!--     p2 <- panel_theme(plot_timeseries_ribbon( -->
<!--       gw_stim, -->
<!--       iv1            = "tr_sequence", -->
<!--       iv2            = "stim_ordered", -->
<!--       mean           = "mean_per_sub_norm_mean", -->
...
<!--       ggtitle        = paste0("B  Stim epoch — Main effect of stim", -->
<!--                               "  (N=", length(unique(sw_stim$sub)), -->
<!--                               ", p=", round(cluster_stim$cluster_p_value, 3), ")"), -->
<!--       color_mapping  = STIM_COLORS, -->
<!--       show_legend    = TRUE, -->
<!--       cluster_results = cluster_stim -->
<!--     )) + -->
<!--       scale_x_continuous(breaks = make_breaks(gw_stim), -->
<!--                          labels = round(make_breaks(gw_stim), 2), -->
<!--                          limits = range(gw_stim$tr_sequence)) -->
...
<!--                                "tr_value") -->
<!--     sw_cue_stim$mean_per_sub <- as.numeric(sw_cue_stim$mean_per_sub) -->
<!--     sw_cue_stim <- sw_cue_stim %>% add_tr_sequence()  # ← ADD THIS LINE -->

<!--     message("\n=== Testing stim epoch: cueH vs cueL ===") -->
<!--     cluster_cue_stim <- cluster_permutation_test( -->
<!--       data = sw_cue_stim, -->
<!--       condition_var = "cue_ordered", -->
<!--       # time_var = "tr_ordered", -->
<!--       time_var = "tr_sequence",  -->
...
<!--     message(sprintf("Cluster-corrected p-value: %.4f", cluster_cue_stim$cluster_p_value)) -->
...
<!--     p3 <- panel_theme(plot_timeseries_ribbon( -->
<!--       gw_cue_stim, -->
<!--       iv1            = "tr_sequence", -->
<!--       iv2            = "cue_ordered", -->
<!--       mean           = "mean_per_sub_norm_mean", -->
...
<!--       ggtitle        = paste0("C  Stim epoch — Main effect of cue", -->
<!--                               "  (N=", length(unique(sw_cue_stim$sub)), -->
<!--                               ", p=", round(cluster_cue_stim$cluster_p_value, 3), ")"), -->
<!--       color_mapping  = CUE_COLORS, -->
<!--       show_legend    = TRUE, -->
<!--       cluster_results = cluster_cue_stim -->
<!--     )) + -->
<!--       scale_x_continuous(breaks = make_breaks(gw_cue_stim), -->
<!--                          labels = round(make_breaks(gw_cue_stim), 2), -->
<!--                          limits = range(gw_cue_stim$tr_sequence)) -->
...
<!--     # -------------------------------------------------------------------------- -->
<!--     #   PLOT 4 — Cue x Stim interaction (test cueH_stimH vs cueL_stimL) -->
<!--     # -------------------------------------------------------------------------- -->
<!--     sw_6cond <- meanSummary(stim_long, -->
<!--                             c("sub", "tr_ordered", "sixcond"), -->
<!--                             "tr_value") -->
<!--     sw_6cond$mean_per_sub <- as.numeric(sw_6cond$mean_per_sub) -->
<!--     sw_6cond <- sw_6cond %>% add_tr_sequence()   -->

<!--     message("\n=== Testing interaction: cueH_stimH vs cueL_stimL ===") -->
<!--     sw_interaction <- sw_6cond %>% filter(sixcond %in% c("cueH_stimH", "cueL_stimL")) -->

<!--     cluster_interaction <- cluster_permutation_test( -->
<!--       data = sw_interaction, -->
<!--       condition_var = "sixcond", -->
<!--       # time_var = "tr_ordered", -->
<!--       time_var = "tr_sequence",  -->
<!--       value_var = "mean_per_sub", -->
<!--       subject_var = "sub", -->
<!--       conditions = c("cueH_stimH", "cueL_stimL"), -->
<!--       n_permutations = 5000, -->
<!--       threshold_p = 0.05, -->
<!--       alpha = 0.05 -->
<!--     ) -->

<!--     message(sprintf("Cluster-corrected p-value: %.4f", cluster_interaction$cluster_p_value)) -->
...
<!--     p4 <- panel_theme(plot_timeseries_ribbon( -->
<!--       gw_6cond, -->
<!--       iv1            = "tr_sequence", -->
<!--       iv2            = "sixcond", -->
<!--       mean           = "mean_per_sub_norm_mean", -->
...
<!--       ggtitle        = paste0("D  Stim epoch — Cue x Stim interaction", -->
<!--                               "  (N=", length(unique(sw_6cond$sub)), -->
<!--                               ", p=", round(cluster_interaction$cluster_p_value, 3), ")"), -->
<!--       color_mapping  = SIXCOND_COLORS, -->
<!--       show_legend    = TRUE, -->
<!--       cluster_results = cluster_interaction -->
<!--     )) + -->
<!--       scale_x_continuous(breaks = make_breaks(gw_6cond), -->
<!--                          labels = round(make_breaks(gw_6cond), 2), -->
<!--                          limits = range(gw_6cond$tr_sequence)) + -->
