...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
model.factorize_demean = lmer(demean_outcome~ CUE_high_gt_low*stim_factor*demean_expect +lag.demean_outcome+ (1|src_subject_id), data = pvc)
summary(model.factorize_demean)
```
...
cooksd <- lmer_onefactor_cooksd(pvc,
                               taskname = "pain",
                               iv = "cue_name",
                               dv = "odr_expectoutcome",
                               subject_keyword = "src_subject_id",
                               dv_keyword = "distance from 1:1",
                               model_savefname = file.path(analysis_dir, "TEST.txt"),
...
iv <- "cue_name"; iv_keyword <- "cue";
dv <- "odr_expectoutcome"; dv_keyword <- "odr_expectoutcome"
...
save_fname = file.path(analysis_dir, "TESTpain.png")
...
total <-
  plot_twovariable(
    ...
    alpha = .8, fit_lm = TRUE, lm_method = "lm", identity_line = TRUE, size = 1
  )
total
```
...
save_fname = file.path(analysis_dir, "TESTvicarious.png")
p <- plot_halfrainclouds_onefactor(subjectwise, groupwise, iv,
                                      subjectwise_mean, group_mean, se, subject,
                                      ggtitle, title, xlab, ylab, taskname, ylim,
                                      w = 10, h = 10, dv_keyword, color =color_scheme, save_fname)
...
pvc$odr_expectoutcome = abs(pvc$demean_expect + pvc$demean_outcome) / sqrt(2)
cooksd = lmer_onefactor_cooksd(pvc, "pain", iv = "cue_name", dv= "odr_expectoutcome", subject_keyword = "src_subject_id",
                                  dv_keyword = "distance from 1:1", model_savefname = file.path(analysis_dir, "TESTcog.txt"), print_lmer_output = TRUE)
...
save_fname = file.path(analysis_dir, "TESTcog.png")
p <- plot_halfrainclouds_onefactor(subjectwise, groupwise, iv,
                                      subjectwise_mean, group_mean, se, subject,
                                      ggtitle, title, xlab, ylab, taskname, ylim,
                                      w = 10, h = 10, dv_keyword, color =color_scheme, save_fname)
...
- expectancy-outcome relationship differs across cue and tasks, "taskname\*lin:CUE*high_gt_low:demean_expect", t(14130) = 5.758, p < .001
  ...
  taskname*lin:CUE_high_gt_low:demean_expect 2.629e-01 4.565e-02 1.413e+04 5.758 8.67e-09 *\*\*
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
con.pvc <- pvc.pain[pvc.pain$congruency_status == "congruent", ]

df_dropna <- con.pvc[!is.na(con.pvc$congruency_status),]
df_low = df_dropna[df_dropna$param_stimulus_type == "low_stim", ]
low <-
  plot_twovariable(
    df_low, iv1 = "demean_expect", iv2 = "demean_outcome",
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
incon.pvc <- pvc.pain[pvc.pain$congruency_status == "incongruent", ]

df_dropna <- incon.pvc[!is.na(incon.pvc$congruency_status),]
df_low = df_dropna[df_dropna$param_stimulus_type == "low_stim", ]
low <-
  plot_twovariable(
    df_low, iv1 = "demean_expect", iv2 = "demean_outcome",
    ...
    xlab = "expectation rating", ylab = "outcome rating",
    ggtitle = "Low intensity",
    color_scheme = c("high_cue" ="#C24EB2","low_cue" =  "#5D5C5C"),
    alpha = .8, fit_lm = TRUE, lm_method = "lm", identity_line = TRUE
  )
...
med <-
  plot_twovariable(
    ...
    xlab = "expectation rating", ylab = "outcome rating",
    ggtitle = "Medium intensity",
    color_scheme = c("high_cue" ="#B238D9","low_cue" =  "#5D5C5C"),
    alpha = .8, fit_lm = TRUE, lm_method = "lm", identity_line = TRUE
  )
...
high <-
  plot_twovariable(
    ...
    xlab = "expectation rating", ylab = "outcome rating",
    ggtitle = "High intensity",
    color_scheme = c("high_cue" ="#A323FF","low_cue" =  "#5D5C5C"),
    alpha = .8, fit_lm = TRUE, lm_method = "lm", identity_line = TRUE
  )
plots <- ggpubr::ggarrange(low, med, high, ncol = 3, nrow = 1, common.legend = FALSE, legend = "bottom")
plots_title <- annotate_figure(plots, top = text_grob("individual differences\n - cue effects from outcome ratings", color = "black", face = "bold", size = 15))
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
con.pvc <- pvc.pain[pvc.pain$congruency_status2 == "congruent", ]

df_dropna <- con.pvc[!is.na(con.pvc$demean_expect) & !is.na(con.pvc$demean_outcome),]
df_low = df_dropna[df_dropna$param_stimulus_type == "low_stim", ]
low <-
  plot_twovariable(
    df_low, iv1 = "demean_expect", iv2 = "demean_outcome",
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
incon.pvc <- pvc.pain[pvc.pain$congruency_status2 == "incongruent", ]

df_dropna <- incon.pvc[!is.na(incon.pvc$demean_expect) & !is.na(incon.pvc$demean_outcome),]
df_low = df_dropna[df_dropna$param_stimulus_type == "low_stim", ]
low <-
  plot_twovariable(
    df_low, iv1 = "demean_expect", iv2 = "demean_outcome",
