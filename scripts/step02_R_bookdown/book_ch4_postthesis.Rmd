...
analysis_dir <- "/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure"
...
sink(file.path(savedir, "beh_output.txt")) # Redirects output to 'my_output.txt'
summary(model.beh)
```
...
cue_means <- df_filter %>%
  ...
  summarise(
    mean = mean(outcomerating_impute, na.rm = TRUE),
...
print(cue_means)
# # Get marginal means for CUE, collapsing across STIM
# emm_cue_marginal <- emmeans(model.beh, ~ CUE_high_gt_low)
# 
# # Summary with SE, CI, and stats
# summary(emm_cue_marginal)
# 
# # Test the difference between high and low cue
# contrast_cue <- pairs(emm_cue_marginal)
# summary(contrast_cue)
# 
# # If you want more detail
# confint(emm_cue_marginal)  # Confidence intervals
# test(emm_cue_marginal)     # Hypothesis tests
```
...
g <- plot_lineplot_twofactor_subset(subset,taskname = "pain",
...
ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/fig1', paste0("fig1_beh_maineffect", as.character(Sys.Date()), ".svg")), plot=k, w=4, h=4, dpi=300)
...
ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/fig2', paste0("NPS_maineffect", as.character(Sys.Date()), ".svg")), plot=k, w=4, h=4, dpi=300)
```
...
ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/fig2', paste0("NPSpos_maineffect", as.character(Sys.Date()), ".svg")), plot=k, w=4, h=4, dpi=300)
```

### NPS pos linear analysis
```{r}
model.NPSpos <- lmer(npspos ~ CUE_high_gt_low*STIM_linear +
       CUE_high_gt_low*STIM_quadratic + (CUE_high_gt_low+STIM_linear ||sub), data = df_filter)
# summary(model.NPSpos)
sjPlot::tab_model(model.NPSpos,
                  title = "Multilevel-modeling: \nlmer(NPSpos ~ CUE * STIM + (CUE + STIM || sub), data = pain)",
                  CSS = list(css.table = '+font-size: 12;'))
equatiomatic::extract_eq(model.NPSpos)
sink("/Users/h/Desktop/NPSpos_output.txt")  # Redirects output to 'my_output.txt'
summary(model.NPSpos)
sink(NULL)  #
...
ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/fig2', paste0("NPSneg_maineffect", as.character(Sys.Date()), ".svg")), plot=k, w=4, h=4, dpi=300)
```


### NPS pos linear analysis
```{r}
model.NPSneg <- lmer(npsneg ~ CUE_high_gt_low*STIM_linear +
       CUE_high_gt_low*STIM_quadratic + (STIM_linear|sub), data = df_filter)

sjPlot::tab_model(model.NPSneg,
                  title = "Multilevel-modeling: \nlmer(NPSneg ~ CUE * STIM + (CUE + STIM || sub), data = pain)",
                  CSS = list(css.table = '+font-size: 12;'))
equatiomatic::extract_eq(model.NPSneg)
sink("/Users/h/Desktop/NPSneg_output.txt")  # Redirects output to 'my_output.txt'
summary(model.NPSneg)
sink(NULL)  #
...
plot the nps signal from cue epoch
```{r}
# load NPS

# rename NPS as NPScueepoch
# merge with df_filter
# 
# Load data
NPS_cueepoch <- read.csv("/Users/h/Documents/projects_local/cue_expectancy/data/curated/sub-all_signature-NPS_runtype-pain_event-cue.csv")
...
# Merge
# df_filter <- merge(df_filter, NPS_cueepoch, by = "singletrial_fname", all.x = TRUE)

NPS_cueepoch$stim <- NA; NPS_cueepoch$STIM_linear <- NA; NPS_cueepoch$STIM_quadratic <- NA;
NPS_cueepoch$CUE_high_gt_low <- NA;
NPS_cueepoch$SES_linear <- NA;NPS_cueepoch$SES_quadratic <- NA
# NPS_cueepoch$stim[NPS_cueepoch$stimulusintensity == "low_stim"] <-  -0.5 # social influence task
# NPS_cueepoch$stim[NPS_cueepoch$stimulusintensity == "med_stim"] <- 0 # no influence task
# NPS_cueepoch$stim[NPS_cueepoch$stimulusintensity == "high_stim"] <-  0.5 # no influence task
# 
# # NPS_cueepoch$STIM <- factor(NPS_cueepoch$stimulusintensity)
# 
# # contrast code 1 linear
# NPS_cueepoch$STIM_linear[NPS_cueepoch$stimulusintensity == "low_stim"] <- -0.5
# NPS_cueepoch$STIM_linear[NPS_cueepoch$stimulusintensity == "med_stim"] <- 0
# NPS_cueepoch$STIM_linear[NPS_cueepoch$stimulusintensity == "high_stim"] <- 0.5
# 
# # contrast code 2 quadratic
# NPS_cueepoch$STIM_quadratic[NPS_cueepoch$stimulusintensity == "low_stim"] <- -0.33
# NPS_cueepoch$STIM_quadratic[NPS_cueepoch$stimulusintensity == "med_stim"] <- 0.66
# NPS_cueepoch$STIM_quadratic[NPS_cueepoch$stimulusintensity == "high_stim"] <- -0.33

# # social cude contrast
# NPS_cueepoch$CUE_high_gt_low[NPS_cueepoch$cue == "low_cue"] <-  -0.5 # social influence task
# NPS_cueepoch$CUE_high_gt_low[NPS_cueepoch$cue == "high_cue"] <-  0.5 # no influence task

NPS_cueepoch$EXPECT <- NPS_cueepoch$expectrating
NPS_cueepoch$OUTCOME <- NPS_cueepoch$outcomerating


# Extract subject ID from singletrial_fname
NPS_cueepoch <- NPS_cueepoch %>%
  mutate(sub = str_extract(singletrial_fname, "sub-\\d+"))

# Or using base R:
NPS_cueepoch$sub <- sub("(sub-\\d+).*", "\\1", NPS_cueepoch$singletrial_fname)

# Verify extraction
table(NPS_cueepoch$sub)
head(NPS_cueepoch %>% select(singletrial_fname, sub))
NPS_cueepoch <- NPS_cueepoch %>%
  mutate(
    cue = str_extract(singletrial_fname, "cuetype-(high|low)"),
    cue = str_replace(cue, "cuetype-", ""),
    cue = paste0(cue, "_cue")  # Convert "high" to "high_cue"
  )

# Create contrast-coded variable
NPS_cueepoch$CUE_high_gt_low <- NA
NPS_cueepoch$CUE_high_gt_low[NPS_cueepoch$cue == "low_cue"] <- -0.5
NPS_cueepoch$CUE_high_gt_low[NPS_cueepoch$cue == "high_cue"] <- 0.5


# linear analysis ____________________________________
model.cueepochNPS <- lmer(cueepoch_NPS ~ CUE_high_gt_low + (CUE_high_gt_low|sub), data = NPS_cueepoch)
summary(model.cueepochNPS)
sjPlot::tab_model(model.cueepochNPS,
                  title = "Multilevel-modeling: \nlmer(NPS_cueepoch ~ CUE + (CUE  | sub), data = NPS_cueepoch)",
                  CSS = list(css.table = '+font-size: 12;'))
equatiomatic::extract_eq(model.cueepochNPS)
sink("/Users/h/Desktop/cueepochNPS_output.txt")  # Redirects output to 'my_output.txt'
summary(model.cueepochNPS)
sink(NULL)  #

# linear analysis ____________________________________
model.cueepochNPSpos <- lmer(cueepoch_NPSpos ~ CUE_high_gt_low + (CUE_high_gt_low|sub), data = NPS_cueepoch)
summary(model.cueepochNPSpos)
sjPlot::tab_model(model.cueepochNPSpos,
                  title = "Multilevel-modeling: \nlmer(NPS_cueepoch ~ CUE + (CUE  | sub), data = NPS_cueepoch)",
                  CSS = list(css.table = '+font-size: 12;'))
equatiomatic::extract_eq(model.cueepochNPSpos)
sink("/Users/h/Desktop/cueepochNPSpos_output.txt")  # Redirects output to 'my_output.txt'
summary(model.cueepochNPSpos)
sink(NULL)  #
```
...
g <- cueR::plot_lineplot_twofactor_subset(SIIPS_groupwise, taskname = "pain",
                        iv1 = "stim_ordered", iv2 = "cue_ordered",
                        mean = "mean_per_sub_norm_mean", error = "se",
...
ggsave(file.path(analysis_dir, paste0("SIIPS_maineffect", as.character(Sys.Date()), ".svg")), plot=k, w=4, h=4, dpi=300)
    ...
    ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/supp', paste0("SIIPS_by_session_", Sys.Date(), ".svg")), plot=k, w=4, h=4, dpi=300)
```
...
taskname <- "pain"
subject <- "sub"
df_filter$stim_name <- NA
df_filter$cue_name <- NA
...
# Plot 3: Low cue only ________________________________________________________
g <- cueR::plot_lineplot_twofactor_subset(
  SIIPS_groupwise[SIIPS_groupwise$cue_ordered == "low cue", ], 
  taskname = "pain",  # Changed from task to taskname
  iv1 = "stim_ordered", 
  iv2 = "cue_ordered",
  mean = "mean_per_sub_norm_mean", 
  error = "se",
  color = c("low cue" = lowcuehex), 
  ggtitle = paste0("SIIPS N = ", length(unique(SIIPS_subjectwise$sub))),
...
ggsave(file.path(analysis_dir, paste0("SIIPS_maineffect_", Sys.Date(), "_lowcue.svg")), 
       plot = g, width = 4, height = 4, dpi = 300)

# Plot 4: Both cues ____________________________________________________________
k <- cueR::plot_lineplot_twofactor_subset(
  SIIPS_groupwise, 
  taskname = "pain",  # Changed from task to taskname
  iv1 = "stim_ordered", 
  iv2 = "cue_ordered",
...
ggsave(file.path(analysis_dir, paste0("SIIPS_maineffect_", Sys.Date(), ".svg")), 
       plot = k, width = 4, height = 4, dpi = 300)

ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/supp', 
                 paste0("SIIPS_by_session_", Sys.Date(), ".svg")), 
       plot = k, width = 4, height = 4, dpi = 300)
```
...
sink(file.path("/Users/h/Desktop", "SIIPS_output.txt"))  # Redirects output to 'my_output.txt'
summary(model.SIIPS)
...
# load SCR data
SCR_wide <- read.csv(file.path(main_dir, "analysis/physio/nobaseline/glm/glm-factorial_task-pain_scr.tsv"), sep = "\t")
# Reshape SCR from wide to long format

# Convert SCR from wide to long format _________________________________________
SCR_long <- SCR_wide %>%
  pivot_longer(
  ...
  mutate(
    stimulusintensity = paste0(stimulusintensity),
    cue = paste0(cue)
  )


# Check overlapping subjects ___________________________________________________
scr_subjects <- unique(SCR_long$sub)
data_subjects <- unique(data$sub)

overlapping_subjects <- intersect(scr_subjects, data_subjects)
cat("Number of overlapping subjects:", length(overlapping_subjects), "\n")
cat("SCR subjects not in data:", setdiff(scr_subjects, data_subjects), "\n")
cat("Data subjects not in SCR:", setdiff(data_subjects, scr_subjects), "\n")

# Filter for overlapping subjects only _________________________________________
SCR_filtered <- SCR_long %>%
  filter(sub %in% overlapping_subjects)

# Prepare plotting variables ___________________________________________________
SCR_filtered$cue_name <- ifelse(SCR_filtered$cue == "high_cue", "High cue", "Low cue")
SCR_filtered$stim_name <- case_when(
...
SCR_filtered$stim_ordered <- factor(SCR_filtered$stim_name, levels = c("Low", "Med", "High"))
SCR_filtered$cue_ordered <- factor(SCR_filtered$cue_name, levels = c("Low cue", "High cue"))

# Calculate subject-wise means _________________________________________________
SCR_subjectwise <- SCR_filtered %>%
  group_by(sub, stim_ordered, cue_ordered) %>%
  summarise(mean_per_sub = mean(scr, na.rm = TRUE), .groups = "drop")

# Calculate group-level summary ________________________________________________
SCR_groupwise <- SCR_subjectwise %>%
...
# p <- ggplot(SCR_groupwise, aes(x = stim_ordered, y = mean, 
#                                 color = cue_ordered, group = cue_ordered)) +
#   geom_linerange(aes(ymin = mean - se, ymax = mean + se), 
#                  linewidth = 0.5) +
#   geom_line(linewidth = 1) +
#   geom_point(size = 4, shape = 16) +
#   scale_color_manual(values = c("Low cue" = lowcuehex, "High cue" = highcuehex)) +
#   labs(
#     title = "SCR",
...
p <- cueR::plot_lineplot_twofactor_subset(SCR_groupwise, 
                                          taskname = "pain",
                        iv1 = "stim_ordered", iv2 = "cue_ordered",
                        mean = "mean", error = "se",
                        color = c("Low cue" = lowcuehex,"High cue" = highcuehex),
                        ggtitle = paste0("SCR N =", length(unique(SCR_subjectwise$sub))),
...
# ggsave(file.path(analysis_dir, paste0("SCR_maineffect_", Sys.Date(), ".svg")),
#        plot = p, width = 8, height = 4, dpi = 300)

# Print summary table __________________________________________________________
print(SCR_groupwise)
# Save
ggsave(file.path(analysis_dir, paste0("fig1/SCR_maineffect_", Sys.Date(), ".svg")),
       plot = p, width = 8, height = 4, dpi = 300)
```
...
SCR_groupwise <- SCR_subjectwise %>%
  group_by(stim_ordered, cue_ordered) %>%
  summarise(
    mean_per_sub_norm_mean = mean(mean_per_sub, na.rm = TRUE),  # Changed from "mean"
    se = sd(mean_per_sub, na.rm = TRUE) / sqrt(n()),
    .groups = "drop"
  )
...
### SCR pos linear analysis
```{r}
# Apply contrast coding to SCR data ____________________________________________

# Initialize contrast columns
SCR_long$stim <- NA
SCR_long$STIM_linear <- NA
SCR_long$STIM_quadratic <- NA
SCR_long$CUE_high_gt_low <- NA

# STIM contrast coding (same as your original scheme) _________________________
# stim: -0.5, 0, 0.5
SCR_long$stim[SCR_long$stimulusintensity == "low_stim"] <- -0.5
SCR_long$stim[SCR_long$stimulusintensity == "med_stim"] <- 0
SCR_long$stim[SCR_long$stimulusintensity == "high_stim"] <- 0.5

# STIM_linear: -0.5, 0, 0.5
SCR_long$STIM_linear[SCR_long$stimulusintensity == "low_stim"] <- -0.5
SCR_long$STIM_linear[SCR_long$stimulusintensity == "med_stim"] <- 0
SCR_long$STIM_linear[SCR_long$stimulusintensity == "high_stim"] <- 0.5

# STIM_quadratic: -0.33, 0.66, -0.33
SCR_long$STIM_quadratic[SCR_long$stimulusintensity == "low_stim"] <- -0.33
SCR_long$STIM_quadratic[SCR_long$stimulusintensity == "med_stim"] <- 0.66
SCR_long$STIM_quadratic[SCR_long$stimulusintensity == "high_stim"] <- -0.33

# CUE contrast coding __________________________________________________________
# CUE_high_gt_low: -0.5, 0.5
SCR_long$CUE_high_gt_low[SCR_long$cue == "low_cue"] <- -0.5
SCR_long$CUE_high_gt_low[SCR_long$cue == "high_cue"] <- 0.5

# Create factor version
SCR_long$STIM <- factor(SCR_long$stimulusintensity)

# Verify the coding ____________________________________________________________
cat("STIM coding verification:\n")
table(SCR_long$stimulusintensity, SCR_long$STIM_linear)
table(SCR_long$stimulusintensity, SCR_long$STIM_quadratic)

cat("\nCUE coding verification:\n")
table(SCR_long$cue, SCR_long$CUE_high_gt_low)

# Filter for overlapping subjects ______________________________________________
SCR_filtered <- SCR_long %>%
  filter(sub %in% overlapping_subjects)

# Run the model ________________________________________________________________
library(lmerTest)

model.SCR <- lmer(scr ~ CUE_high_gt_low*STIM_linear +
       CUE_high_gt_low*STIM_quadratic + 
       (CUE_high_gt_low + STIM_linear || sub),  
       data = SCR_filtered)

# Check for singularity ________________________________________________________
if (isSingular(model.SCR)) {
  cat("Model is singular. Trying simpler random effects...\n")
  model.SCR <- lmer(scr ~ CUE_high_gt_low*STIM_linear +
         CUE_high_gt_low*STIM_quadratic + 
         (1 | sub), 
         data = SCR_filtered)
...
# Display results ______________________________________________________________
sjPlot::tab_model(model.SCR,
                  title = "Multilevel-modeling: \nlmer(SCR ~ CUE * STIM + (CUE + STIM || sub))",
                  CSS = list(css.table = '+font-size: 12;'))

equatiomatic::extract_eq(model.SCR)

# Save and print output ________________________________________________________
sink("/Users/h/Desktop/SCR_output.txt")
summary(model.SCR)
sink(NULL)
...
df_filter$singletrial_fname <- sapply(df_filter$singletrial_fname, clean_filename)  # ADD THIS

# Then proceed with your overlap check and join
overlapping_cols <- intersect(names(df_filter), names(PEdf))
overlapping_cols <- setdiff(overlapping_cols, "singletrial_fname")
...
write.csv(PEmerge, file.path(main_dir, "data/curated/sub-all_runtype-pain_merge.tsv"), sep = "\t", row.names = FALSE)
library(jsonlite)

# Create the list/data structure
metadata <- list(
  code_name = "~/scripts/step02_R_bookdown/book_ch4_postthesis.Rmd",
  code_path = "~/scripts/step02_R_bookdown/book_ch4_postthesis.Rmd",
  description = "This code compiles NPS, SIIPS, behavioral, and model-fitted RL data in one dataframe while removing NA values.",
  input_files_directory = "~/analysis/fmri/nilearn",
  output_directory = "/dartfs-hpc/rc/lab/C/CANlab/labdata/projects/spacetop_projects_cue/data/durated"
)

# Save as JSON (pretty formatted)
write_json(metadata, file.path(main_dir, "data/curated/sub-all_runtype-pain_merge.json"), pretty = TRUE, auto_unbox = TRUE)
...
n_subjects <- length(unique(PEmerge$sub))
batlow_colors <- scico(n = n_subjects, palette = "batlow", begin = 0.1, end = 0.9)
# viridis - colorblind-friendly, perceptually uniform
viridis_colors <- viridis::viridis(n = n_subjects, option = "B")  # "D" = viridis
# or try: option = "C" (plasma), "B" (inferno), "A" (magma)
...
plot.PE_NPS <- ggplot(PEmerge, aes(x = PE_mdl2, y = NPS)) +
  geom_point(aes(colour = sub), size = .1, alpha = .3) +  # Points colored by subject
...
ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/fig4', paste0("NPS_PE", as.character(Sys.Date()), ".svg")), plot=plot.PE_NPS, w=4, h=4, dpi=300)
...
svg("/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/fig4_correlation.svg", width = 8, height = 8)
...
ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/supp', paste0("correlation", as.character(Sys.Date()), ".svg")), plot=k, w=4, h=4, dpi=300)
```
...
NPSPEcombined <- lmer(NPS ~ CUE_high_gt_low + STIM_linear +
       STIM_quadratic  + scale(PE_mdl2) + (1  | sub),
     data = PEmerge)
     # control=lmerControl(isSingular(NPSPE, tol = 1e-4)))
     # check.conv.singular = .makeCC(action = "ignore",  tol = 1e-4)))
summary(NPSPEcombined)
sjPlot::tab_model(NPSPEcombined,
                  title = "model 2: \nlmer(NPS ~ cue + stim + PE + (cue + stim | sub), data = pain)",
                  CSS = list(css.table = '+font-size: 12;'))
equatiomatic::extract_eq(NPSPEcombined)
# Save and print output ________________________________________________________
sink("/Users/h/Desktop/NPSPEpartial_output.txt")
summary(NPSPEcombined)
library(sjPlot)
...
# print(opt.summary)
# Extract the summary properly
opt.summary <- summary(aa)
print(opt.summary)

# Or if you want a data frame:
opt.df <- as.data.frame(summary(aa)$fixef)
print(opt.df)
...
r_squared <- performance::r2(model.npscuestim)
...
print(g)
  ...
  ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/fig3', paste0('lineplot_task-all_rating-', dv_keyword,dv, '.svg', sep = "")), plot=k, w=4, h=4, dpi=300)
...
#### 7-2. plot ROI per session. 
```{r}
#### plot sessionwise ROIs [ Update ]
library(ggplot2)
library(dplyr)
...
for (i in seq_along(dependent_vars)) {
  ...
  df_filter_ses$cue_name <- ifelse(df_filter_ses$cue == "high_cue", "High cue", "Low cue")
  
  df_filter_ses$stim_name <- case_when(
    df_filter_ses$stimulusintensity == "low_stim" ~ "Low",
    df_filter_ses$stimulusintensity == "med_stim" ~ "Med",
    df_filter_ses$stimulusintensity == "high_stim" ~ "High"
  )
  ...
  ggsave(file.path(analysis_dir, paste0(dv, "_by_session_", Sys.Date(), ".svg")),
    ...
    ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/supp', paste0("roi-", dv, "_bysession_", Sys.Date(), ".svg")), plot=k, w=4, h=4, dpi=300)
}
```
...
print(sprintf("after filtering out subjects that have less than 2 trials in cell, we have N=%d -> N=%d",length(unique(data$sub)), length(unique(df_filter_ses$sub)) ))
```
...
beh_summary <- df_filter_ses %>%
  group_by(ses_name, stim_ordered, cue_ordered, sub) %>%
  summarise(mean_per_sub = mean(outcomerating_impute, na.rm = TRUE), .groups = "drop") %>%
  group_by(ses_name, stim_ordered, cue_ordered) %>%
  summarise(
    mean = mean(mean_per_sub, na.rm = TRUE),
    se = sd(mean_per_sub, na.rm = TRUE) / sqrt(n()),
...
# 3. Plot with facets by session
p <- ggplot(beh_summary, aes(x = stim_ordered, y = mean, 
                                color = cue_ordered, group = cue_ordered)) +
  geom_linerange(aes(ymin = mean - se, ymax = mean + se), 
                 linewidth = 0.5) +
  geom_line(linewidth = 1) +
...
# Save
ggsave(file.path(analysis_dir, paste0("beh_by_session_", Sys.Date(), ".svg")),
       plot = p, width = 10, height = 4, dpi = 300)

    ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/supp', paste0("beh_by_session_", Sys.Date(), ".svg")), plot=k, w=4, h=4, dpi=300)
```


## 10. Sessionwise NPS [ Update ]

> 
* instead of | I'm using || to keep the correlation across random slopes 0.
* This effectively reduces the parameter space from 10 -> 4
* Even so, the model is singular
...
```
Let's check the correlation across variables
```{r}
VarCorr(model.NPSses)

```
> Cue and stimulus intensity, essentially zero variability across participants. 
Subjects don't vary in their CUE response
Subjects don't vary in their STIM response
Subjects DO vary in their SES response (Std.Dev = 1.81) and baseline (1.52)


> Therefore, I adjust to model to simply include the SES random slopes

> Interpretation: the cue and stimulu interaction effect (prehpas predictive coding) differs as a function of session. the stimulus response becomes increasingly nonlinear from session 1 to session 3, while in the low cue condition this pattern is attenuated or reversed.
...
pred <- ggpredict(model.NPSses, 
                  terms = c("STIM_quadratic [-0.33, 0, 0.66]", 
                           
                           "CUE_high_gt_low [-0.5, 0.5]", "SES_linear [-0.5, 0, 0.5]"))
...
# 2. Calculate summary statistics
NPS_summary <- df_filter_ses %>%
  group_by(ses_name, stim_ordered, cue_ordered, sub) %>%
  summarise(mean_per_sub = mean(NPS, na.rm = TRUE), .groups = "drop") %>%
  group_by(ses_name, stim_ordered, cue_ordered) %>%
  summarise(
    mean = mean(mean_per_sub, na.rm = TRUE),
    se = sd(mean_per_sub, na.rm = TRUE) / sqrt(n()),
...
# 3. Plot with facets by session - NO STRIP BARS
p <- ggplot(NPS_summary, aes(x = stim_ordered, y = mean, 
                                color = cue_ordered, group = cue_ordered)) +
  geom_linerange(aes(ymin = mean - se, ymax = mean + se), 
                 linewidth = 0.5) +
  geom_line(linewidth = 1) +
  ...
  theme(
    ...
    strip.text = element_text(size = 14, face = "bold"),  # Keep text, remove bar
    legend.position = "bottom"
  )
...
# Save
ggsave(file.path(analysis_dir, paste0("NPS_by_session_", Sys.Date(), ".svg")),
       plot = p, width = 10, height = 4, dpi = 300)

    ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/supp', paste0("NPS_by_session_", Sys.Date(), ".svg")), plot=k, w=4, h=4, dpi=300)
```

## 10-2. Session wise NPSpos [ Update ]

> Because the variance of random slopes for cue and stim effect is essentially 0..
We'll remove them from the random slope estimation.

 Groups   Name            Std.Dev.  
 sub      (Intercept)     9.4459e-01
 sub.1    CUE_high_gt_low 2.8995e-10
 sub.2    STIM_linear     1.5846e-04
 sub.3    SES_linear      1.2570e+00
 Residual                 2.0591e+00
...
sjPlot::tab_model(model.NPSposses,
                  title = "Multilevel-modeling: \nlmer(NPS positive weights ~ CUE * STIM * SES+ (CUE + STIM + SES|| sub), data = pain)",
                  CSS = list(css.table = '+font-size: 12;'))
equatiomatic::extract_eq(model.NPSposses)

# Check the correlation across random effects
# VarCorr(model.NPSposses)
```

#### 10-2a. plot sessionwise NPSpos
```{r}
library(ggplot2)
library(dplyr)
...
# 2. Calculate summary statistics
NPSpos_summary <- df_filter_ses %>%
  group_by(ses_name, stim_ordered, cue_ordered, sub) %>%
  summarise(mean_per_sub = mean(npspos, na.rm = TRUE), .groups = "drop") %>%
  group_by(ses_name, stim_ordered, cue_ordered) %>%
  summarise(
    mean = mean(mean_per_sub, na.rm = TRUE),
    se = sd(mean_per_sub, na.rm = TRUE) / sqrt(n()),
...
# 3. Plot with facets by session
p <- ggplot(NPSpos_summary, aes(x = stim_ordered, y = mean, 
                                color = cue_ordered, group = cue_ordered)) +
  geom_linerange(aes(ymin = mean - se, ymax = mean + se), 
                 linewidth = 0.5) +
  geom_line(linewidth = 1) +
...
# Save
ggsave(file.path(analysis_dir, paste0("NPSpos_by_session_", Sys.Date(), ".svg")),
       plot = p, width = 10, height = 4, dpi = 300)

    ggsave(file.path('/Users/h/Dropbox (Personal)/2025_Jung_SpaceTop_PVC_pain_CueFx_Signatures_ROIs/Figure/supp', paste0("NPSpos_by_session_", Sys.Date(), ".svg")), plot=k, w=4, h=4, dpi=300)
```


## 11. Sessionwise SIIPS [ Update ]
...
### linear estimator
> We can drop this since the model above works. This code simply tests different optimizers from the optimx family and the nloptwrap family
...
# 2. Calculate summary statistics
SIIPS_summary <- df_filter_ses %>%
  group_by(ses_name, stim_ordered, cue_ordered, sub) %>%
  summarise(mean_per_sub = mean(SIIPS, na.rm = TRUE), .groups = "drop") %>%
  group_by(ses_name, stim_ordered, cue_ordered) %>%
  summarise(
    mean = mean(mean_per_sub, na.rm = TRUE),
    se = sd(mean_per_sub, na.rm = TRUE) / sqrt(n()),
...
# 3. Plot with facets by session
p <- ggplot(SIIPS_summary, aes(x = stim_ordered, y = mean, 
                                color = cue_ordered, group = cue_ordered)) +
  geom_linerange(aes(ymin = mean - se, ymax = mean + se), 
                 linewidth = 0.5) +
  geom_line(linewidth = 1) +
...
# Save
ggsave(file.path(analysis_dir, paste0("SIIPS_by_session_", Sys.Date(), ".svg")),
       plot = p, width = 10, height = 4, dpi = 300)
