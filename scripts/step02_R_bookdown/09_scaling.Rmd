...
<!--     mutate(OUTCOME_demean = OUTCOME - OUTCOME_cm) %>% #CWC -->
...
<!--   data_a3 <- data_p2 %>% -->
<!--     group_by(!!sym("subject"), !!sym("ses"), !!sym("run")) %>% -->
...
<!--   #data_centered <- data_a3cmc[complete.cases(data_a3cmc$lag.OUTCOME_demean),] -->
<!--   df.centered_NA <- data_a3cmc %>%  -->
<!--     filter(!is.na(OUTCOME)) %>% # Remove NA values -->
<!--     filter(!is.na(EXPECT)) %>% # Remove NA values -->
<!--     select(subject, ses, run, runtype, cue, stimintensity, -->
<!--       # src_subject_id, session_id, param_task_name, param_run_num,  -->
<!--       #      param_cue_type, param_stimulus_type, -->
<!--            OUTCOME, EXPECT, OUTCOME_cm, OUTCOME_demean, EXPECT_cm, EXPECT_demean, -->
<!--            OUTCOME_zscore, EXPECT_zscore, EXPECT_cmc)  -->
<!--   #cat(sprintf("%d rows removed due to NA values", nrow(data_centered) - nrow(df.centered_NA))) -->
...
df.centered_NA$subject <- factor(df.centered_NA$subject)
...
```
```{r}
# Assuming df.centered_NA is your filtered data
g <- ggplot(df.centered_NA, aes(x = subject, y = OUTCOME, fill = subject)) +
  geom_boxplot(outlier.shape = NA, width = 1, position = position_dodge(2)) +  
...
### lmer model compare z score vs nonzscore
```{r}
model.pain_z <- lmer(OUTCOME ~ EXPECT_zscore + (EXPECT_zscore|sub), df.PVC_center[df.PVC_center$runtype == "runtype-pain", ])
model.vic_z <- lmer(OUTCOME ~ EXPECT_zscore + (EXPECT_zscore|sub), df.PVC_center[df.PVC_center$runtype == "runtype-vicarious", ])
model.cog_z <- lmer(OUTCOME ~ EXPECT_zscore + (EXPECT_zscore|sub), df.PVC_center[df.PVC_center$runtype == "runtype-cognitive", ])
model.pain_raw <- lmer(OUTCOME ~ EXPECT + (EXPECT|sub), df.PVC_center[df.PVC_center$runtype == "runtype-pain", ])
model.vic_raw <- lmer(OUTCOME ~ EXPECT + (EXPECT|sub), df.PVC_center[df.PVC_center$runtype == "runtype-vicarious", ])
model.cog_raw <- lmer(OUTCOME ~ EXPECT + (EXPECT|sub), df.PVC_center[df.PVC_center$runtype == "runtype-cognitive", ])
model.pain_cmc <- lmer(OUTCOME ~ EXPECT_demean + EXPECT_cmc + (1|sub), df.PVC_center[df.PVC_center$runtype == "runtype-pain", ])
model.vic_cmc <- lmer(OUTCOME ~ EXPECT_demean + EXPECT_cmc + (1|sub), df.PVC_center[df.PVC_center$runtype == "runtype-vicarious", ])
model.cog_cmc <- lmer(OUTCOME ~ EXPECT_demean + EXPECT_cmc + (1|sub), df.PVC_center[df.PVC_center$runtype == "runtype-cognitive", ])


model.pain_z <- lmer(OUTCOME ~ EXPECT_zscore + (EXPECT_zscore|sub), df.PVC_center[df.PVC_center$runtype == "runtype-pain", ])
model.pain_bothz <- lmer(OUTCOME_zscore ~ EXPECT_zscore + (EXPECT_zscore|sub), df.PVC_center[df.PVC_center$runtype == "runtype-pain", ])
model.pain_bothCM <- lmer(OUTCOME_demean ~ EXPECT_demean + (EXPECT_demean|sub), df.PVC_center[df.PVC_center$runtype == "runtype-pain", ])
model.pain_CM <- lmer(OUTCOME ~ EXPECT_demean + (EXPECT_demean|sub), df.PVC_center[df.PVC_center$runtype == "runtype-pain", ])
...
tidy_model_with_name <- function(model, model_name, task) {
  
  X <- effectsize::standardize_parameters(model, method = "pseudo")
  beta_std <- X$Std_Coefficient[2]
  broom.mixed::tidy(model) %>%
    filter(effect == "fixed", term != "(Intercept)") %>%
    select(term, estimate, p.value) %>%
    mutate(model = model_name,
           estimate_standardized = beta_std) %>%
    select(model, term, estimate,estimate_standardized, p.value)
}
#### SANDBOX 2
...
tidy_model_with_name <- function(model, model_name) {
  ...
  # Exclude the intercept and grab all other standardized coefficients
  beta_stds <- X$Std_Coefficient[X$Parameter != "(Intercept)"]

  # Tidy the model, filter out intercept, and add standardized coefficients
  tidy_data <- broom.mixed::tidy(model) %>%
    filter(effect == "fixed", term != "(Intercept)") %>%
    select(term, estimate, p.value) %>%
    mutate(model = model_name)

  # Add the standardized coefficients to the tidy data
...
```{r}
library(DT)
# # Print or use kable/DT for a nicer table in R Markdown
...
plot_endertofighi(df.PVC_center[df.PVC_center$runtype == "runtype-vicarious", ], "vicarious",color_low = "#5bba6f", color_high = "#054a29" )

plot_endertofighi(df.PVC_center[df.PVC_center$runtype == "runtype-cognitive", ], "cognitive", color_low = "#caf0f8", color_high = "#03045e")
```
...
model.pain_cmc <- glmmTMB(OUTCOME ~ EXPECT_demean + EXPECT_cmc + (EXPECT_demean|sub), df.PVC_center[df.PVC_center$runtype == "runtype-pain", ], )
