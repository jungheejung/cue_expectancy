...
> what about task-specific components?
\n
Some thoughts:
...
df_head <- head(df.centered_NA)  # Example with the mtcars dataset
kable(df_head, format = "html") %>%
...
# sorted_subjects <- sorted_data %>%
#   dplyr::select(!!rlang::sym(subject_key))
...
# summary(model.z)
sjPlot::tab_model(model.z,
              title = paste0(tools::toTitleCase(taskname), ": \nlmer(OUTCOME ~ EXPECT_Z + (EXPECT_Z | sub), data = pvc)"),
              CSS = list(css.table = '+font-size: 12;'))

print_dash("model with raw scores, i.e. non Zscores")
# summary(model.nonz)
sjPlot::tab_model(model.nonz,
              title = paste0(tools::toTitleCase(taskname), ": \nlmer(OUTCOME ~ EXPECT_raw + (EXPECT_raw | sub), data = pvc)"),
              CSS = list(css.table = '+font-size: 12;'))

print_dash("model with raw scores CMC and CWC")
# summary(model.cmc)
sjPlot::tab_model(model.cmc,
              title = paste0(tools::toTitleCase(taskname), ": \nlmer(OUTCOME ~ EXPECT_demean + EXPECT_cmc + (1 | sub), data = pvc)"),
              CSS = list(css.table = '+font-size: 12;'))
...
## This looks at the correlation matrix across participants for
"OUTCOME", "EXPECT_cmc", "EXPECT_demean", "EXPECT_zscore"
Only run within Rmd for Quality Control, it generates 100 plots
```{r include=FALSE}
library(dplyr)
head(df.PVC_center)
...
> Pattern betwen the two plots should be the same
...
### lmer results across models
 
> As we can see... 
 
```{r echo=FALSE}
print("range of CMC values" , range(df.PVC_center[df.PVC_center$runtype == "runtype-pain", "EXPECT_cmc"]))
print("range of demean values" , range(df.PVC_center[df.PVC_center$runtype == "runtype-pain", "EXPECT_demean"]))
...
summary(model.pain_cmc)
sjPlot::tab_model(model.pain_cmc,
              title = paste0(tools::toTitleCase("pain"), ": \nlmer(OUTCOME ~ EXPECT_demean + EXPECT_cmc + (EXPECT_demean | sub), data = pvc)"),
              CSS = list(css.table = '+font-size: 12;'))

cueR::print_dash("lmer Vicarious")
# summary(model.vic_cmc)
sjPlot::tab_model(model.vic_cmc,
              title = paste0(tools::toTitleCase("vicarious"), ": \nlmer(OUTCOME ~ EXPECT_demean + EXPECT_cmc + (EXPECT_demean | sub), data = pvc)"),
              CSS = list(css.table = '+font-size: 12;'))

cueR::print_dash("lmer Cognitive")
# summary(model.cog_cmc)
sjPlot::tab_model(model.cog_cmc,
              title = paste0(tools::toTitleCase("cognitive"), ": \nlmer(OUTCOME ~ EXPECT_demean + EXPECT_cmc + (EXPECT_demean | sub), data = pvc)"),
              CSS = list(css.table = '+font-size: 12;'))

```

### plot random effects across tasks 

> The random effects of expectation effects are domain general.
> They are highly correlated across tasks, although, I wonder how much of this is driven by seeming outliers
...
Here, I plot the random effects in a histogram form, and cross compare across tasks
the code creates two types of visualizations for the random effects of expectation ratings across different tasks: one with separate panels for each task and another with all distributions overlaid in one plot. These visualizations help in comparing the distributions of random effects across different tasks.

> The random effects of expectations are highly similar across tasks, 
where the overlap of the distributions deliver the point
