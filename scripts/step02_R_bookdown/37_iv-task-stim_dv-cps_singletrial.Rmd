...
plot_halfrainclouds_twofactor(
  ...
  analysis_dir <- file.path(main_dir, "analysis", "mixedeffect", "model37_CPS")
  save_fname = file.path(analysis_dir, "plot.png")
) 
```
...
### Linear model ver 1: Kragel2018 ~ stimulus_intenisy 
```{r echo=TRUE, message=TRUE, warning=TRUE, paged.print=FALSE}
model.cogstim <- lmer(Kragel18CogControl ~ stim_con_linear + stim_con_quad +(1 |sub), data = data_screen)
...
model.cogcuestim <- lmer(Kragel18CogControl ~ CUE_high_gt_low*stim_con_linear + CUE_high_gt_low*stim_con_quad + (1|sub), data = data_screen)
...
## Kragel2018 ~ session * cue * stimulus_intensity {.tabset}
