# [fMRI] NPScosine \~ session {#NPScosine_ses}

## What is the purpose of this notebook? {.unlisted .unnumbered}

- Here, I model NPS cosine values as a function of cue, stimulus intensity and expectation ratings.
-
- 11/29/2024: Updated based on NPS corr values
- inputs from ./scripts/step10_nilearn/signature/step02_apply_nps_cosine.m
...
nps$runtype <- "pain"
beh_pain <- beh[beh$runtype == "pain", ]
df_merge <- inner_join(beh_pain, nps, by = c("sub","ses", "run", "runtype", "trial_index", "cue", "stimulusintensity", "singletrial_fname", "expectrating", "expectlabel", "outcomerating", "outcomelabel"))
...
```{r}
df_filter.NA <- df_filter %>% filter(!is.na(nps_cosine))  # Remove NA values
head(df_filter.NA)
# Sort the data by median "outcome" in ascending order
sorted_data <- df_filter.NA %>%
  group_by(sub) %>%
  dplyr::summarize(median_nps = median(nps_cosine, na.rm = TRUE)) %>%
  arrange(median_nps) %>%
  select(sub)
...
ggtitle <- paste(taskname, " - NPS cosine (degree)"); title <- paste(taskname, " - actual")
w <- 10; h <- 6
...
sub_mean <- "mean_per_sub"; group_mean <- "mean_per_sub_norm_mean"; dv_keyword <- "NPScosine"
se <- "se"; subject <- "sub";
...
g <- plot_lineplot_twofactor_subset(subsetNPS, taskname = "pain",
                        ...
                        color = c("low cue" = "#5D5C5C", "#D73027"), ggtitle = "Within pain task: NPS cosine as a function of stimulus intensity level and cue",
                        xlab = "Stimulus intensity", ylab = "NPS (cosine)")
...
g <- plot_lineplot_twofactor(NPSstimcue_groupwise,
                        ...
                        color = c("High" = "#9100C6",
                                  "Med" = "#FF2519",
                                  "Low" = "#FFBB00"),
                        ggtitle = "Within pain task: NPS cosine as a function of stimulus intensity level and cue",
                        xlab = "Stimulus intensity", ylab = "NPS (cosine)")
g + theme(aspect.ratio=.8)
```

## 1. lmer :: Linear model results (NPScosine \~ paintask: 2 cue x 3 stimulus_intensity)
t-tests use Satterthwaite's method ['lmerModLmerTest']
> Modeling random slopes of intensity effects, we find a significant effect of stimintensity on NPS cosine values, but not cue effects. The greater the stimulus intensity, the greater the NPS values (b = xx, SE = xx, t(xx) =  xx, p < xx). There is no significant cue effect
...
model.npscuestim <- lmer(nps_cosine ~
...
sjPlot::tab_model(model.npscuestim,
                  title = "Multilevel-modeling: \nlmer(NPScosine ~ CUE * STIM + (CUE + STIM | sub), data = pvc)",
                  CSS = list(css.table = '+font-size: 12;'))


model.singular <- lmer(nps_cosine ~
                          CUE_high_gt_low*STIM_linear + CUE_high_gt_low*STIM_quadratic +
...
model.npscuestim.summarystats.sub <- meanSummary(df_filter.NA,
                                      c("sub", "CUE_high_gt_low"), "nps_cosine")
...
model.npscuestim <- lmer(nps_cosine ~
                          ...
                          ( STIM_quadratic + CUE_high_gt_low|sub), data = df_filter,
...
NPSstimcue_subjectwise <- meanSummary(df_filter.no1,
                                      c(subject, model_iv1, model_iv2), "nps_cosine")
NPSstimcue_groupwise <- summarySEwithin(
  data = NPSstimcue_subjectwise,
  measurevar = "mean_per_sub",
  withinvars = c(model_iv1, model_iv2),
...
model.npscuestim.no1 <- lmer(nps_cosine ~
                          CUE_high_gt_low*STIM_linear + CUE_high_gt_low*STIM_quadratic +
...
sjPlot::tab_model(model.npscuestim.no1,
                  title = "Multilevel-modeling: \nlmer(NPScosine WITHOUT 1st trial ~ CUE * STIM + (CUE + STIM | sub), data = pvc)",
                  CSS = list(css.table = '+font-size: 12;'))

```

## 3. NPScosine trial wise plot

Let's look at the progression of NPS cosine over time. Is it indicative of convergence, and perhaps a learned effect? This will help us link the results back to prediction error.
...
for (p in 1:total_pages) {
  ...
  g <- plot_per_sub(df_clean, "trial_index_sub", dv = "nps_cosine", sub = "sub", xlab_str = "No. of trials",
             ylab_str = "NPS (cosine)", taskname, ncol_num, nrow_num, p)
  print(g)
}
...
for (p in 1:total_pages) {
  # print(plot_per_sub(df_clean.no1, taskname, ncol_num, nrow_num, p))
  g <- plot_per_sub(df_clean.no1, "trial_index_sub", dv = "nps_cosine", sub = "sub", xlab_str = "No. of trials",
             ylab_str = "NPS (cosine)", taskname, ncol_num, nrow_num, p)
  print(g)
}
...
## 4. NPScosine \~ SES \_ CUE \_ STIM

### Q. Is the cue effect on NPScosine different across sessions? {.unlisted .unnumbered}
...
sjPlot::tab_model(model.npsses,
                  title = "Multilevel-modeling: \nlmer(NPScosine ~ CUE * STIM * SES + (STIM + SES | sub), data = pvc)",
                  CSS = list(css.table = '+font-size: 12;'))
```
...
### model converge [OK: optimizer nlminbwrap]
...
dv <- "nps_cosine"
   ...
   # calculate mean and se
    ...
    se <- "se"
    subject <- "sub"
    ggtitle <- paste(str_to_title(taskname), " - NPS cosine ", sesname )
...
# CUE_high_gt_low+STIM+EXPECT_demean
sjPlot::tab_model(model.npsses,
                  title = "Multilevel-modeling: \nlmer(NPScosine ~ CUE * STIM * SES + (CUE + STIM + SES| sub), data = pain)",
                  CSS = list(css.table = '+font-size: 12;'))
...
df_filter_ses <- df_filter_ses %>%
  ...
  mutate(z_nps_cosine = atanh(nps_cosine)) %>%
  ungroup()

model.npsses2 <- lmer(z_nps_cosine ~
                          CUE_high_gt_low*STIM_linear*SES_linear +
                          CUE_high_gt_low*STIM_quadratic*SES_linear +
                          CUE_high_gt_low*STIM_linear*SES_quadratic +
                          CUE_high_gt_low*STIM_quadratic*SES_quadratic +
...
sjPlot::tab_model(model.npsses2,
                  title = "Multilevel-modeling: \nlmer(ArcTanhNPScosine ~ CUE * STIM * SES + (CUE + STIM + SES| sub), data = pain)",
                  CSS = list(css.table = '+font-size: 12;'))

#### model 3 Archtanh. control for stim effect and see if the residual is cue * ses
model.npsses3 <- lmer(z_nps_cosine ~
...
sjPlot::tab_model(model.npsses3,
                  title = "Multilevel-modeling: \nlmer(ArcTanhNPScosine3 ~ CUE * STIM * SES + (CUE + STIM + SES| sub), data = pain)",
                  CSS = list(css.table = '+font-size: 12;'))

# model 4
model.npsses4 <- lmer(z_nps_cosine ~
                          CUE_high_gt_low*STIM_linear +
                          CUE_high_gt_low*STIM_quadratic + SES_linear +
                          (CUE_high_gt_low + STIM_linear   |
         sub), data = df_filter_ses,
...
sjPlot::tab_model(model.npsses4,
                  title = "Multilevel-modeling: \nlmer(ArcTanhNPScosine4 ~ CUE * STIM + SES + (CUE + STIM | sub), data = pain)",
                  CSS = list(css.table = '+font-size: 12;'))
```
...
for (sesname in c("ses-01", "ses-03", "ses-04")) {
taskname = "pain"
    ggtitle <- paste(taskname, " - NPS cosine product")
    title <- paste(taskname, " - NPScosine")
    subject <- "sub"
    w <- 10
    h <- 6
  ...
  dv <- "nps_cosine"
   ...
   # calculate mean and se
    ...
    se <- "se"
    subject <- "sub"
    ggtitle <- paste(str_to_title(taskname), " - NPS cosine Cooksd removed ", sesname )
    title <- paste(str_to_title(taskname), " - cue level")
    xlab <- ""
    ylab <- "NPS (cosine)"
    ylim <- c(-.1,.25)
