...
analysis_folder <- "book_signature"
  analysis_dir <-
    file.path(main_dir,
              "analysis",
              "mixedeffect",
...
nps <- read.csv(file.path(main_dir, "analysis/fmri/nilearn/deriv01_signature/rampup_plateau", paste0("signature-",signature_key,"_sub-all_runtype-pvc_event-stimulus.tsv")), sep = ",")
# intersect
...
# extract metadata ___
sig_df = df_merge %>% separate(
  singletrial_fname,
  sep = "_",
  c(
    "sub", "ses", "run", "runtype", "event", "trial", "cuetype", "stimintensity"
...
sig_df$stimintensity <- sig_df$stimulusintensity
