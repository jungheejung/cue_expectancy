function multivariate_mediation_brain_results_report(varargin)
    ...
    fprintf('LOADING multivariate mediation results');
    ...
    fprintf('Loaded multivariate mediation!');
    ...
    % ----------------------------------------------------------------
    num_components = 2
    dat_fdr = cell(num_components, 1)
    single_nii_fname = fullfile('/Volumes/spacetop_projects_social/analysis/fmri/spm/multivariate/s03_concatnifti/sub-0065/sub-0065_task-social_run-cognitive_ev-stim_l2norm.nii');
    fprintf('FDR-corrected results');
    % dat_fdr = obj(size(pdm.Wfull,2))
    for i = 1:size(pdm.Wfull,2)
        example_dat = fmri_data(single_nii_fname)
        %% Results table for individual PDMs, FDR-corrected 
        
        disp(strcat('FDR-corrected results, PDM ',num2str(i)));
        disp(strcat('FDR q < .05 = p < ', num2str(pdm.pThreshold(i))));
    
        % threshold PDM at fdr threshold from
        % pdm.pThreshold(1))
        dat_fdr{i} = example_dat;
        ...
        fprintf(strcat('FDR-corrected results, PDM ',num2str(i)));
        disp(strcat('FDR q < .05 = p < ', num2str(pdm.pThreshold(i))));
        ...
        canlab_results_fmridisplay(reg_all_fdr{i},'outline','linewidth',0.1,'montagetype','full hcp');
        drawnow, snapnow;
    ...
    fprintf('FDR-corrected results, joint PDM');
    fprintf('FDR q < .05 = p < %3.8f\n', pdm.pThreshold(i+1));
    ...
    if ~isempty(dat_fdr{i+1})
        ...
        canlab_results_fmridisplay(reg_all_fdr{i+1},'outline','linewidth',0.1,'montagetype','full hcp');
        drawnow, snapnow;
    ...
    fprintf('uncorrected results');
    dat_unc = cell(num_components, 1)
    single_nii_fname = fullfile('/Volumes/spacetop_projects_social/analysis/fmri/spm/multivariate/s03_concatnifti/sub-0065/sub-0065_task-social_run-cognitive_ev-stim_l2norm.nii');
    example_dat = fmri_data(single_nii_fname)
    for i = 1:size(pdm.Wfull,2)
        ...
        fprintf(strcat('uncorrected results, PDM #',num2str(i)));
        fprintf('p uncorrected < %3.8f\n', pthresh);
    
        % threshold PDM at pthresh defined above
        dat_unc{i} = example_dat;
        ...
        canlab_results_fmridisplay(reg_all_unc{i},'outline','linewidth',0.1,'montagetype','full hcp');
        drawnow, snapnow;
    ...
    fprintf('uncorrected results, joint PDM');
    fprintf('p uncorrected < %3.8f\n', pthresh);
    ...
    if ~isempty(dat_unc{i+1})
        ...
        canlab_results_fmridisplay(reg_all_unc{i+1},'outline','linewidth',0.1,'montagetype','full hcp');
        drawnow, snapnow;
    ...
    % fprintf('source reconstructions');
    
    % for n = 1:size(pdm.Wfull,2)
        
    %     fprintf(strcat('source reconstruction map, PDM #',num2str(n)));
    ...
    fprintf('Saving region objects and tables to disk');
