...
%% stimulus intensity -> stim -> actual
...
for r = 1:length(run)
    ...
    stim_input.single_nii = fullfile(main_dir, strcat('/analysis/fmri/spm/multivariate/s03_concatnifti/sub-0065/sub-0065_task-social_run-', run{r}, '_ev-', event,'_l2norm.nii'));
    ...
    options.codeToEvaluate = sprintf('plot_pdm(%s)','stim_input');%strcat('task=', run{r});
    options.format = 'pdf';
