...
print(sub_num)
print(type(sub_num))
sub_str = f"sub-{int(sub_num):04d}"
sub_dir = os.path.join(main_dir, 'analysis', 'fmri', 'fsl', 'multivariate', 'isolate_nifti', sub_str)
print(sub_dir)
...
for i in range(len(key_input['run'])):
    print(f"sub-{int(sub_num):04d}_ses-{int(key_input['ses']):02d}_run-{int(key_input['run'][i]):02d}*.nii.gz")
    exclude_files = glob.glob(os.path.join(sub_dir, f"sub-{int(sub_num):04d}_ses-{int(key_input['ses']):02d}_run-{int(key_input['run'][i]):02d}*.nii.gz"))
print(exclude_files)
