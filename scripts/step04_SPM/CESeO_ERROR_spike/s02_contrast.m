...
disp( sub );
output_dir = fullfile(main_dir, 'analysis', 'fmri', 'spm', 'univariate', 'model02_CESeO_ERROR_spike', ...
'1stLevel', sub);
...
for run_ind = 1: size(A,1)
    ...
    cue_P         = [ m1(task),0,0,0,0,covariate ]; % 01
    cue_V         = [ m2(task),0,0,0,0,covariate ]; % 02
    cue_C         = [ m3(task),0,0,0,0,covariate ]; % 03
    cue_G         = [ m4(task),0,0,0,0,covariate ]; % 04

    stim_P        = [ 0,0,m1(task),0,0,covariate ]; % 05
    stim_V        = [ 0,0,m2(task),0,0,covariate ]; % 06
    stim_C        = [ 0,0,m3(task),0,0,covariate ]; % 07
    stim_G        = [ 0,0,m4(task),0,0,covariate ]; % 08

    stimXexpect_P    = [ 0,0,0,m1(task),0,covariate ]; % 09
    stimXexpect_V    = [ 0,0,0,m2(task),0,covariate ]; % 10
    stimXexpect_C    = [ 0,0,0,m3(task),0,covariate ]; % 11

    motor         = [ 0,1,0,0,1,covariate ]; % 12

    simple_cue_P         = [ m5(task),0,0,0,0,covariate ]; % 13
    simple_cue_V         = [ m6(task),0,0,0,0,covariate ]; % 14
    simple_cue_C         = [ m7(task),0,0,0,0,covariate ]; % 15

    simple_stim_P        = [ 0,0,m5(task),0,0,covariate ]; % 16
    simple_stim_V        = [ 0,0,m6(task),0,0,covariate ]; % 17
    simple_stim_C        = [ 0,0,m7(task),0,0,covariate ]; % 18

    simple_stimXexpect_P    = [ 0,0,0,m5(task),0,covariate ]; % 19
    simple_stimXexpect_V    = [ 0,0,0,m6(task),0,covariate ]; % 20
    simple_stimXexpect_C    = [ 0,0,0,m7(task),0,covariate ]; % 21

    c01 = [ c01  cue_P];          c02 = [ c02  cue_V];          c03 = [ c03  cue_C];          c04 = [ c04  cue_G];
    c05 = [ c05  stim_P];         c06 = [ c06  stim_V];         c07 = [ c07  stim_C];         c08 = [ c08  stim_G];
    c09 = [ c09  stimXexpect_P];  c10 = [ c10  stimXexpect_V];  c11 = [ c11  stimXexpect_C];
    c12 = [ c12  motor];
    c13 = [ c13  simple_cue_P];          c14 = [ c14  simple_cue_V];         c15 = [ c15  simple_cue_C];
    c16 = [ c16  simple_stim_P];         c17 = [ c17  simple_stim_V];        c18 = [ c18  simple_stim_C];
    c19 = [ c19  simple_stimXexpect_P];  c20 = [ c20  simple_stimXexpect_V]; c21 = [ c21  simple_stimXexpect_C];
