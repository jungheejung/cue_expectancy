...
% 2-1. Bayes map of domain general (pain vicarious)_____________________________________________
...
for i = 1:length(pain_bf.dat)
    if pain_bf.dat(i) > thres &&  vic_bf.dat(i)  > thres  && pain_t.dat(i) ~= 0 && vic_t.dat(i) ~= 0 %&& cog_t.dat(i) ~= 0 
...
domaingeneral_pos.fullpath = fullfile(save_bayesdir, 'pain_vicarious_stimepoch_tmap.nii');
