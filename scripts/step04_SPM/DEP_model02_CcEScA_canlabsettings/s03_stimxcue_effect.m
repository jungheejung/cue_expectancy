...
pain_t = dir(fullfile(spm_1stlevel_dir, '*/spmT_0027.nii'));    
...
vic_t = dir(fullfile(spm_1stlevel_dir,'*/spmT_0028.nii'));    
...
cog_t = dir(fullfile(spm_1stlevel_dir,'*/spmT_0029.nii'));    
...
pain_dat.fullpath = fullfile(save_dir, 'stimxcue_P_T.nii');
write(pain_dat)
vic_dat.fullpath = fullfile(save_dir, 'stimxcue_V_T.nii');
write(vic_dat)
cog_dat.fullpath = fullfile(save_dir, 'stimxcue_C_T.nii');
write(cog_dat)
...
drawnow, snapnow;
montage(P_fdr);
saveas(gcf, fullfile(save_dir,'stimxcue_fdr_01pain.png'));

P_unc = threshold(pain_dat, .005, 'unc', 'k', 10);
montage(P_unc)
drawnow, snapnow;
saveas(gcf, fullfile(save_dir,'stimxcue_unc_01pain.png'));
close all
%% V
create_figure('montage'); axis off
drawnow, snapnow;
montage(V_fdr);
saveas(gcf,fullfile(save_dir,'stimxcue_fdr_02vicarious.png'));

V_unc = threshold(vic_dat, .005, 'unc', 'k', 10);
montage(V_unc)
drawnow, snapnow;
saveas(gcf,fullfile(save_dir,'stimxcue_unc_02vicarious.png'));
close all
...
drawnow, snapnow;
montage(C_fdr);
saveas(gcf,fullfile(save_dir,'stimxcue_fdr_03cognitive.png'));

C_unc = threshold(cog_dat, .005, 'unc', 'k', 10);
montage(C_unc)
drawnow, snapnow;
saveas(gcf,fullfile(save_dir,'stimxcue_unc_03cognitive.png'));
close all

create_figure('montage'); axis off
drawnow, snapnow;
montage(G_fdr);
saveas(gcf,fullfile(save_dir,'stimxcue_fdr_04general.png'));

G_unc = threshold(gen_dat, .005, 'unc', 'k', 10);
montage(G_unc)
drawnow, snapnow;
saveas(gcf,fullfile(save_dir,'stimxcue_unc_04general.png'));
close all
