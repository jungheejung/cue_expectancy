...
pain_t = dir(fullfile(spm_1stlevel_dir, '*/spmT_0009.nii'));    
...
vic_t = dir(fullfile(spm_1stlevel_dir,'*/spmT_0010.nii'));    
...
cog_t = dir(fullfile(spm_1stlevel_dir,'*/spmT_0011.nii'));    
...
pain_dat.fullpath = fullfile(save_dir, 'stim_P_gt_VC_T.nii');
write(pain_dat)
vic_dat.fullpath = fullfile(save_dir, 'stim_V_gt_PC_T.nii');
write(vic_dat)
cog_dat.fullpath = fullfile(save_dir, 'stim_C_gt_PV_T.nii');
write(cog_dat)
...
if ~exist(fullfile(save_dir,'stim_contrast'), 'dir');
    mkdir(fullfile(save_dir,'stim_contrast'));
end
%% montage(P_fdr);
create_figure('montage'); axis off
drawnow, snapnow;
montage(P_fdr);
saveas(gcf, fullfile(save_dir,'stim_contrast','stim_P_gt_VC_fdr_01pain.png'));

P_unc = threshold(pain_dat, .005, 'unc', 'k', 10);
montage(P_unc)
drawnow, snapnow;
saveas(gcf, fullfile(save_dir,'stim_contrast','stim_P_gt_VC_unc_01pain.png'));
close all
%% V
create_figure('montage'); axis off
drawnow, snapnow;
montage(V_fdr);
saveas(gcf,fullfile(save_dir,'stim_contrast','stim_V_gt_PC_fdr_02vicarious.png'));

V_unc = threshold(vic_dat, .005, 'unc', 'k', 10);
montage(V_unc)
drawnow, snapnow;
saveas(gcf,fullfile(save_dir,'stim_contrast','stim_V_gt_PC_unc_02vicarious.png'));
close all
...
drawnow, snapnow;
montage(C_fdr);
saveas(gcf,fullfile(save_dir,'stim_contrast','stim_C_gt_PV_fdr_03cognitive.png'));

C_unc = threshold(cog_dat, .005, 'unc', 'k', 10);
montage(C_unc)
drawnow, snapnow;
saveas(gcf,fullfile(save_dir,'stim_contrast','stim_C_gt_PV_unc_03cognitive.png'));
close all
