...
pain_t = dir(fullfile(spm_1stlevel_dir, '*/spmT_0021.nii'));    
...
vic_t = dir(fullfile(spm_1stlevel_dir,'*/spmT_0022.nii'));    
...
cog_t = dir(fullfile(spm_1stlevel_dir,'*/spmT_0023.nii'));    
...
if ~exist(fullfile(save_dir,'cuexcue_simple'), 'dir')
    mkdir(fullfile(save_dir,'cuexcue_simple'));
end
pain_dat.fullpath = fullfile(save_dir,'cuexcue_simple', 'cuexcue_P_T.nii');
write(pain_dat)
vic_dat.fullpath = fullfile(save_dir,'cuexcue_simple', 'cue_V_T.nii');
write(vic_dat)
cog_dat.fullpath = fullfile(save_dir,'cuexcue_simple', 'cue_C_T.nii');
write(cog_dat)
gen_dat.fullpath = fullfile(save_dir,'cuexcue_simple', 'cue_G_T.nii');
write(gen_dat)
motor_dat.fullpath = fullfile(save_dir,'cuexcue_simple', 'motor_T.nii');
write(motor_dat)
...
drawnow, snapnow;
montage(P_fdr);
saveas(gcf, fullfile(save_dir,'cuexcue_simple','cue_fdr_01pain.png'));
close all
P_unc = threshold(pain_dat, .005, 'unc', 'k', 10);
montage(P_unc)
drawnow, snapnow;
saveas(gcf, fullfile(save_dir,'cuexcue_simple','cue_unc_01pain.png'));

%% V
create_figure('montage'); axis off
drawnow, snapnow;
montage(V_fdr);
saveas(gcf,fullfile(save_dir,'cuexcue_simple','cue_fdr_02vicarious.png'));
close all
V_unc = threshold(vic_dat, .005, 'unc', 'k', 10);
montage(V_unc)
drawnow, snapnow;
saveas(gcf,fullfile(save_dir,'cuexcue_simple','cue_unc_02vicarious.png'));
close all
...
drawnow, snapnow;
montage(C_fdr);
saveas(gcf,fullfile(save_dir,'cuexcue_simple','cue_fdr_03cognitive.png'));
close all
C_unc = threshold(cog_dat, .005, 'unc', 'k', 10);
montage(C_unc)
drawnow, snapnow;
saveas(gcf,fullfile(save_dir,'cuexcue_simple','cue_unc_03cognitive.png'));
close all

create_figure('montage'); axis off
drawnow, snapnow;
montage(G_fdr);
saveas(gcf,fullfile(save_dir,'cuexcue_simple','cue_fdr_04general.png'));
close all
G_unc = threshold(gen_dat, .005, 'unc', 'k', 10);
montage(G_unc)
drawnow, snapnow;
saveas(gcf,fullfile(save_dir,'cuexcue_simple','cue_unc_04general.png'));
close all
...
drawnow, snapnow;
montage(M_fdr);
saveas(gcf,fullfile(save_dir,'cuexcue_simple','/motor_fdr.png'));

M_unc = threshold(motor_dat, .005, 'unc', 'k', 10);
montage(M_unc)
drawnow, snapnow;
saveas(gcf,fullfile(save_dir,'cuexcue_simple','/motor_unc.png'));
close all
