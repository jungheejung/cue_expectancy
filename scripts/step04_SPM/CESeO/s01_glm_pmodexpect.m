function s01_glm_pmodexpect(sub, input_dir, main_dir, fmriprep_dir, badruns_json, save_dir)
%-----------------------------------------------------------------------
% spm SPM - SPM12 (7771)
...
for run_ind = 1:size(A, 1)
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(1).name = strcat(task, '_CUE');
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(2).name = strcat(task, '_EXPECT_RATING');
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(3).name = strcat(task, '_STIM');
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(4).name = strcat(task, '_OUTCOME_RATING');
