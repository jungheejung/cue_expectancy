...
#SBATCH -o ./log_con/con_%A_%a.o
#SBATCH -e ./log_con/con_%A_%a.e
#SBATCH --account=DBIC
...
INPUT_DIR="/dartfs-hpc/rc/lab/C/CANlab/labdata/projects/spacetop_projects_cue/analysis/fmri/spm/univariate/model01_6cond_highlowcue_rampplateau"
### GPT
FILE="${MAIN_DIR}/scripts/step00_qc/qc03_fmriprep_visualize/bad_runs.json"
BAD=$(jq -r 'keys[]' ${FILE})   
IFS=$'\n' bad_array=($BAD)
subdirectories=($(find "$INPUT_DIR" -mindepth 1 -maxdepth 1 -type d -name "sub-*" -exec basename {} \;))
...
SAVE_DIR="${INPUT_DIR}/1stlevel_rampup/${PARTICIPANT_LABEL}"
echo ${PARTICIPANT_LABEL}
module load matlab/r2020a
matlab -nodisplay -nosplash -batch "addpath('${SPM_DIR}'); addpath(genpath('${CANLABCORE_DIR}')); addpath(genpath('${MAIN_DIR}/scripts/step04_SPM/6conditions_highlowcue_rampplateau')); s02_contrast_18reg_rampplateau_rampup('${PARTICIPANT_LABEL}', '${INPUT_DIR}', '${MAIN_DIR}', '${SAVE_DIR}');"
