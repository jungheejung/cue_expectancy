function s01_glm_6cond_ttl1(sub, input_dir, main_dir, fmriprep_dir, badruns_json, save_dir)
    ...
    for run_ind = 1:size(A, 1)
        ...
        % NOTE: 10/11/2023 remove TTL 2 onset times in glm
        % if strcmp(task,'pain')
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(1).duration = double(repelem(5, length(double(cue.onset03_stim(highcue(:,1) & highstim(:,1)))))');
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(2).name = 'cue-high_stim-med';
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(2).onset = double(cue.onset03_stim(highcue(:,1) & medstim(:,1)));
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(2).duration = double(repelem(5, length(double(cue.onset03_stim(highcue(:,1) & medstim(:,1)))))');
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(3).name = 'cue-high_stim-low';
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(3).onset = double(cue.onset03_stim(highcue(:,1) & lowstim(:,1)));
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(3).duration = double(repelem(5, length(double(cue.onset03_stim(highcue(:,1) & lowstim(:,1)))))');
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(4).name = 'cue-low_stim-high';
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(4).onset = double(cue.onset03_stim(lowcue(:,1) & highstim(:,1)));
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(4).duration = double(repelem(5, length(double(cue.onset03_stim(lowcue(:,1) & highstim(:,1)))))');
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(5).name = 'cue-low_stim-med';
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(5).onset = double(cue.onset03_stim(lowcue(:,1) & medstim(:,1)));
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(5).duration = double(repelem(5, length(double(cue.onset03_stim(lowcue(:,1) & medstim(:,1)))))');
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(6).name = 'cue-low_stim-low';
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(6).onset = double(cue.onset03_stim(lowcue(:,1) & lowstim(:,1)));
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(6).duration = double(repelem(5, length(double(cue.onset03_stim(lowcue(:,1) & lowstim(:,1)))))');
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(7).duration = double(repelem(1, length(double(cue.onset01_cue)))');
        ...
        % Combine onsets and durations from both EXPECT_RATING and OUTCOME_RATING
        combined_onsets = [double(cue.onset02_ratingexpect); double(cue.onset04_ratingoutcome)];
        combined_durations = [double(cue.pmod_expectRT); double(cue.pmod_outcomeRT)];

        % Sort combined_onsets and their corresponding durations together 
        % This is to make sure events are in temporal order
        [sorted_onsets, sort_idx] = sort(combined_onsets);
        sorted_durations = combined_durations(sort_idx);

        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(8).name = 'RATING';
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(8).onset = sorted_onsets; %double(cue.onset02_ratingexpect);
        matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(8).duration = sorted_durations; %double(cue.pmod_expectRT);
