...
fmriprep_dir = '/Users/h/Downloads/';
...
niilist = dir(fullfile(fmriprep_dir, 'sub-*task-social*_bold.nii'));
...
% extract onset info ________________
...
% sortedonsetT.sub_num(:) = sscanf(char(extractBetween(sortedonsetT.name(:), 'sub-', '_')),'%d');
% sortedonsetT.ses_num(:) = sscanf(char(extractBetween(sortedonsetT.name, 'ses-', '_')),'%d'); 
% sortedonsetT.run_num(:) = sscanf(char(extractBetween(sortedonsetT.name, 'run-', '_')),'%d');
...
%     strcat(sub, '_', ses, '_task-social_acq-mb8_', run, '_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz'));
    nii_fname = fullfile('/Users/h/Downloads',...
...
%     if ~isfile(dir(fullfile(onset_dir, sub, ses, strcat(sub, '_', ses, '_task-social_', strcat('run-', sprintf('%02d', A.run_num(run_ind))), '-*_events.tsv'))))
%         continue
%     end
    onset_fullpath = fullfile(onset_fname.folder, onset_fname.name);
    social        = struct2table(tdfread(onset_fullpath));
...
%     scans = spm_select('Expand',nii_fname);
    matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).scans = cellstr(nii_fname);
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(2).onset = double(social.event02_expect_displayonset(~ismissing(social.event02_expect_RT)));
    matlabbatch{1}.spm.stats.fmri_spec.sess(run_ind).cond(2).duration = double(social.event02_expect_RT(~ismissing(social.event02_expect_RT)));
