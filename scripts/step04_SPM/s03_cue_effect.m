...
% orthviews(con)
% pain
pain_t = dir('/Volumes/social/analysis/fmri/spm/model-01_CcEScaA/1stLevel/*/spmT_0009.nii');    
...
% vicarious
vic_t = dir('/Volumes/social/analysis/fmri/spm/model-01_CcEScaA/1stLevel/*/spmT_0010.nii');    
vic_fldr = {vic_t.folder}; fname = {vic_t.name};
vic_scan_files = strcat(vic_fldr,'/', fname)';
spm('Defaults','fMRI')
vic_fd = fmri_data(vic_scan_files);

cog_t = dir('/Volumes/social/analysis/fmri/spm/model-01_CcEScaA/1stLevel/*/spmT_0011.nii');    
cog_fldr = {cog_t.folder}; fname = {cog_t.name};
cog_scan_files = strcat(cog_fldr,'/', fname)';
spm('Defaults','fMRI')
cog_fd = fmri_data(cog_scan_files);

gen_t = dir('/Volumes/social/analysis/fmri/spm/model-01_CcEScaA/1stLevel/*/spmT_0012.nii');    
...
motor_t = dir('/Volumes/social/analysis/fmri/spm/model-01_CcEScaA/1stLevel/*/spmT_0017.nii');    
...
pain_dat.fullpath = fullfile('/Users/h/Documents/projects_local/social_influence_analysis/figures', 'stimXcue_P_T.nii');
write(pain_dat)
vic_dat.fullpath = fullfile('/Users/h/Documents/projects_local/social_influence_analysis/figures', 'stimXcue_V_T.nii');
write(vic_dat)
cog_dat.fullpath = fullfile('/Users/h/Documents/projects_local/social_influence_analysis/figures', 'stimXcue_C_T.nii');
write(cog_dat)
gen_dat.fullpath = fullfile('/Users/h/Documents/projects_local/social_influence_analysis/figures', 'stimXcue_G_T.nii');
write(gen_dat)
motor_dat.fullpath = fullfile('/Users/h/Documents/projects_local/social_influence_analysis/figures', 'motor_T.nii');
write(motor_dat)
...
drawnow, snapnow;
montage(P_fdr);
saveas(gcf,'../../figures/CUEpmod_fdr_01pain.png');

P_unc = threshold(pain_dat, .005, 'unc', 'k', 10);
montage(P_unc)
drawnow, snapnow;
saveas(gcf,'../../figures/CUEpmod_unc_01pain.png');

%% V
create_figure('montage'); axis off
drawnow, snapnow;
montage(V_fdr);
saveas(gcf,'../../figures/CUEpmod_fdr_02vicarious.png');

V_unc = threshold(vic_dat, .005, 'unc', 'k', 10);
montage(V_unc)
drawnow, snapnow;
saveas(gcf,'../../figures/CUEpmod_unc_02vicarious.png');
close all
...
drawnow, snapnow;
montage(C_fdr);
saveas(gcf,'../../figures/CUEpmod_fdr_03cognitive.png');

C_unc = threshold(cog_dat, .005, 'unc', 'k', 10);
montage(C_unc)
drawnow, snapnow;
saveas(gcf,'../../figures/CUEpmod_unc_03cognitive.png');
close all

create_figure('montage'); axis off
drawnow, snapnow;
montage(G_fdr);
saveas(gcf,'../../figures/CUEpmod_fdr_04general.png');

G_unc = threshold(gen_dat, .005, 'unc', 'k', 10);
montage(G_unc)
drawnow, snapnow;
saveas(gcf,'../../figures/CUEpmod_unc_04general.png');
close all
...
drawnow, snapnow;
montage(M_fdr);
saveas(gcf,'../../figures/motor_fdr.png');

M_unc = threshold(motor_dat, .005, 'unc', 'k', 10);
montage(M_unc)
drawnow, snapnow;
saveas(gcf,'../../figures/motor_unc.png');
