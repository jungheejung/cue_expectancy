---
title: "cue-expectancy_biomarker-extraction_May2022"
author: "Heejung Jung"
date: "5/5/2022"
...
# Stim P > VC
p.fname = '/Volumes/spacetop_projects_social/analysis/fmri/spm/univariate/model-02_CcEScA/extract_nps/model-02_CcEScA_con_0009/model-02_CcEScA_con_0009.csv'
# Stim P > VC
v.fname = '/Volumes/spacetop_projects_social/analysis/fmri/spm/univariate/model-02_CcEScA/extract_nps/model-02_CcEScA_con_0010/model-02_CcEScA_con_0010.csv'
c.fname = '/Volumes/spacetop_projects_social/analysis/fmri/spm/univariate/model-02_CcEScA/extract_nps/model-02_CcEScA_con_0011/model-02_CcEScA_con_0011.csv'
p.df = read.csv(p.fname)
v.df = read.csv(v.fname)
c.df = read.csv(c.fname)
p.df$con = 'pain_gt_VC'
v.df$con = 'vicarious_gt_PC'
c.df$con = 'cognitive_gt_PV'
```

# model summary
```{r}
model.p.nps = lm( nps~1 , p.df)
model.v.nps = lm( nps~1 , v.df)
model.c.nps = lm( nps~1, c.df)
summary(model.p.nps)
summary(model.v.nps)
summary(model.c.nps)
```
# contrasts
```{r}
stim_contrast = rbind(p.df, v.df, c.df)
```

# plot NPS pain rating per temperature

```{r}
a = ggplot(data = stim_contrast, aes(x = con, y = nps)) +
  geom_point(color = 'black') +
  #stat_smooth(method = 'lm') +
  theme_classic()

# e = ggplot(data = stim_contrast, aes(x = expect_rating, y = nps)) +
...
h <- 6
data = stim_contrast
# /Users/h/Dropbox/projects_dropbox/social_influence_analysis/analysis/mixedeffect/extract_biomarker
main_dir = '/Users/h/Dropbox/projects_dropbox/social_influence_analysis'
analysis_dir <- file.path(main_dir, "analysis", "mixedeffect","extract_biomarker", "glm_stimcontrast",as.character(Sys.Date()))
...
# for (taskname in c("pain", "vicarious", "cognitive")) {
    ...
    data$subject_factor <- factor(data$subject)

    data$con_factor = factor(data$con)

    cooksd <- lmer_onefactor_cooksd_randomintercept(
...
```
```{r}
title = 'nps extraction per task'
g <- ggplot(data = data, aes(y = nps, x = con_factor, fill = con_factor)) +
geom_flat_violin(position = position_nudge(x = .2, y = 0), adjust = 1.5, trim = FALSE, alpha = .3, colour = NA) +
geom_point(aes(y = nps, color = con_factor), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
# geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.8) +
      geom_boxplot(
      aes(y = nps, color = con_factor),
      outlier.shape = NA, alpha = 0.8, width = .1, colour = "black"
    ) +
expand_limits(x = 2.8) +
guides(fill = FALSE) +
...
# scale_fill_brewer(palette = "Spectral") +
  scale_fill_manual(values = c('#171893','#941100', '#019052')) +
  scale_color_manual(values = c('#171893','#941100', '#019052')) +
#coord_flip() +
  ggtitle("stimulus contrast - NPS") +
  xlab("stimulus event contrasts") +
  ylab("NPS value") +
...
es = data %>%
  group_by(con_factor) %>%
  mutate(effectsize = mean(nps)/sd(nps))
```
